...
const db = createDb();
const account = await db.execute("SELECT id FROM accounts WHERE display_name = 'Josh Hale' LIMIT 1");
if (!account.rows.length) throw new Error("Run deno task db:owner before seeding the demo");
const repository = new TursoLessonRepository(db);
const stored = await repository.createLesson(String(account.rows[0].id), resolved as any);
await db.execute({
  sql: "UPDATE lesson_revisions SET status = 'published', provenance_json = ?, published_at = COALESCE(published_at, ?) WHERE id = ?",
  args: [JSON.stringify(resolved.normalizedLesson.provenance), Date.now(), stored.revisionId],
});
