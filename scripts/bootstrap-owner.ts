...
const prior = await db.execute({ sql: "SELECT token_prefix, scopes_json FROM api_tokens WHERE account_id = ? AND name = ? AND revoked_at IS NULL", args: [accountId, "owner-cli"] });
if (prior.rows.length) {
  ...
  if (missing.length) {
    ...
    await db.execute({ sql: "UPDATE api_tokens SET scopes_json = ? WHERE token_prefix = ?", args: [JSON.stringify([...scopes, ...missing]), prefix] });
    console.log(`Owner account is ready; token prefix ${prefix} gained the ${missing.join(", ")} scope and was not rotated.`);
  } else {
    console.log(`Owner account is ready; existing token prefix ${prefix} was not rotated.`);
  }
} else {
  ...
  await db.execute({
    ...
    args: [crypto.randomUUID(), accountId, "owner-cli", created.prefix, await tokenHash(created.token), JSON.stringify(OWNER_SCOPES), Date.now()],
  });
...
if (await hasEnv(envPath, "LEARN_SESSION_KEY")) {
  ...
  await setEnv(envPath, "LEARN_SESSION_KEY", base64Url(crypto.getRandomValues(new Uint8Array(32))));
