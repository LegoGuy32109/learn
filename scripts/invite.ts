...
export function parseInviteArgs(
  ...
  const options: InviteOptions = {
    baseUrl: environment.LEARN_BASE_URL || "http://localhost:8000",
    json: false,
    help: false,
  };
...
export async function mintInvite(
  ...
  const response = await fetch(new URL("/api/v1/sign-in-invites", baseUrl), {
    method: "POST",
    headers: { authorization: `Bearer ${token}` },
  });
  const body = await response.json().catch(() => ({}));
  if (response.status === 401) {
    throw new Error("The token was rejected (401). Check LEARN_OWNER_TOKEN.");
  }
  if (response.status === 403) {
    throw new Error(
      "The token lacks the account:owner scope (403). Mint one with deno task token:mint --scopes account:owner.",
    );
  }
  if (!response.ok) {
    throw new Error(
...
if (import.meta.main) {
  try {
    const options = parseInviteArgs(Deno.args, {
      LEARN_BASE_URL: Deno.env.get("LEARN_BASE_URL"),
    });
    if (options.help) {
      console.log(HELP);
    } else {
      ...
      if (!token) {
        throw new Error(
          "LEARN_OWNER_TOKEN is not set. Run deno task db:owner or mint a token with the account:owner scope.",
        );
      }
      ...
      if (options.json) {
        ...
        console.log(
          `Open this link on the phone within ten minutes. It works once:`,
        );
        console.log("");
