...
async function ensureDatabase(name: string): Promise<DatabaseInfo> {
  const existing = (await databases()).find((database) => (database.Name ?? database.name) === name);
...
for (const environment of ["local", "dev"] as const) {
  ...
  const values = { TURSO_DB_URL: `libsql://${hostname}`, TURSO_DB_TOKEN: await token(name) };
