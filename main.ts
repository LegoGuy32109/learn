...
if (!sessionKey) {
  console.warn(
    "LEARN_SESSION_KEY is not set; browser sessions will not survive a restart. Run deno task db:owner to add one to .env.",
  );
...
const app = createApp({
  ...
  relyingParty: configuredRelyingParty({
    rpId: Deno.env.get("WEBAUTHN_RP_ID"),
    origins: Deno.env.get("WEBAUTHN_ORIGINS"),
  }),
  revision: servingRevision(),
  // A visitor who is not signed in sees only a passkey sign-in; the featured lesson is the owner's
  // newest published revision.
  guests: "closed",
});
