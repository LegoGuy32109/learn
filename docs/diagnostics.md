...
| `concept.option.id` | error | `/concepts/<i>/options/<j>/id` | resolver only | The option ID is missing, repeats another option in the Concept, does not match ^[A-Za-z0-9_-]{1,64}$, or is __proto__, constructor or prototype. | Use a short unique slug such as reuse or revalidate. |
...
| `misconception.id` | error | `/concepts/<i>/misconceptions/<j>/id` | resolver only | The misconception ID is missing, repeats another in the Concept, does not match ^[A-Za-z0-9_-]{1,64}$, or is a reserved name. | Use a short unique slug that names the belief, such as fresh_means_newest. |
...
| `mcq.feedback.missing` | error | `/questions/<i>/feedback/<optionId>` | schema and resolver | An option of the Concept, possibly the key, has no feedback entry. | Add feedback for every option ID in the Concept's set, including the key. |
| `mcq.feedback.extra` | error | `/questions/<i>/feedback/<optionId>` | schema and resolver | feedback has a key that is not an option ID of the Concept. | Remove the entry or correct the option ID. |
| `mcq.map.missing` | error | `/questions/<i>/map/<optionId>` | schema and resolver | A distractor option has no map entry. | Map every non-key option ID to a misconception ID in the Concept. |
...
| `mcq.map.extra` | error | `/questions/<i>/map/<optionId>` | schema and resolver | map has an entry for the key or for an ID that is not an option of the Concept. | Keep only the distractor option IDs in map. |
