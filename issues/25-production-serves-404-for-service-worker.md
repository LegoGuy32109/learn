...
- [ ] `GET /sw.js` on production answers 200 `text/javascript` with the
      build hash and precache list substituted.
- [ ] `deno task smoke:prod` checks `/sw.js` and fails when it is missing.
- [ ] `deno task audit:prod` passes the six checks above, including the
      offline reload of `/` and of a learning URL.
- [ ] `deno task check` still type-checks the worker under the `webworker`
