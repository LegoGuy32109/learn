...
function validateCommon(
  ...
  if (typeof event.id !== "string" || !UUID_V4.test(event.id)) {
    fail("event.id", "/id", "Every event needs a UUIDv4 id.");
  }
  if (typeof event.type !== "string" || !types.includes(event.type)) {
    fail("event.type", "/type", `type must be one of ${types.join(", ")}.`);
  }
  if (event.lessonRevisionId !== revisionId) {
    fail(
      "event.revision",
      "/lessonRevisionId",
      "Every event in a push belongs to the push's Lesson Revision.",
    );
  }
  if (event.epoch !== epoch) {
    fail(
      "event.epoch",
      "/epoch",
      "Every event in a push belongs to the push's progress epoch.",
    );
  }
  if (!isIsoTime(event.occurredAt)) {
    fail(
      "event.occurredAt",
      "/occurredAt",
...
function validateLearning(
  ...
  if (event.type === "card_seen") {
    ...
    if (!concept) {
      return reject(
        "concept.unknown",
        "/conceptId",
        "conceptId is not a Concept of this Lesson Revision.",
      );
    }
    if (
      ...
      reject(
        "card.unknown",
        "/cardId",
        "cardId is not a Card of that Concept in this Lesson Revision.",
      );
    }
    return;
  }
  ...
  if (!question) {
    return reject(
      "question.unknown",
      "/questionId",
      "questionId is not a Question of this Lesson Revision.",
    );
  }
  if (question.conceptId !== event.conceptId) {
    reject(
      "question.concept",
      "/conceptId",
      "conceptId is not the Concept that owns this Question.",
    );
  }
  if (question.poolId !== event.poolId) {
    reject(
      "question.pool",
      "/poolId",
      "poolId is not the Pool that owns this Question.",
    );
  }
  if (typeof event.flowKind !== "string" || !FLOW_KINDS.has(event.flowKind)) {
    reject(
      "answer.flowKind",
      "/flowKind",
      "flowKind must be check or wrap_up.",
    );
  }
  if (typeof event.attemptId !== "string" || !event.attemptId) {
    reject(
      "answer.attemptId",
      "/attemptId",
      "attemptId must name the Check or Wrap-up attempt.",
    );
  }
  if (!isAnswer(event.answer)) {
    return reject(
      "answer.value",
      "/answer",
      "answer must be text, a number, or null for I don't know.",
    );
  }
  if (typeof event.correct !== "boolean") {
    return reject("answer.correct", "/correct", "correct must be a boolean.");
  }
  const expected = event.answer !== null &&
    evaluateAnswer(lesson, question, event.answer);
  if (expected !== event.correct) {
    reject(
      "answer.correctness",
      "/correct",
      "correct does not match the shared evaluator's verdict for this answer.",
    );
  }
}
...
function validateNavigation(
  ...
  if (!("checkpoint" in event)) {
    return reject(
      "checkpoint.missing",
      "/checkpoint",
      "A navigation_checkpointed event carries a checkpoint, or null to close the run.",
    );
  }
  ...
  if (!isRecord(checkpoint)) {
    return reject(
      "checkpoint.shape",
      "/checkpoint",
      "checkpoint must be an object or null.",
    );
  }
  const frontier = checkpoint.learningEventFrontier;
  if (
    !Array.isArray(frontier) ||
    !frontier.every((id) => typeof id === "string" && UUID_V4.test(id))
  ) {
    return reject(
      "checkpoint.frontier",
  ...
  if (
    ...
    Number(checkpoint.conceptIndex) >= concepts.length
  ) {
    reject(
      "checkpoint.concept",
      "/checkpoint/conceptIndex",
  ...
  if (
    ...
    !checkpoint.queue.every((id) => typeof id === "string" && known.has(id))
  ) {
    reject(
      "checkpoint.queue",
      "/checkpoint/queue",
      "queue must list Question IDs of this Lesson Revision.",
    );
  }
}
...
export function validateBatch(
  ...
  if (events.length > MAX_BATCH_EVENTS) {
    rejections.push({
      ...
      code: "events.count",
      path: "/events",
      message: `A push carries at most ${MAX_BATCH_EVENTS} events.`,
    });
    return { accepted, rejections };
  }
  ...
  events.forEach((candidate, index) => {
    if (!isRecord(candidate)) {
      rejections.push({
        ...
        code: "event.shape",
        path: `/events/${index}`,
        message: "Every event is an object.",
      });
      return;
    }
    const id = typeof candidate.id === "string" ? candidate.id : null;
    const reject: Reject = (code, path, message) =>
      rejections.push({
        index,
        id,
        code,
    ...
    if (seen.has(candidate.id as string)) {
      reject(
        "event.duplicate",
        "/id",
        "The same event id appears twice in one push.",
      );
      return;
    }
