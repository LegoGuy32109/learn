...
> = {
  unknown: {
    ...
    detail: "Check that the whole link was copied, or ask for a new one.",
  },
  expired: {
    title: "This invite link has expired",
    detail: "An invite works for ten minutes. Ask for a new one.",
  },
  used: {
    title: "This invite link was already used",
    detail:
      "Each invite signs in once. If this phone is not signed in, ask for a new one.",
  },
};
...
export function invitePage(lookup: InviteLookup, token: string): string {
  if (lookup.state !== "valid") {
    ...
    const inner = `<div class="notice" role="alert"><span><strong>${
  ...
  const inner = [
    ...
    `<script>window.__INVITE__=${
      inlineJson({ token, displayName: lookup.account.displayName })
    }</script>`,
