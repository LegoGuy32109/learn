...
import { DIAGNOSTICS, type DiagnosticEntry } from "../../shared/authoring/diagnostics.js";
...
function row(entry: DiagnosticEntry): string {
  ...
  return `| \`${entry.code}\` | ${entry.severity} | \`${entry.path || "(document)"}\` | ${schema} | ${cell(entry.meaning)} | ${cell(entry.fix)} |`;
}

export function diagnosticsMarkdown(): string {
  const errors = DIAGNOSTICS.filter((entry) => entry.severity === "error");
  const warnings = DIAGNOSTICS.filter((entry) => entry.severity === "warning");
  const header = "| Code | Severity | Path | Caught by | Meaning | Fix |\n| --- | --- | --- | --- | --- | --- |";
