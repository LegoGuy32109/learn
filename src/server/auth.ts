...
export const UNAUTHENTICATED: AuthResult = {
  ok: false,
...
export class TokenAuthenticator implements Authenticator {
  ...
  async authenticate(
    ...
    const result = await this.db.execute({
      sql:
        "SELECT id, account_id, token_prefix, scopes_json, expires_at FROM api_tokens WHERE token_hash = ? AND revoked_at IS NULL",
      args: [hash],
    });
