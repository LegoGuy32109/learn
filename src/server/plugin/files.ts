...
export async function pluginTree(
  ...
  return {
    ...
    [`${skill}scripts/lesson-validator.js`]:
      generated["public/tools/lesson-validator.js"],
...
export async function pluginFiles(
  ...
  const bytes = Object.fromEntries(
    Object.entries(tree).map(([path, text]) => [path, encoder.encode(text)]),
  );
  ...
  const repository = await bareRepository(
    bytes,
    `learn-lesson plugin ${PLUGIN_VERSION}, generated from the lesson/v1 contract (${DIAGNOSTIC_CODES.length} diagnostic codes)`,
  );
  const files: Record<string, Uint8Array> = {};
  for (const path of ordered) files[PLUGIN_ROOT + path] = bytes[path];
  files[`${PLUGIN_ROOT}marketplace.json`] = encoder.encode(
    pretty(marketplaceManifest(origin, archiveSha256)),
  );
  files[`${PLUGIN_ROOT}${ARCHIVE_FILE}`] = archive;
  for (const [path, content] of Object.entries(repository.files)) {
    files[`${PLUGIN_ROOT}${REPOSITORY_DIR}/${path}`] = content;
  }
...
export async function committedPluginFiles(
  repo: URL,
