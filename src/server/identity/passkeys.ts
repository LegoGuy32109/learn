...
function malformed(kind: string): PasskeyError {
  return new PasskeyError(
    ...
    `The ${kind} response is not a WebAuthn credential.`,
  );
}
...
export class PasskeyService {
  ...
  async mintInvite(
    ...
    if (!account) {
      throw new PasskeyError(
        404,
        "Account not found",
        "An invite can be minted only for an existing account.",
      );
    }
  ...
  private async requireInvite(
    token: string,
  ): Promise<{ invite: Invite; account: Account }> {
  ...
  async registrationOptions(rp: RelyingParty, inviteToken: string) {
    ...
    const options = await generateRegistrationOptions({
      ...
      excludeCredentials: existing.map((credential) => ({
        id: credential.id,
        transports: credential.transports as Transports | undefined,
      })),
    ...
    await this.repository.insertChallenge({
      ...
      purpose: "register",
      expiresAt: this.clock() + CHALLENGE_TTL_MS,
    });
    return options;
  }
  ...
  async register(
    ...
    ) throw malformed("registration");
    const challenge = decodeClientDataChallenge(
    ...
    if (!consumed || consumed.accountId !== account.id) {
      throw new PasskeyError(
        401,
        "Challenge rejected",
        "The registration challenge is missing, expired, or already used.",
      );
    }
  ...
  async authenticate(rp: RelyingParty, credential: unknown): Promise<SignedIn> {
    ...
    if (!consumed) {
      throw new PasskeyError(
        401,
        "Challenge rejected",
        "The sign-in challenge is missing, expired, or already used.",
      );
    }
    ...
    try {
      verification = await verifyAuthenticationResponse({
        ...
        credential: {
          ...
          transports: stored.transports as Transports | undefined,
        },
      });
    } catch (error) {
      throw new PasskeyError(
    ...
    await this.repository.recordCredentialUse(
      stored.id,
