...
export function validateScopes(scopes: string[]): string[] {
  ...
  if (!unique.length) {
    throw new TokenAdminError("At least one scope is required.");
  }
  const unknown = unique.filter((scope) =>
    !(KNOWN_SCOPES as readonly string[]).includes(scope)
  );
  if (unknown.length) {
    throw new TokenAdminError(
      `Unknown scope: ${unknown.join(", ")}. Known scopes: ${
        KNOWN_SCOPES.join(", ")
      }.`,
    );
  }
...
function validateName(name: string): string {
  ...
  if (trimmed.length > 64) {
    throw new TokenAdminError("A token name is limited to 64 characters.");
  }
  return trimmed;
}
...
function validateExpiry(
  ...
  if (!Number.isSafeInteger(expiresAt)) {
    throw new TokenAdminError("Expiry must be a millisecond timestamp.");
  }
  if (expiresAt <= now) {
    throw new TokenAdminError("Expiry must be in the future.");
  }
  return expiresAt;
}

const COLUMNS =
  "name, token_prefix, scopes_json, created_at, last_used_at, expires_at, revoked_at";
...
export class TokenAdmin {
  ...
  async mint(request: MintRequest): Promise<MintedToken> {
    ...
    const active = await this.db.execute({
      sql:
        "SELECT id FROM api_tokens WHERE account_id = ? AND name = ? AND revoked_at IS NULL",
      args: [request.accountId, name],
    });
    if (active.rows.length) {
      throw new TokenAdminError(
        `An active token named "${name}" already exists. Revoke it or choose another name.`,
      );
    }
    ...
    await this.db.execute({
      sql:
        "INSERT INTO api_tokens(id, account_id, name, token_prefix, token_hash, scopes_json, created_at, expires_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
      args: [
        crypto.randomUUID(),
        request.accountId,
        name,
  ...
  async list(accountId: string): Promise<TokenMetadata[]> {
    const result = await this.db.execute({
      sql:
        `SELECT ${COLUMNS} FROM api_tokens WHERE account_id = ? ORDER BY created_at ASC, token_prefix ASC`,
      args: [accountId],
    });
  ...
  async find(
    accountId: string,
    selector: { prefix?: string; name?: string },
  ): Promise<TokenMetadata | null> {
    const result = selector.prefix
      ? await this.db.execute({
        sql:
          `SELECT ${COLUMNS} FROM api_tokens WHERE account_id = ? AND token_prefix = ?`,
        args: [accountId, selector.prefix],
      })
      : selector.name
      ? await this.db.execute({
        sql:
          `SELECT ${COLUMNS} FROM api_tokens WHERE account_id = ? AND name = ? AND revoked_at IS NULL`,
        args: [accountId, selector.name],
      })
      : null;
    if (!result || !result.rows.length) return null;
    if (result.rows.length > 1) {
      throw new TokenAdminError(
        "The selector matches more than one token. Use --prefix.",
      );
    }
  ...
  async revoke(accountId: string, prefix: string): Promise<TokenMetadata> {
    ...
    if (!existing) {
      throw new TokenAdminError(
        `No token with prefix ${prefix} belongs to this account.`,
      );
    }
    if (existing.revokedAt != null) {
      throw new TokenAdminError(`Token ${prefix} was already revoked.`);
    }
    const now = this.clock();
    await this.db.execute({
      sql:
        "UPDATE api_tokens SET revoked_at = ? WHERE account_id = ? AND token_prefix = ? AND revoked_at IS NULL",
  ...
  async rotate(accountId: string, prefix: string): Promise<RotatedToken> {
    ...
    if (existing.revokedAt != null) {
      throw new TokenAdminError(
        `Token ${prefix} was already revoked and cannot be rotated. Mint a new token instead.`,
      );
    }
    const now = this.clock();
    if (existing.expiresAt != null && existing.expiresAt <= now) {
      throw new TokenAdminError(
        `Token ${prefix} has expired and cannot be rotated. Mint a new token instead.`,
      );
    }
    const created = newPersonalToken();
    const hash = await tokenHash(created.token);
    ...
    const confirmed = await this.db.execute({
      sql:
    ...
    if (!confirmed.rows.length) {
      throw new TokenAdminError(
        `The replacement for ${prefix} was not confirmed. The old token is still active.`,
      );
    }
    await this.db.execute({
      sql:
  ...
  async resolveAccount(
    ...
    if (accountId) {
      ...
      if (!result.rows.length) {
        throw new TokenAdminError(`Account ${accountId} does not exist.`);
      }
      return {
    ...
    const result = await this.db.execute(
      "SELECT id, display_name FROM accounts ORDER BY created_at ASC LIMIT 2",
    );
    if (!result.rows.length) {
      throw new TokenAdminError(
        "No account exists. Run deno task db:owner first.",
      );
    }
    if (result.rows.length > 1) {
      throw new TokenAdminError(
        "More than one account exists. Pass --account <id>.",
      );
    }
    return {
      id: String(result.rows[0].id),
