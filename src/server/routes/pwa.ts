...
export function renderWorker(source: string, build: Build): string {
  if (
    !source.includes(HASH_PLACEHOLDER) || !source.includes(PRECACHE_PLACEHOLDER)
  ) {
...
export function pwaRoutes(dependencies: Dependencies): Route[] {
  const build = dependencies.build ??
    diskBuild(page(null, { signedIn: false, displayName: null }));
  return [
    ...
    route(
      ...
      () =>
        Promise.resolve(
          html(page(null, { signedIn: false, displayName: null })),
        ),
    ),
    ...
    route(
      ...
      async () =>
        script(
          await Deno.readTextFile(new URL("sw-routing.js", workerDirectory)),
        ),
    ),
  ];
}
