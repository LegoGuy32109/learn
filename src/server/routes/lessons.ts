...
async function reader(request: Request, dependencies: Dependencies): Promise<string | Response> {
  const account = await currentAccount(request, dependencies, READ_SCOPE);
  if (account && "forbidden" in account) return problem(403, "Insufficient scope", `This token does not have the ${READ_SCOPE} scope.`);
  if (!account) return problem(401, "Authentication required", "Sign in, or use a valid, unexpired, unrevoked bearer token.");
...
export function lessonRoutes(dependencies: Dependencies): Route[] {
  return [
    ...
    route("GET", "/api/v1/lessons", async (request) => {
      ...
      return privateJson({ revisions: await dependencies.lessons.listMine(accountId) });
    }),

    route("GET", "/api/v1/shelf", async (request) => {
      const accountId = await reader(request, dependencies);
      if (accountId instanceof Response) return accountId;
      return privateJson({ lessons: await dependencies.lessons.shelf(accountId) });
    }),
    ...
    route("GET", "/api/v1/lessons/:lessonId", async (request, params) => {
      const accountId = await reader(request, dependencies);
      if (accountId instanceof Response) return accountId;
      const stored = await dependencies.lessons.latestRevision(accountId, params.lessonId);
    ...
    route("GET", "/api/v1/lessons/:lessonId/revisions/:revisionId", async (request, params) => {
      const accountId = await reader(request, dependencies);
      if (accountId instanceof Response) return accountId;
      const stored = await dependencies.lessons.getRevision(accountId, params.lessonId, params.revisionId);
