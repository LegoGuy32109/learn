...
export const lessonSchema = {
  ...
  properties: {
    schema: { const: "lesson/v1", description: "The lesson schema version. Independent of the API version." },
    schemaVersion: { const: 1, description: "Optional numeric alias of schema; the resolver accepts either." },
    title: { type: "string", minLength: 1, description: "The lesson title shown on the shelf." },
    assumedKnowledge: { type: "string", minLength: 1, description: "One sentence naming what the learner already knows." },
    concepts: { type: "array", minItems: 1, items: { $ref: "#/$defs/concept" }, description: "Concepts in teaching order. Each owns its Cards, shared option set, misconceptions and one Pool ID." },
    questions: {
      type: "array",
      minItems: 4,
      items: { $ref: "#/$defs/question" },
      description: "All Questions, top level, each naming its Concept and that Concept's Pool. Every Pool needs at least three drawable Questions and one reserved Question.",
    },
    sources: { type: "array", minItems: 1, items: { $ref: "#/$defs/source" }, description: "Structured sources the lesson was authored from." },
  ...
  $defs: {
    uuid: {
      ...
      description: "A UUIDv4. Concept, Pool, Card and Question IDs are UUIDv4 and unique across the lesson.",
    },
    localId: {
      type: "string",
      pattern: "^[A-Za-z0-9_-]{1,64}$",
      not: { enum: ["__proto__", "constructor", "prototype"] },
      description: "A short stable identifier, unique within its Concept. Used for option and misconception IDs.",
    },
    ...
    stem: {
      ...
      pattern: "\\S",
      not: {
        pattern: "\\b([Tt]he ([Ss]econd|[Tt]hird|[Oo]ther|[Aa]bove|[Ff]ormer|[Ll]atter)|[Bb]esides the|[Oo]ther than the|[Tt]his approach|[Aa]s mentioned|[Tt]hat same|[Tt]he previous)\\b",
      },
      description: "The Question text. It must stand alone with the Cards removed, so it may not contain an unbound reference such as \"the above\" or \"this approach\".",
    },
    concept: {
      type: "object",
      additionalProperties: false,
      ...
      properties: {
        id: { $ref: "#/$defs/uuid" },
        title: { $ref: "#/$defs/text", description: "The Concept title shown on the rail." },
        statement: { $ref: "#/$defs/text", description: "Optional. What the learner can do after the Concept, as one sentence." },
        poolId: { $ref: "#/$defs/uuid", description: "The ID of this Concept's Pool. Questions reference it as poolId." },
        options: {
          type: "array",
          minItems: OPTION_COUNT,
          maxItems: OPTION_COUNT,
          items: { $ref: "#/$defs/option" },
          description: "Exactly three options shared by every MCQ in the Concept. Option IDs are unique in the Concept. The longest text may be at most 1.35 times the shortest.",
        },
        misconceptions: {
          type: "array",
          items: { $ref: "#/$defs/misconception" },
          default: [],
          description: "The beliefs a wrong option reveals. Optional only for a Concept without MCQs; every misconception must be named by the map of some MCQ in this Concept.",
        },
        cards: { type: "array", minItems: 2, items: { $ref: "#/$defs/card" }, description: "At least two Cards. A Card teaches one idea." },
      },
    },
    option: {
      type: "object",
      ...
      properties: {
        id: { $ref: "#/$defs/localId" },
        text: { $ref: "#/$defs/text", description: "The option as the learner reads it." },
      },
    },
    misconception: {
      type: "object",
      ...
      properties: {
        id: { $ref: "#/$defs/localId" },
        statement: { $ref: "#/$defs/text", description: "The belief itself, written as the learner would hold it." },
        correctingCardId: { $ref: "#/$defs/uuid", description: "A Card in the same Concept that corrects the belief." },
      },
    },
    card: {
      type: "object",
      ...
      properties: {
        ...
        body: {
          ...
          minItems: 1,
          items: { $ref: "#/$defs/text" },
          description: "Paragraphs. Inline HTML such as <code> and <em> is allowed. 120 to 200 words across the Card after tags are stripped. Write at least two paragraphs; the corrective view shows only the first.",
        },
      },
    },
    questionBase: {
      type: "object",
      required: ["id", "conceptId", "poolId", "type", "stem", "correctingCardId"],
      properties: {
        id: { $ref: "#/$defs/uuid" },
        conceptId: { $ref: "#/$defs/uuid", description: "The Concept this Question checks." },
        poolId: { $ref: "#/$defs/uuid", description: "The poolId of that Concept." },
        type: { enum: ["mcq", "numeric", "short"] },
        reserved: { type: "boolean", default: false, description: "A reserved Question is never drawn by a Concept Check; the Wrap-up draws reserved Questions first. Each Pool needs at least one." },
        stem: { $ref: "#/$defs/stem" },
        correctingCardId: { $ref: "#/$defs/uuid", description: "The Card in the Question's Concept shown after I don't know, or after a wrong numeric or short answer." },
      },
    },
    question: {
      type: "object",
      ...
      oneOf: [{ $ref: "#/$defs/mcq" }, { $ref: "#/$defs/numeric" }, { $ref: "#/$defs/short" }],
    ...
    mcq: {
      ...
      properties: {
        type: { const: "mcq" },
        key: { $ref: "#/$defs/localId", description: "The option ID of the correct option in the Concept's shared set." },
        map: {
          type: "object",
          ...
          description: "One entry per distractor: the option ID maps to the ID of the misconception choosing it reveals. The key is not in the map.",
        },
        feedback: {
          type: "object",
          ...
          description: "One sentence or two per option, including the key, keyed by option ID.",
        },
      },
    },
    numeric: {
      ...
      properties: {
        type: { const: "numeric" },
        reserved: { const: false, description: "A numeric Question is never reserved." },
        answer: { type: "number", description: "The number, which must appear in a Card of the Question's Concept." },
        tolerance: { type: "number", exclusiveMinimum: 0, description: "Accepted distance from the answer. Set it from the claim, not the decimal. Zero is not allowed." },
        unit: { type: "string", description: "Optional unit shown beside the input." },
        feedback: { $ref: "#/$defs/text", description: "Shown after any answer." },
      },
    },
    short: {
      type: "object",
      ...
      properties: {
        type: { const: "short" },
        answer: { $ref: "#/$defs/text", description: "The canonical answer. Matching ignores case and surrounding whitespace." },
        aliases: { type: "array", items: { $ref: "#/$defs/text" }, description: "Optional other accepted spellings." },
        feedback: { $ref: "#/$defs/text", description: "Shown after any answer." },
      },
    },
    source: {
      ...
      properties: {
        type: { $ref: "#/$defs/text", description: "The kind of source, such as specification, documentation, repository, commit or conversation." },
        title: { $ref: "#/$defs/text" },
        locator: { $ref: "#/$defs/text", description: "A URL, path, commit SHA or other way to find the source again." },
        capturedText: { type: ["string", "null"], description: "Optional excerpt captured at authoring time." },
      },
    },
    provenance: {
      description: "Which agent authored the lesson. Provide every field or decline explicitly. Omission is invalid.",
      oneOf: [
        {
          type: "object",
          additionalProperties: false,
          required: ["status", "client", "harness", "model", "client_version", "session_reference"],
          properties: {
            status: { const: "provided" },
            client: { $ref: "#/$defs/text", description: "The agent product, or unknown." },
            harness: { $ref: "#/$defs/text", description: "The runtime the agent ran in, or unknown." },
            model: { $ref: "#/$defs/text", description: "The model identifier, or unknown." },
            client_version: { $ref: "#/$defs/text", description: "The client version, or unknown." },
            session_reference: { $ref: "#/$defs/text", description: "An opaque reference to the authoring session, or unknown." },
          },
        },
        { type: "object", additionalProperties: false, required: ["status"], properties: { status: { const: "declined" } } },
      ],
    },
  },
};
