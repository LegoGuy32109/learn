...
const REFRESH = "M20 11a8.1 8.1 0 0 0 -15.5 -2m-.5 -4v4h4 M4 13a8.1 8.1 0 0 0 15.5 2m.5 4v-4h-4";
const BOOK = "M19 4v16h-12a2 2 0 0 1 -2 -2v-12a2 2 0 0 1 2 -2h12z M19 16h-12a2 2 0 0 0 -2 2 M9 8h6";
...
export function renderShelf(root, nav) {
  ...
  const refresh = nav.account.signedIn ? `<button class="iconbtn" data-action="refresh" aria-label="Refresh shelf">${icon(REFRESH)}</button>` : "";
  const head = `<div class="libhead"><div><p class="eyebrow">Your learning</p><h1 class="libtitle">Mine</h1></div>${refresh}</div>`;
  const status = statusLine(nav);
  const notice = nav.notice ? `<p class="notice" role="alert">${escape(nav.notice)}</p>` : "";
  const lessons = entries.length ? entries.map(card).join("") : emptyShelf(nav);
  root.innerHTML = `<section class="page shelf">${head}${status}${notice}${lessons}${accountPanel(nav.account)}</section>`;
...
export function renderLessonPrompt(root, nav) {
  ...
  const title = guest ? "Sign in to open this lesson" : "This lesson could not be opened";
  const reason = `${escape(nav.promptReason ?? "")} ${guest ? "Sign in with your passkey while connected and it will load onto this device." : ""}`.trim();
  const intro = `<div><p class="eyebrow">Lesson</p><h1>${title}</h1><p>${reason}</p></div>`;
  const message = nav.account.message ? `<p class="state" id="account-message" aria-live="polite">${escape(nav.account.message)}</p>` : "";
  const signIn = guest ? '<button class="go" data-action="sign-in">Sign in with a passkey</button>' : "";
  const actions = `<div class="actions stack">${signIn}<button class="go quiet" data-action="shelf">Back to shelf</button></div>`;
  root.innerHTML = `<section class="page overview prompt">${intro}${message}${actions}</section>`;
  bind(root, (action) => handle(action, nav), () => {});
}
 ...
 */
function card(entry) {
  const meta = `${entry.conceptCount} concepts · ${entry.questionCount} questions`;
  const status = `<span class="status ${entry.progress.state}">${stateName(entry.progress)}</span>`;
  const outdated = entry.outdated ? '<span class="status outdated">Outdated</span>' : "";
  const main = `<span class="lmain"><span class="lname">${escape(entry.title)}</span><span class="lmeta">${meta}</span><span class="lstatus">${status}${outdated}</span></span>`;
  return `<button class="lesson" data-action="open:${entry.lessonId}" data-lesson="${entry.lessonId}">${main}<span class="chev">${icon(CHEVRON)}</span></button>`;
}
...
function statusLine(nav) {
  ...
  const text = {
    ...
    failed: "The server could not be reached. Showing the lessons on this device.",
...
function emptyShelf(nav) {
  let text = "Lessons you open while signed in stay on this device. Sign in with your passkey while connected to load your shelf.";
  if (nav.account.signedIn && nav.remoteStatus === "ok") text = "Create a lesson with the agent plugin, then refresh this shelf.";
  else if (nav.account.signedIn) text = "You are offline and no lesson is on this device yet. Reconnect and refresh.";
  return `<div class="empty" id="shelf-empty"><span class="mark">${icon(BOOK)}</span><h3>Nothing on this shelf yet</h3><p>${text}</p></div>`;
}
