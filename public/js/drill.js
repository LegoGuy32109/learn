...
import { activeConcept, current, enterCorrective, leaveCorrective, submitAnswer } from "../../src/client/learning/flow.js";
import { advanceDrill, startDrill } from "../../src/client/learning/drill-flow.js";
import { DRILL_ANSWERED, reduceDrill } from "../../src/shared/learning/drill.js";
import { afterFooterView, drillRailView, drillSummaryView, footerView, regionView } from "../../src/client/learning/views.js";
...
export async function startDrilling(session, nav) {
  ...
  if (!session.drillFlow) session.drillFlow = session.savedDrillCheckpoint || startDrill(session.lesson, newRun());
...
export function renderDrill(root, session, nav) {
  ...
  const close = `<button class="close" data-action="overview" aria-label="Close drill">${icon(CLOSE)}</button>`;
  const header = `<header class="shellhead">${backButton("back", "Back")}<h1>${lesson.title}</h1>${close}</header>`;
  const body = flow.screen === "summary" ? drillSummaryView(outcomes) : regionView(lesson, flow, activeConcept(lesson, flow), current(lesson, flow));
...
async function handle(action, session, nav) {
  ...
  if (action === "back") {
    if (session.drillFlow.screen === "corrective") return commit(session, nav, leaveCorrective(session.drillFlow));
  ...
  if (action === "advance") return commit(session, nav, advanceDrill(session.drillFlow));
  if (action === "corrective") return commit(session, nav, enterCorrective(session.lesson, session.drillFlow));
  if (action === "return") return commit(session, nav, leaveCorrective(session.drillFlow));
}
