...
Deno.test({ name: "audit: adversarial probes on a phone viewport", sanitizeOps: false, sanitizeResources: false, fn: async () => {
  ...
  try {
    ...
    await check("listeners · no console error or warning during the probes", () => {
  ...
  if (failed.length) throw new Error(`${failed.length} probe checks failed; see ${REPORT_PATH}`);
} });
...
async function openFirstCard(page: any) {
  ...
  await expect(page.locator(".cardbody h2")).toHaveText(LESSON.concepts[0].cards[0].heading);
}
...
async function doubleTapContinueOnCard(fresh: Fresh) {
  ...
  await check("double-tap · Continue on Card 1 advances one Card and marks only Card 1 Seen", async () => {
    const seen = (await readStore(page, "learning_events")).filter((event) => event.type === "card_seen").map((event) => event.cardId);
...
async function doubleTapAfterFeedback(fresh: Fresh) {
  const { page, context } = await fresh();
  await openFirstCard(page);
  for (let index = 0; index < LESSON.concepts[0].cards.length; index++) await tap(page, "Continue");
  await expect(page.locator(".qhead")).toBeVisible();
  const asked = await answer(page, false);
  await expect(page.locator(".footer .go")).toHaveText("Try another from this concept");
  const before = await projection(page, "checkpoint");
  await doubleTap(page, '[data-action="advance"]');
  await check("double-tap · Try another after feedback moves to exactly the next unseen Question", async () => {
...
async function enterKeySubmits(fresh: Fresh) {
  ...
  for (let steps = 0; steps < LESSON.questions.length && !(await page.locator("#answer").count()); steps++) {
  ...
  await page.waitForTimeout(300);
  await check("keyboard · Enter in the answer field submits the answer, as the plugin renderer does", async () => {
...
async function emptyAnswerSubmit(fresh: Fresh) {
  ...
  const shown = await signature(page);
  const last = (await readStore(page, "drill_events")).filter((event) => event.type === "drill_question_answered").at(-1);
  observe(
    "empty answer",
    shown.verdict
      ? `Tapping Answer with a blank field on ${JSON.stringify(asked.slice(0, 40))} is graded ${shown.verdict} and recorded as answer ${JSON.stringify(last?.answer)}; the plugin renderer grades a blank field the same way.`
...
async function deadBackControls(fresh: Fresh) {
  ...
  for (let steps = 0; steps < 4 && (await page.locator(".qhead").count()); steps++) {
    await answer(page, true);
    await tap(page, "Continue");
  }
...
async function reloadAtOverviewAfterLearned(fresh: Fresh) {
  ...
  for (let steps = 0; steps < 60 && !(await page.locator(".lessonhero h1").count()); steps++) {
  ...
  observe("overview after Learned", `The overview of a Learned lesson offers ${JSON.stringify(overview.startLabel)}.`);
  await clearProjections(page);
  await page.reload();
  await settle(page);
  const reloaded = await signature(page);
  await check("reload · overview of a Learned lesson comes back as the overview", () => {
...
async function wrapUpRequeueIsLater() {
  ...
  for (let seed = 0; seed < 200; seed++) {
    const next = advanceWrapUp({ queue: ["missed", "b", "c"], correct: false, seed });
    if (next.queue[0] === "missed") immediate.push(seed);
  }
  await check("wrap-up · a missed Concept is never re-asked immediately (200 seeds, three Concepts)", () => {
    expect(immediate).toEqual([]);
  });
  observe("wrap-up requeue", `${immediate.length} of 200 seeds re-ask the missed Concept as the very next Question; e.g. seeds ${immediate.slice(0, 5).join(", ")}.`);
}
...
async function optionOrderPerQuestion() {
  ...
  const flow = startCheck(LESSON, cardsFlow(0), { seed: 7, attemptId: "attempt" });
  const mcqs = LESSON.questions.filter((question: any) => question.conceptId === concept.id && question.type === "mcq");
  const orders = new Set(
    mcqs.map((question: any) => {
      const html = questionView(concept, question, flow);
      return [...html.matchAll(/data-answer="([^"]+)"/g)].map((match) => match[1]).join(",");
    }),
  );
  await check("options · MCQs in one Concept Check attempt do not all share one option permutation", () => {
