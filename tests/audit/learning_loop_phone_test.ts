...
async function fullWalk(browser: any, ORIGIN: string, errors: string[], consoleMessages: string[]) {
...
async function drillFromFresh(browser: any, ORIGIN: string, errors: string[], consoleMessages: string[]) {
  const context = await browser.newContext({ viewport: VIEWPORT, colorScheme: "light" });
  const page = await context.newPage();
  page.setDefaultTimeout(3000);
  attachListeners(page, errors, consoleMessages);
