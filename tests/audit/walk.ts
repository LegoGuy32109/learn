...
export async function fullWalk(
  ...
  await check(
    ...
    async () => {
      ...
      await expect(page.locator(".facts strong")).toHaveText(
        `${LESSON.concepts.length} concepts`,
      );
      await expect(page.locator(".overview .state")).toHaveText("Not started");
      await expect(page.locator(".overview .actions .go").first()).toHaveText(
        "Start lesson",
      );
  ...
  await check(
    ...
    async () => {
      ...
      await expect(page.locator(".cardbody .eyebrow")).toHaveText(
        `${LESSON.concepts[0].title} · Card 1 of ${
          LESSON.concepts[0].cards.length
        }`,
      );
      expect(new URL(page.url()).pathname).toBe(LESSON_PATH);
  ...
  await surface(page, "overview after Back from first Card", {
    screenshot: false,
  });
  ...
  await check(
    ...
    async () => {
      await expect(shelfRow(page, lessonId).locator(".lstatus")).toHaveText(
        "In progress",
      );
  ...
  await surface(page, "shelf after browser Back from first Card", {
    screenshot: false,
  });
  // Recover: make sure we are on Card 1 of Concept 1 before continuing.
  if ((await signature(page)).surface !== "learn") {
    if ((await signature(page)).surface === "shelf") {
      await openFromShelf(page, lessonId);
    }
    await tap(page, "Resume");
  }
  ...
  await check(
    ...
    async () => {
      ...
      expect(seen.map((event) => event.cardId)).toEqual([
  ...
  await check(
    ...
    async () => {
      await expect(page.locator(".cardbody h2")).toHaveText(firstCard);
      expect((await readStore(page, "learning_events")).length).toBe(
        eventsBeforeInspect,
      );
    },
  );
  ...
  await check(
    ...
    async () => {
      await expect(page.locator(".prompt .from")).toHaveText(
        `Concept check · ${LESSON.concepts[0].title}`,
      );
  ...
  await check(
    ...
    async () => {
      ...
      expect(feedbackSignature.controls).toContain(
        "Try another from this concept",
      );
      expect((await readStore(page, "learning_events")).length).toBe(
        eventsBeforeDetour,
  ...
  for (let index = 0; index < LESSON.concepts[1].cards.length; index++) {
    await tap(page, "Continue");
  }
  await check("check 2 · the Check opens for Concept 2", async () => {
    await expect(page.locator(".prompt .from")).toHaveText(
      `Concept check · ${LESSON.concepts[1].title}`,
    );
  });
  ...
  await check(
    ...
    async () => {
      ...
      await expect(
        page.getByRole("button", { name: "Review the correcting card" }),
      ).toBeVisible();
  ...
  await check(
    ...
    async () => {
      await expect(page.locator(".cardbody h2")).toHaveText(
        LESSON.concepts[2].cards[0].heading,
      );
    },
  );
  for (let index = 0; index < LESSON.concepts[2].cards.length; index++) {
    await tap(page, "Continue");
  }
  const drawable3 =
  ...
  for (let attempt = 1; attempt <= drawable3; attempt++) {
    ...
    await checkWrongFeedback(
      page,
      `check 3 wrong ${attempt}`,
      asked,
      last ? "Continue" : "Try another from this concept",
    );
    if (attempt === 2) {
      await surface(page, "check 3 second wrong feedback", {
        screenshot: false,
      });
    }
  ...
  await check(
    ...
    () => {
      ...
      for (const asked of check3Stems) {
        expect(keyFor(asked).question.reserved === true).toBe(false);
      }
    },
  );
  ...
  await check(
    ...
    async () => {
      ...
      await expect(page.locator(".rail i")).toHaveCount(
  ...
  for (
    let steps = 0;
    steps < 6 && (await page.locator(".qhead").count());
    steps++
  ) {
    const asked = await stem(page);
    wrapStems.push(asked);
    await check(
      `wrap-up · Question ${wrapStems.length} is a reserved Question the Checks never showed`,
      () => {
        expect(keyFor(asked).question.reserved).toBe(true);
        expect(askedInChecks.has(asked)).toBe(false);
      },
    );
    if (wrapStems.length === 2) {
      await surface(page, "wrap-up Question 2", { screenshot: false });
    }
    if (wrapStems.length === 3) {
      await surface(page, "wrap-up retried or third Question", {
        screenshot: false,
      });
    }
    await answer(page, true);
    await check(
      `wrap-up · correct answer ${wrapStems.length} marks its Concept Learned`,
      async () => {
        await expect(page.locator(".verdict")).toHaveText("Correct");
        expect(learnedIds(await projection(page, "progress"))).toContain(
          keyFor(asked).concept.id,
        );
      },
    );
    if (wrapStems.length === LESSON.concepts.length + 1) {
      await surface(page, "wrap-up final correct feedback", {
        screenshot: false,
      });
    }
    await tap(page, "Continue");
  ...
  await check(
    ...
    () => {
      ...
      expect(concepts.filter((id) => id === keyFor(wrap1).concept.id).length)
        .toBe(2);
    },
  );
  const retriedIndex = wrapStems.findIndex((asked, index) =>
    index > 0 && keyFor(asked).concept.id === keyFor(wrap1).concept.id
  );
  observe(
    "wrap-up retry",
    `The missed Concept came back as Wrap-up Question ${
      retriedIndex + 1
    } of ${wrapStems.length}, asked with ${
      wrapStems[retriedIndex] === wrap1 ? "the same" : "a different"
    } Question.`,
  );
  await check(
  ...
  observe(
    "Learned summary line",
    `The Learned summary shows ${
      JSON.stringify(await page.locator(".summary .line").allInnerTexts())
  ...
  await check(
    ...
    async () => {
      await expect(page.locator(".overview .actions .go.quiet")).toHaveText(
...
export async function drillFromFresh(
  ...
  await check(
    ...
    async () => {
      await expect(page.locator(".prompt .from")).toContainText(
        `Every question · 1 of ${total}`,
      );
  ...
  await surface(page, "drill Question 1", {
    screenshot: true,
    checkpoint: "drill_checkpoint",
  });
  const asked: string[] = [];
  ...
  for (let position = 1; position <= total; position++) {
    ...
    if (key.isMcq) {
      ...
      if (!optionOrders.has(key.concept.id)) {
        optionOrders.set(key.concept.id, new Set());
      }
      optionOrders.get(key.concept.id)!.add(order);
    }
    const mode = position % 4 === 0
      ? "idk"
      : position % 3 === 0
      ? "wrong"
      : "right";
    if (mode === "idk") {
      await idk(page);
      await check(
        `drill · I don't know at ${position} shows the answer, the correcting Card and Continue`,
        async () => {
          await expect(page.locator(".verdict")).toHaveText("Recorded");
      ...
      await checkWrongFeedback(
        page,
        `drill wrong at ${position}`,
        text,
        "Continue",
      );
      if (position === 3) {
        await surface(page, "drill wrong feedback", {
          screenshot: true,
          checkpoint: "drill_checkpoint",
        });
        await tap(page, "Review the correcting card");
        ...
        await surface(page, "drill corrective Card", {
          screenshot: true,
          checkpoint: "drill_checkpoint",
        });
        await tapAction(page, "back");
      ...
      await answer(page, true);
      await check(
        `drill · correct at ${position} shows Correct and Continue`,
        async () => {
          await expect(page.locator(".verdict")).toHaveText("Correct");
          await expect(page.locator(".footer .go")).toHaveText("Continue");
        },
    ...
    if (position === Math.floor(total / 2)) {
      await surface(page, "drill feedback halfway", {
        screenshot: false,
        checkpoint: "drill_checkpoint",
      });
      await tap(page, "Continue");
      await surface(page, "drill Question after halfway reload", {
        screenshot: false,
        checkpoint: "drill_checkpoint",
      });
      ...
      await check(
        ...
        async () => {
          ...
          await expect(page.locator(".prompt .from")).toContainText(
            `Every question · ${position + 1} of ${total}`,
          );
        },
      );
      continue;
  ...
  await check(
    ...
    () => {
      ...
      for (const question of LESSON.questions) {
        expect(asked).toContain(question.stem);
      }
    },
  );
  ...
  await check(
    ...
    async () => {
      await expect(page.locator(".lessonhero h1")).toHaveText(
        "Every question seen",
      );
      await expect(page.getByText("Drill does not earn Learned."))
        .toBeVisible();
      await expect(page.locator(".summary.drill .line")).toHaveCount(
        LESSON.concepts.length,
      );
      for (const concept of LESSON.concepts) {
        await expect(
          page.locator(".summary.drill .line span").filter({
            hasText: concept.title,
          }),
        ).toBeVisible();
      }
  ...
  await surface(page, "drill summary", {
    screenshot: true,
    checkpoint: "drill_checkpoint",
  });
  ...
  await check(
    ...
    async () => {
      ...
      expect(
        drill.some((event) =>
          event.type === "drill_checkpointed" && event.checkpoint === null
        ),
      ).toBe(true);
    },
  );
...
async function checkQuestionShape(page: any, name: string) {
  await check(
    ...
    async () => {
      const text = await stem(page);
      const key = keyFor(text);
      await expect(page.getByRole("button", { name: "I don't know" }))
        .toBeVisible();
      if (key.isMcq) {
...
async function checkDraftNotCheckpointed(page: any, name: string) {
  ...
  await settle(page);
  await check(
    `${name} · an unanswered short-answer draft reloads blank`,
    async () => {
      await expect(page.locator("#answer")).toHaveValue("");
    },
  );
...
async function checkWrongFeedback(
  ...
  await check(
    `${name} · Not quite, feedback text, belief for a distractor, clamped correcting Card under the action row, ${expectedAction}`,
    async () => {
...
async function snapshot(page: any, lessonId?: string) {
  return {
    ...
    navigation: await readStore(page, "navigation_events"),
    shelfStatus:
      (await shelfRow(page, lessonId).locator(".lstatus").first().textContent())
        ?.trim() ?? null,
  };
}
