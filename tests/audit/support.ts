...
export function keyFor(stem: string) {
  const question = LESSON.questions.find((candidate: any) => candidate.stem === stem);
  if (!question) throw new Error(`Unknown stem: ${stem}`);
  const concept = LESSON.concepts.find((candidate: any) => candidate.id === question.conceptId);
  const keyOption = concept.options.find((option: any) => option.id === question.key);
  const wrongOption = concept.options.find((option: any) => option.id !== question.key);
  return {
    question,
    concept,
...
export async function answer(page: any, correct: boolean): Promise<string> {
  ...
  if (key.isMcq) await page.getByRole("button", { name: value, exact: true }).click();
  else {
...
export async function signature(page: any) {
  return await page.evaluate(() => {
    ...
    const all = (selector: string) => [...document.querySelectorAll(selector)].map((element) => (element.textContent ?? "").trim());
    ...
    return {
      ...
      controls: all(".footer .go, .footer .idk, .footer .back, .actions .go, .source, .close"),
      rail: [...document.querySelectorAll(".rail i b")].map((bar) => (bar as HTMLElement).style.width),
...
function resumeFields(checkpoint: any) {
  ...
  const { screen, flowKind, conceptIndex, cardIndex, seed, attemptId, queue, feedback, detour, wrapTotal, runId, total } = checkpoint;
  return { screen, flowKind, conceptIndex, cardIndex, seed, attemptId, queue, feedback, detour, wrapTotal, runId, total };
}
 ...
 */
export async function reloadSame(page: any, name: string, options: { checkpoint?: string; expected?: (before: any) => any } = {}) {
...
const FORBIDDEN_WORDS = [/\bscores?\b/i, /\bstreaks?\b/i, /\bdifficulty\b/i, /\bmaster(y|ed|ing)?\b/i, /\bestimated?\b/i, /\b\d+\s*(min|mins|minutes?|hours?|hrs?)\b/i, /\bpartial credit\b/i];
...
export async function forbidden(page: any, name: string) {
  ...
  await check(`forbidden · ${name} · no score, streak, difficulty, mastery or time estimate in the DOM`, () => {
    for (const pattern of FORBIDDEN_WORDS) {
      const match = html.match(pattern);
      if (match) throw new Error(`${pattern} matched ${JSON.stringify(match[0])} in ${name}`);
    }
...
export async function visual(page: any, name: string) {
  const report = await page.evaluate(() => {
    ...
    const controls = [...document.querySelectorAll("button, input, summary, a[href]")].filter(visible);
    const small = controls
      .map((element) => {
        const rect = element.getBoundingClientRect();
        return { label: (element.getAttribute("aria-label") ?? element.textContent ?? "").trim().slice(0, 40), w: Math.round(rect.width), h: Math.round(rect.height) };
      })
      .filter((box) => Math.min(box.w, box.h) < 44);
    const idk = document.querySelector(".idk")?.getBoundingClientRect() ?? null;
    const app = document.querySelector("#app") as HTMLElement;
    const container = document.querySelector(".shell, .page") as HTMLElement | null;
    let safeArea = false;
    if (container) {
      for (const sheet of [...document.styleSheets]) {
        ...
        for (const rule of rules) {
          ...
          if (matches && /safe-area-inset-bottom/.test(style.style.paddingBottom || style.style.padding || "")) safeArea = true;
        }
      }
    }
    return {
      ...
      idk: idk ? { left: Math.round(idk.left), right: Math.round(idk.right), width: Math.round(idk.width) } : null,
  ...
  await check(`visual · ${name} · every control is at least 44px on its shortest side`, () => {
  ...
  if (report.idk) {
    await check(`visual · ${name} · I don't know is contained away from the screen edge`, () => {
      expect(report.idk!.left).toBeGreaterThanOrEqual(24);
      expect(report.idk!.right).toBeLessThanOrEqual(report.innerWidth - 24);
    });
  }
  await check(`visual · ${name} · bottom safe-area padding on ${report.container}`, () => {
...
export async function schemes(page: any, name: string) {
  shot += 1;
  const prefix = `${String(shot).padStart(2, "0")}-${name.replace(/[^a-z0-9]+/gi, "-").toLowerCase()}`;
...
export async function surface(page: any, name: string, options: { screenshot?: boolean; reload?: boolean; checkpoint?: string; expected?: (before: any) => any } = {}) {
  await forbidden(page, name);
  await visual(page, name);
  if (options.screenshot) await schemes(page, name);
  if (options.reload !== false) await reloadSame(page, name, { checkpoint: options.checkpoint, expected: options.expected });
}

export function attachListeners(page: any, errors: string[], consoleMessages: string[]) {
...
export function report(title: string) {
  ...
  const lines = [`## ${title}`, "", `${passed.length} passed, ${failed.length} failed, ${observations.length} observations`, ""];
  if (failed.length) {
    lines.push("### Failed", "");
    for (const result of failed) lines.push(`- ${result.name}`, "  ```", ...(result.detail ?? "").split("\n").map((line) => `  ${line}`), "  ```");
    lines.push("");
  }
