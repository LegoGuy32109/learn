...
async function fixture(
  ...
  await Deno.writeTextFile(
    new URL(`fixtures/${file}.json`, here),
...
await fixture("ref-no-concepts", "reference-validator", {
  ...
  exact: false,
  note: "Every question then references a missing Concept as well.",
}, (lesson) => lesson.concepts = []);
await fixture("ref-duplicate-concept-id", "reference-validator", {
  valid: false,
  codes: ["concept.id"],
  exact: false,
  note:
    "Questions of the second Concept now resolve to the first, so pool and Card membership also fail.",
}, (lesson) => lesson.concepts[1].id = lesson.concepts[0].id);
await fixture("ref-no-statement", "reference-validator", {
  valid: true,
  codes: [],
  note:
    "validate.mjs fails a Concept without a statement; lesson/v1 made statement optional (Josh's decision of 2026-09-21).",
}, (lesson) => delete lesson.concepts[0].statement);
await fixture("ref-options-count", "reference-validator", {
  valid: false,
  codes: ["concept.options.count"],
  exact: false,
  note:
    "Two options; every MCQ of the Concept then has an extra feedback and map entry.",
}, (lesson) => lesson.concepts[2].options.pop());
await fixture("ref-option-ratio", "reference-validator", {
  valid: false,
  codes: ["concept.options.ratio"],
}, (lesson) => lesson.concepts[2].options[0].text = "Drain the outbox first.");
await fixture("ref-duplicate-card-id", "reference-validator", {
  valid: false,
  codes: ["card.id"],
  exact: false,
  note:
    "The second Card's ID repeats the first, so the misconceptions and Questions that named it dangle.",
}, (lesson) => lesson.concepts[2].cards[1].id = lesson.concepts[2].cards[0].id);
...
}, (lesson) => lesson.concepts[2].cards[0].body = []);
await fixture("ref-card-words", "reference-validator", {
  valid: false,
  codes: ["card.words"],
}, (lesson) => lesson.concepts[2].cards[0].body = bodyOf(40));
await fixture(
  "ref-card-single-paragraph",
  "reference-validator",
  {
    valid: true,
    codes: ["card.paragraphs.single"],
    note: "A warning; the document stays valid.",
  },
  (lesson) =>
    lesson.concepts[2].cards[0].body = [
      lesson.concepts[2].cards[0].body.join(" "),
    ],
);
await fixture("ref-no-cards", "reference-validator", {
  valid: false,
  codes: ["concept.cards.minimum"],
  exact: false,
  note:
    "Misconceptions and Questions of the Concept lose their correcting Cards too.",
}, (lesson) => lesson.concepts[2].cards = []);
await fixture("ref-misconception-no-statement", "reference-validator", {
  valid: false,
  codes: ["misconception.statement"],
}, (lesson) => lesson.concepts[2].misconceptions[0].statement = "");
await fixture(
  "ref-misconception-card-outside",
  "reference-validator",
  { valid: false, codes: ["misconception.card"] },
  (lesson) =>
    lesson.concepts[2].misconceptions[0].correctingCardId =
      lesson.concepts[0].cards[0].id,
);
await fixture("ref-no-reserved", "reference-validator", {
  valid: false,
...
await fixture("ref-drawable-under-3", "reference-validator", {
  valid: false,
  codes: ["lesson.key.longest", "pool.drawable.minimum"],
  note:
...
await fixture("ref-duplicate-question-id", "reference-validator", {
  valid: false,
  codes: ["question.id"],
}, (lesson) => lesson.questions[1].id = lesson.questions[0].id);
...
await fixture(
  "ref-stem-deixis",
  "reference-validator",
  { valid: false, codes: ["question.stem.unbound"] },
  (lesson) =>
    lesson.questions[0].stem =
      "Which of the above does the server do when a passkey is removed?",
);
await fixture("ref-numeric-reserved", "reference-validator", {
  valid: false,
  codes: ["numeric.reserved"],
}, (lesson) => lesson.questions[2].reserved = true);
...
await fixture("ref-misconception-unused", "reference-validator", {
  ...
  lesson.concepts[2].misconceptions.push({
    id: "spare_belief",
    statement: "Sign-out is instantaneous on every device.",
    correctingCardId: lesson.concepts[2].cards[0].id,
  });
});
await fixture("ref-key-longest", "reference-validator", {
  valid: false,
  codes: ["lesson.key.longest"],
}, (lesson) => {
...
await fixture("key-longest-every-mcq", "key-longest", {
  valid: false,
  codes: ["lesson.key.longest"],
  exact: false,
  note:
    "Same construction as ref-key-longest but with the misconception behind the longest option left unused, as a first draft would.",
}, (lesson) => {
  for (const question of mcqs(lesson)) {
    const concept = conceptOf(lesson, question);
    const longest = concept.options.reduce(
...
for (const phrase of deixis) {
  await fixture(`deixis-${phrase.replaceAll(" ", "-")}`, "deixis", {
    valid: false,
    codes: ["question.stem.unbound"],
  }, (lesson) => {
    lesson.questions[0].stem =
      `When a passkey is removed, what does the server do, ${phrase} step aside?`;
  });
}
await fixture(
  "deixis-capitalised",
  ...
  (lesson) =>
    lesson.questions[0].stem =
      "The above happens when a passkey is removed. What does the server do?",
);
await fixture(
  "deixis-inside-word",
  "deixis",
  {
    valid: true,
    codes: [],
    note: '"the otherwise" is not "the other": the rule is word-bounded.',
  },
  (lesson) =>
    lesson.questions[0].stem =
      "When a passkey is removed, the otherwise idle epoch is used how?",
);

await fixture("reserved-numeric", "reserved-numeric", {
  valid: false,
  codes: ["numeric.reserved"],
}, (lesson) => lesson.questions[2].reserved = true);
await fixture("reserved-numeric-only-reserved", "reserved-numeric", {
  valid: false,
  codes: ["numeric.reserved"],
  note:
    "The numeric is the only reserved Question of its Pool. pool.reserved.missing does NOT fire: the resolver counts a reserved numeric toward the Pool's reserved count even while rejecting it. Harmless today because numeric.reserved always rejects the document, but the two rules disagree about what a reserved Question is.",
}, (lesson) => {
  lesson.questions[4].reserved = false;
  lesson.questions[2].reserved = true;
});

await fixture("nesting-200-deep", "hostile-document", {
  valid: false,
  codes: ["document.nesting"],
...
await fixture("nesting-32-deep", "hostile-document", {
  ...
  unknownProperties: true,
  note:
    "Exactly the limit inside an unknown property: the resolver drops the property; the schema rejects the unknown key.",
}, (lesson) => {
...
await fixture("duplicate-card-id-across-concepts", "duplicate-ids", {
  ...
  codes: ["card.id"],
  exact: false,
}, (lesson) => lesson.concepts[2].cards[0].id = lesson.concepts[0].cards[0].id);
await fixture("duplicate-pool-id-across-concepts", "duplicate-ids", {
  valid: false,
  codes: ["pool.id"],
  exact: false,
}, (lesson) => lesson.concepts[2].poolId = lesson.concepts[0].poolId);
await fixture("duplicate-question-id-across-concepts", "duplicate-ids", {
  valid: false,
  codes: ["question.id"],
}, (lesson) => lesson.questions[9].id = lesson.questions[0].id);
await fixture("duplicate-option-id-across-concepts", "duplicate-ids", {
  valid: true,
  codes: [],
  note:
    "Option IDs are local to a Concept, so the same slug in two Concepts is allowed.",
}, (lesson) => {
...
await fixture(
  "duplicate-misconception-id-in-concept",
  "duplicate-ids",
  { valid: false, codes: ["misconception.id"], exact: false },
  (lesson) =>
    lesson.concepts[2].misconceptions[1].id =
      lesson.concepts[2].misconceptions[0].id,
);

for (const name of ["__proto__", "constructor", "prototype"]) {
  await fixture(`reserved-name-option-${name}`, "reserved-names", {
    valid: false,
    codes: ["concept.option.id.invalid"],
    exact: false,
  }, (lesson) => {
  ...
  await fixture(`reserved-name-misconception-${name}`, "reserved-names", {
    valid: false,
    codes: ["misconception.id.invalid"],
    exact: false,
  }, (lesson) => {
    const concept = lesson.concepts[2];
  ...
  await fixture(`reserved-name-top-level-${name}`, "reserved-names", {
    valid: true,
    codes: [],
    unknownProperties: true,
    note:
      "An unknown top-level key. The resolver drops it; the schema rejects it as a typo.",
  }, (lesson) => {
    ...
    Object.assign(
      ...
      JSON.parse(
        text.replace(
          /^\{/,
          `{${JSON.stringify(name)}:{"valid":true,"polluted":true},`,
        ),
      ),
    );
  });
...
await fixture(
  ...
  "provenance",
  { valid: true, codes: [] },
  (lesson) => lesson.provenance = { status: "declined" },
);
...
await fixture("provenance-declined-with-fields", "provenance", {
  ...
  unknownProperties: true,
  note:
    "Declined plus the five fields: the resolver keeps only the status; the schema rejects the extra fields.",
}, (lesson) => lesson.provenance.status = "declined");
