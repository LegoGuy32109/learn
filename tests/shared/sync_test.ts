...
function checkpoint(
  ...
  return {
    ...
    epoch: 0,
    occurredAt,
    checkpoint: { screen: "card", marker, learningEventFrontier: frontier },
  };
}

Deno.test("union by id keeps one copy of each event and the first copy seen", () => {
  const local = [
    learning("a", "lesson_started"),
    learning("b", "card_seen", { cardId: "x", from: "local" }),
  ];
  const remote = [
    learning("b", "card_seen", { cardId: "x", from: "remote" }),
    learning("c", "card_seen", { cardId: "y" }),
  ];
...
Deno.test("progress reduces to the same state whatever order the events arrive in", () => {
  const cards = lesson.concepts[0].cards.map((card, index) =>
    learning(`card-${index}`, "card_seen", {
      cardId: card.id,
      conceptId: lesson.concepts[0].id,
    })
  );
  const wrapUp = learning("wrap", "question_answered", {
    flowKind: "wrap_up",
...
Deno.test("a checkpoint's frontier counts only learning events the union has accepted", () => {
  ...
  assertEquals(
    frontierCount(
      checkpoint("c1", "2026-09-22T10:00:00.000Z", ["a", "b", "unknown"], "x"),
      accepted,
    ),
    2,
  );
  assertEquals(
    frontierCount({
      ...checkpoint("c2", "2026-09-22T10:00:00.000Z", [], "y"),
      checkpoint: null,
    }, accepted),
    0,
  );
...
Deno.test("a checkpoint that depends on less accepted evidence never replaces one that depends on more, however late or recent", () => {
  const evidence = [
    learning("a", "lesson_started"),
    learning("b", "card_seen"),
    learning("c", "card_seen"),
  ];
  const fuller = checkpoint(
    "full",
    "2026-09-22T10:00:00.000Z",
    ["a", "b", "c"],
    "ahead",
  );
  const staleButLater = checkpoint(
    "stale",
  ...
  assertEquals(
    selectCheckpoint([fuller, staleButLater], evidence)?.marker,
    "ahead",
  );
  assertEquals(
    selectCheckpoint([staleButLater, fuller], evidence)?.marker,
    "ahead",
  );
  // A frontier that names evidence nobody has accepted counts for nothing.
  const bluffing = checkpoint("bluff", "2026-09-22T12:00:00.000Z", [
    "a",
    "ghost-1",
    "ghost-2",
    "ghost-3",
...
Deno.test("equal frontiers: the later client clock wins, then the greater id, so both sides agree without either clock alone", () => {
  ...
  const sameClockLow = checkpoint(
    "00000000-0000-4000-8000-000000000001",
    "2026-09-22T10:00:00.000Z",
    ["a", "b"],
    "low",
  );
  const sameClockHigh = checkpoint(
  ...
  assertEquals(
    selectCheckpoint([sameClockLow, sameClockHigh], evidence)?.marker,
    "high",
  );
  assertEquals(
    selectCheckpoint([sameClockHigh, sameClockLow], evidence)?.marker,
    "high",
  );
  const accepted = new Set(["a", "b"]);
  ...
  assertEquals(
    compareCheckpointEvents(sameClockLow, sameClockLow, accepted),
    0,
  );
});
...
Deno.test("selection ignores other event types and returns null without a checkpoint", () => {
  ...
  const drill = {
    ...checkpoint("d", "2026-09-22T10:00:00.000Z", [], "drill"),
