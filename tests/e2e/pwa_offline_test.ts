...
Deno.test({ name: "phone reopens a cached lesson offline and takes an update between Questions", sanitizeOps: false, sanitizeResources: false, fn: async () => {
  ...
  try {
    ...
    const iconSizes = parsed.manifest.icons.map((icon: { sizes: string }) => icon.sizes);
    ...
    await page.evaluate(() => fetch("/api/v1/schemas/lesson/v1").then((response) => response.text()));
    ...
    await fromIcon.getByRole("button", { name: /How browser HTTP caching works/ }).click();
    await fromIcon.getByRole("button", { name: "Resume" }).click();
    await expect(fromIcon.locator(".cardbody h2")).toHaveText("Age measures stored time");
    const apiOffline = await fromIcon.evaluate(() => fetch("/api/v1/schemas/lesson/v1").then(() => "served", () => "failed"));
    ...
    await page.waitForFunction(async () => (await navigator.serviceWorker.ready).waiting !== null);
    ...
    await page.waitForFunction(() => navigator.serviceWorker.controller !== null);
    await expect(page.locator(".feedback")).toBeVisible();
    await page.waitForFunction(() => caches.keys().then((names) => names.length === 1));
    expect(await page.evaluate(() => caches.keys())).toEqual(["learn-shell-e2e000000001"]);
  } finally {
    await browser.close();
