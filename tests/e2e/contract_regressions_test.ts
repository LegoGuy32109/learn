...
Deno.test({
  name: "resume, corrective routing, and I don't know obey the flow contract",
  sanitizeOps: false,
    ...
    try {
...

      {
        ...
        for (let index = 0; index < 3; index++) {
          ...
          await page.getByRole("button", { name: "Review the correcting card" })
            .click();
          const heading = (await page.locator(".cardbody h2").textContent()) ??
            "";
          expect(conceptHeadings).toContain(heading);
          await page.getByRole("button", { name: "Return to questions" })
            .click();
          const label = index < 2
