...
Deno.test({
  name:
    "phone shelf lists the account's lessons, caches on open, works offline, and handles an outdated revision",
  sanitizeOps: false,
  ...
  fn: async () => {
    ...
    const agent = (path: string, body: string) =>
      fetch(`${origin}${path}`, {
        method: "POST",
        headers: {
    ...
    try {
      ...
      await expect(page.locator(".lesson").first()).toContainText(
        fixture.title,
      );
      ...
      await expect(page.locator(".lesson").first()).not.toContainText(
        "Outdated",
      );
      ...
      await expect(page.getByRole("button", { name: "Start lesson" }))
        .toBeVisible();
      const cachedIds = (await readStore(page, "lessons")).map((record) =>
        record.id
      );
      ...
      expect(
        offlineEvents.every((event) =>
          event.lessonRevisionId === created.revisionId && event.epoch === 0
        ),
      ).toBe(true);
      ...
      await expect(ownerPage.getByRole("button", { name: "Start lesson" }))
        .toBeVisible();
      await laptop.close();
      ...
      await expect(emptyPage.locator("#shelf-empty")).toContainText(
      ...
      await expect(emptyPage.locator("#shelf-empty")).toContainText(
        "Sign in with your passkey while connected",
      );
