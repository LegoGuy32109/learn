...
Deno.test({
  name: "two taps within one render cycle have the effect of one tap",
  sanitizeOps: false,
  ...
  fn: async () => {
    ...
    try {
      ...
      await expect(
        page.getByRole("button", { name: "Try another from this concept" }),
      ).toBeVisible();
      ...
      if (await page.locator(".opt").count()) {
        ...
        const answered = await page.evaluate(() =>
          new Promise((resolve, reject) => {
            const request = indexedDB.open("learn-local-v1");
            request.onsuccess = () => {
              const read = request.result.transaction(
                "learning_events",
                "readonly",
              ).objectStore("learning_events").getAll();
              read.onsuccess = () =>
