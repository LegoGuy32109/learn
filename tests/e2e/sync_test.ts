...
Deno.test({
  name:
    "two phones signed in as one account merge offline progress without losing Learned, and a stale checkpoint never moves either back",
  sanitizeOps: false,
  sanitizeResources: false,
  fn: async () => {
    const dependencies = await fixtureDependencies();
    dependencies.lessons = new FixtureLessonRepository(
      fixture,
      FIXTURE_ACCOUNT.id,
    );
    ...
    try {
      ...
      await a.goto(`${origin}/`);
      await expect(a.locator("#account-status")).toHaveText(
        "Signed in as Josh",
      );
      ...
      expect(new Set(eventsOnB.map((event) => event.id))).toEqual(
        new Set(eventsOnA.map((event) => event.id)),
      );
      await expect(status(b)).toHaveText("Synced");
      ...
      await expect.poll(() => learnedConcepts(a), { timeout: 10000 }).toEqual(
        bothLearned,
      );
      ...
      await expect(status(b)).toHaveText("Synced", { timeout: 15000 });
      await expect.poll(() => learnedConcepts(b), { timeout: 10000 }).toEqual(
        bothLearned,
      );
      const unionOnA = await readStore(a, "learning_events");
      const unionOnB = await readStore(b, "learning_events");
      expect(new Set(unionOnA.map((event) => event.id))).toEqual(
        new Set(unionOnB.map((event) => event.id)),
      );
      ...
      for (
        let steps = 0;
        steps < 8 &&
        !(await a.getByRole("heading", { name: "Learned", exact: true })
          .count());
        steps++
      ) {
      ...
      await expect(a.getByRole("heading", { name: "Learned", exact: true }))
        .toBeVisible();
      ...
      await b.getByRole("button", { name: new RegExp(fixture.title) }).click();
      await expect(status(b)).toHaveText("Synced", { timeout: 15000 });
      await expect(b.locator(".overview .state")).toHaveText("Learned", {
        timeout: 10000,
      });
      await b.getByRole("button", { name: "Resume" }).click();
      await expect(b.getByRole("heading", { name: "Learned", exact: true }))
        .toBeVisible();
      ...
      const early = navigationOnB.filter((event) =>
        event.checkpoint?.screen === "card"
      ).sort((x, y) => x.occurredAt.localeCompare(y.occurredAt))[0];
      ...
      const pushed = await fetch(
        `${origin}/api/v1/progress/navigation-events`,
        {
          method: "POST",
          headers: {
            cookie: `learn_session=${cookieValue}`,
      ...
      for (const [page, context] of [[b, deviceB], [a, deviceA]] as const) {
        ...
        await expect.poll(
          async () =>
            (await readStore(page, "navigation_events")).some((event) =>
              event.id === stale.id
            ),
          { timeout: 10000 },
        ).toBe(true);
        ...
        await expect(
          page.getByRole("heading", { name: "Learned", exact: true }),
        ).toBeVisible();
...
Deno.test({
  name:
    "with every sync request failing, learning completes, every event stays in the outbox, and the state says so",
  sanitizeOps: false,
  sanitizeResources: false,
  fn: async () => {
    const dependencies = await fixtureDependencies();
    ...
    try {
      ...
      await continueOn(page);
      await expect(page.locator(".cardbody h2")).toHaveText(
        fixture.concepts[1].cards[0].heading,
      );
      ...
      for (const event of await readStore(page, "navigation_events")) {
      ...
      expect(new Set(pulled.map((event) => event.id))).toEqual(
        new Set(local.map((event) => event.id)),
      );
