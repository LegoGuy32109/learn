...
Deno.test({
  name:
    "drill asks every Question, resumes after a reload, and leaves the shelf Not started",
  sanitizeOps: false,
  ...
  fn: async () => {
    ...
    try {
      ...
      await expect(page.getByText("Drill does not earn Learned."))
        .toBeVisible();
      await page.getByRole("button", { name: "Every question", exact: true })
        .click();
      await expect(page.locator(".prompt .from")).toContainText(
        "Every question · 1 of 12",
      );
      expect(new URL(page.url()).pathname).toBe(
        `/learn/${lesson.lessonId}/drill`,
      );
      ...
      for (let position = 1; position <= lesson.questions.length; position++) {
        await expect(page.locator(".prompt .from")).toContainText(
          `Every question · ${position} of ${lesson.questions.length}`,
        );
        ...
        if (position === 6) {
          ...
          await expect(page.locator(".prompt .from")).toContainText(
            `Every question · ${position + 1} of ${lesson.questions.length}`,
          );
          continue;
        }
        await page.locator(".footer .go").click();
      ...
      await expect(page.getByRole("heading", { name: "Every question seen" }))
        .toBeVisible();
      await expect(page.getByText("Drill does not earn Learned."))
        .toBeVisible();
      ...
      await expect(page.getByRole("button", { name: "Start lesson" }))
        .toBeVisible();
      await expect(
        page.getByRole("button", { name: "Every question", exact: true }),
      ).toBeVisible();
