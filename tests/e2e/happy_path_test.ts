...
import { answerCorrectly, clearProjections, openFirstCard, stem } from "./support/demo.ts";
...
Deno.test({ name: "phone learner resumes and reaches Learned", sanitizeOps: false, sanitizeResources: false, fn: async () => {
  ...
  try {
    ...
    await page.reload();
    await expect(page.getByText("A fresh response can be reused")).toBeVisible();
    let reloadedFeedback = false;
    const checkStems = new Set<string>();
    const wrapUpStems: string[] = [];
    for (let steps = 0; steps < 60 && !(await page.getByText("Learned", { exact: true }).count()); steps++) {
      ...
      if (await continueButton.count() && !(await page.locator(".qhead").count())) {
        await continueButton.click();
        await page.waitForTimeout(40);
        continue;
      }
      if (!(await page.locator(".qhead").count())) throw new Error(await page.locator("body").innerText());
      const wrapUp = ((await page.locator(".prompt .from").textContent()) ?? "").includes("Wrap-up");
    ...
    for (const asked of wrapUpStems) expect(checkStems.has(asked)).toBe(false);
    const body = await page.locator("body").innerText();
    for (const banned of [/\bscore\b/i, /\bstreak\b/i, /\bdifficulty\b/i, /\bmastery\b/i, /\bminutes\b/i]) expect(body).not.toMatch(banned);
