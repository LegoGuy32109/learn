...
import { createAccount, createEphemeralDatabase } from "../db/support/ephemeral.ts";
...
async function freshLesson(db: any, accountId: string, suffix: string) {
  ...
  assert(resolved.valid && resolved.normalizedLesson, `fixture with title "${document.title}" failed to resolve`);
  ...
  await db.execute({ sql: "UPDATE lesson_revisions SET status = 'published', published_at = ? WHERE id = ?", args: [Date.now(), stored.revisionId] });
  return stored;
}

/** A second revision of an existing Lesson, so its shelf entry gets a new `latestRevisionId`. */
async function nextRevision(db: any, accountId: string, lessonId: string, suffix: string) {
  const document = structuredClone(fixture) as Record<string, unknown>;
  delete document.lessonId;
  delete document.revisionId;
  document.title = `${fixture.title} (${suffix})`;
  const resolved = await resolveLesson(document);
  assert(resolved.valid && resolved.normalizedLesson, `fixture with title "${document.title}" failed to resolve`);
  return await new TursoLessonRepository(db).createRevision(accountId, lessonId, resolved as any);
}
...
async function finishWrapUp(page: any) {
  for (let steps = 0; steps < 10 && !(await page.getByRole("heading", { name: "Learned", exact: true }).count()); steps++) {
    await answerCorrectly(page);
    await expect(page.getByText("Correct", { exact: true })).toBeVisible();
    await continueOn(page);
  }
...
async function startOrResume(page: any) {
  await expect(page.getByRole("button", { name: "Start lesson" }).or(page.getByRole("button", { name: "Resume" }))).toBeVisible();
...
async function click(page: any, name: string, options: Record<string, unknown> = {}) {
  try {
...
async function fileDefect(number: number, slug: string, body: string) {
  ...
  defects.push({ number, slug, title: body.split("\n")[0].replace(/^#\s*\d+\s*—\s*/, "") });
}

Deno.test({ name: "cross-device sync, verified adversarially against an ephemeral database", sanitizeOps: false, sanitizeResources: false, fn: async (t) => {
  ...
  try {
    ...
    const setCookie = await dependencies.sessions.issue(new Request(origin), { accountId, displayName: "Josh Hale" });
    ...
    async function inspect(revisionId: string, epoch = 0) {
      const learningRows = await db.execute({ sql: "SELECT id, epoch FROM progress_events WHERE account_id = ? AND lesson_revision_id = ? AND epoch = ?", args: [accountId, revisionId, epoch] });
      const navigationRows = await db.execute({ sql: "SELECT id, epoch FROM navigation_events WHERE account_id = ? AND lesson_revision_id = ? AND epoch = ?", args: [accountId, revisionId, epoch] });
      const navigation = (await progressRepo.all(accountId, "navigation", { lessonRevisionId: revisionId, epoch })).map((row) => row.event);
      const learning = (await progressRepo.all(accountId, "learning", { lessonRevisionId: revisionId, epoch })).map((row) => row.event);
    ...
    // ---------------------------------------------------------------------------------------------
    await t.step("1. A learns online, B resumes at A's exact Card, then at A's exact submitted feedback screen", async () => {
      ...
      try {
        ...
        await expect(a.page.locator(".cardbody h2")).toHaveText(fixture.concepts[0].cards[0].heading);
        await continueOn(a.page);
        await expect(a.page.locator(".cardbody h2")).toHaveText(fixture.concepts[0].cards[1].heading);
        await expect(status(a.page)).toHaveText("Synced", { timeout: 25000 });

        // B opens fresh and resumes: it must land on A's exact second Card, not the first.
        await openLesson(b.page, origin, title);
        await expect(status(b.page)).toHaveText("Synced", { timeout: 25000 });
        await b.page.getByRole("button", { name: "Resume" }).click();
        await expect(b.page.locator(".cardbody h2")).toHaveText(fixture.concepts[0].cards[1].heading);
        ...
        await expect(a.page.getByText("Correct", { exact: true })).toBeVisible();
        await expect(status(a.page)).toHaveText("Synced", { timeout: 25000 });
        ...
        await expect(b.page.getByText("Correct", { exact: true })).toBeVisible({ timeout: 15000 });

        const row = await inspect(lesson.revisionId);
        const localLearning = await readStore(a.page, "learning_events");
        const localNavigation = await readStore(a.page, "navigation_events");
        assertEquals(row.learningCount, localLearning.length, "the server holds exactly the learning events A generated: lesson_started, two card_seen, one question_answered");
        assertEquals(row.navigationCount, localNavigation.length, "the server holds exactly A's checkpoints: on Start, after each Continue, and after the submitted answer");
    ...
    for (const order of ["A then B", "B then A"] as const) {
      await t.step(`2. A and B both offline complete different Concepts, reconnect ${order}`, async () => {
        const lesson = await freshLesson(db, accountId, `scenario 2 ${order}`);
        const title = lesson.content.title;
        const a = await newDevice();
        const b = await newDevice();
        try {
          ...
          await expect(b.page.locator(".cardbody h2")).toHaveText(fixture.concepts[0].cards[0].heading);
          await readCards(b.page, fixture.concepts[0].cards.length);
          await answerCorrectly(b.page);
          await expect(b.page.getByText("Correct", { exact: true })).toBeVisible();
          ...
          for (const device of devices) {
            ...
            await expect(status(device.page)).toHaveText("Synced", { timeout: 25000 });
          }

          const row = await inspect(lesson.revisionId);
          assertEquals(row.learningCount, new Set([...(await readStore(a.page, "learning_events")).map((e: any) => e.id), ...(await readStore(b.page, "learning_events")).map((e: any) => e.id)]).size, "the server's union has every learning event exactly once");
          assertEquals(new Set(row.learningIds).size, row.learningCount, "no learning event is duplicated in progress_events");
    ...
    // ---------------------------------------------------------------------------------------------
    await t.step("3. A and B both offline answer the same Wrap-up Question differently; Learned is never lost and the checkpoint follows the larger frontier, with a deterministic tie breaker", async () => {
      ...
      try {
        ...
        const learningBefore = await Promise.all([readStore(a.page, "learning_events"), readStore(b.page, "learning_events")]);
        ...
        assertEquals(merged.learningCount, new Set([...learningBefore[0].map((e: any) => e.id), ...learningBefore[1].map((e: any) => e.id)]).size, "both answers to the same Question are kept, Learned never lost");
        ...
        const checkpointed = (marker: string, occurredAt: string, id: string) => ({
          id, type: "navigation_checkpointed", lessonRevisionId: lesson.revisionId, epoch: 0, occurredAt,
          checkpoint: { screen: "summary", conceptIndex: 0, cardIndex: 0, flowKind: "wrap_up", seed: 0, attemptId: "", queue: [], feedback: null, detour: null, marker, learningEventFrontier: ids },
        });
        const low = checkpointed("tie-low", "2026-09-22T09:00:00.000Z", "00000000-0000-4000-8000-00000000000a");
        const high = checkpointed("tie-high", "2026-09-22T09:00:00.000Z", "ffffffff-ffff-4fff-bfff-ffffffffffff");
        const pushed = await fetch(`${origin}/api/v1/progress/navigation-events`, {
          method: "POST", headers: { ...cookieHeader, "content-type": "application/json" },
          body: JSON.stringify({ lessonRevisionId: lesson.revisionId, epoch: 0, events: [low, high] }),
        });
        assertEquals(pushed.status, 200);
        await pushed.body?.cancel();
        const served = await (await fetch(`${origin}/api/v1/progress/checkpoint?revision=${lesson.revisionId}&epoch=0`, { headers: cookieHeader })).json();
        assertEquals(served.checkpoint.marker, "tie-high", "equal frontiers and equal clocks break the tie by the greater event id");

        const raw = await inspect(lesson.revisionId);
        assertEquals(selectCheckpoint(raw.navigation, raw.learning), served.checkpoint, "replaying the raw rows through the shared rule reproduces the served checkpoint");
        expect(a.errors).toEqual([]);
        expect(b.errors).toEqual([]);
      } finally {
        await a.context.close();
    ...
    // ---------------------------------------------------------------------------------------------
    await t.step("4. A discards progress; B, offline with old-epoch events, reconnects and is rejected, not resurrected", async () => {
      ...
      try {
        ...
        const revision2 = await nextRevision(db, accountId, lesson.lessonId, "scenario 4, revision 2");

        // A refetches the shelf (a fresh navigation, not client routing) and sees Outdated.
        await openLesson(a.page, origin, title);
        await expect(status(a.page)).toHaveText("Synced", { timeout: 25000 });
        await expect(a.page.locator(".status.outdated")).toHaveText("Outdated");
        await a.page.getByRole("button", { name: "Discard progress and start the new revision" }).click();
        await a.page.getByRole("button", { name: "Discard and start the new revision", exact: true }).click();
        await expect(a.page.locator(".overview .state")).toHaveText("Not started");
        await expect(status(a.page)).toHaveText("Synced", { timeout: 25000 });

        const streamRow = await db.execute({ sql: "SELECT epoch, lesson_revision_id FROM progress_streams WHERE account_id = ? AND lesson_id = ?", args: [accountId, lesson.lessonId] });
        assertEquals(String(streamRow.rows[0].lesson_revision_id), revision2.revisionId);
        const newEpoch = Number(streamRow.rows[0].epoch);
        expect(newEpoch).toBeGreaterThan(0);

        const beforeStaleCount = (await db.execute({ sql: "SELECT COUNT(*) AS n FROM progress_events WHERE account_id = ? AND lesson_revision_id = ? AND epoch = 0", args: [accountId, lesson.revisionId] })).rows[0].n;

        // B reconnects: its old-epoch push is refused. The UI must offer the explicit discard flow.
        await b.context.setOffline(false);
        await expect(status(b.page)).toHaveText("Sync failed", { timeout: 25000 });
        const banner = b.page.locator("#sync-discard");
        await expect(banner).toBeVisible({ timeout: 20000 });
        await expect(banner).toContainText("discarded on another device");

        const afterStaleCount = (await db.execute({ sql: "SELECT COUNT(*) AS n FROM progress_events WHERE account_id = ? AND lesson_revision_id = ? AND epoch = 0", args: [accountId, lesson.revisionId] })).rows[0].n;
        assertEquals(afterStaleCount, beforeStaleCount, "B's stale-epoch push never stores anything server-side");

        await banner.getByRole("button", { name: "Discard here" }).click();
        await expect(b.page.locator(".overview .state")).toHaveText("Not started", { timeout: 20000 });
    ...
    // ---------------------------------------------------------------------------------------------
    await t.step("5. The server drops every request for a full lesson: the device finishes, shows Sync failed, the outbox holds every event, and the server returns to sync once with nothing duplicated", async () => {
      ...
      try {
        ...
        await openLesson(device.page, origin, title);
        await device.page.getByRole("button", { name: "Start lesson" }).click();
        ...
        const navigation = await readStore(device.page, "navigation_events");
        ...
        for (const event of navigation) expect(queuedIds.has(event.id)).toBe(true);
        assertEquals(outbox.length, learning.length + navigation.length, "the outbox holds every event from the whole lesson, learning and navigation alike");

        await device.page.unroute("**/api/v1/progress/**");
        await device.context.setOffline(true);
        await device.context.setOffline(false);
        await device.page.reload();
        await expect(status(device.page)).toHaveText("Synced", { timeout: 20000 });
        expect(await readStore(device.page, "outbox")).toEqual([]);

        const row = await inspect(lesson.revisionId);
        assertEquals(row.learningCount, learning.length, "no learning event was duplicated by the retried batches");
        assertEquals(row.navigationCount, navigation.length, "no navigation event was duplicated by the retried batches");
    ...
    // ---------------------------------------------------------------------------------------------
    await t.step("6. A delayed stale checkpoint arrives after newer progress on both devices; neither moves backward", async () => {
      ...
      try {
        ...
        await expect.poll(async () => {
          before = await (await fetch(`${origin}/api/v1/progress/checkpoint?revision=${lesson.revisionId}&epoch=0`, { headers: cookieHeader })).json();
        ...
        const stale = {
          id: crypto.randomUUID(), type: "navigation_checkpointed", lessonRevisionId: lesson.revisionId, epoch: 0,
          occurredAt: new Date(Date.now() + 3_600_000).toISOString(),
          checkpoint: { screen: "card", conceptIndex: 0, cardIndex: 0, flowKind: "cards", seed: 0, attemptId: "", queue: [], feedback: null, detour: null, marker: "late-stale", learningEventFrontier: [] },
        };
        const pushed = await fetch(`${origin}/api/v1/progress/navigation-events`, {
          method: "POST", headers: { ...cookieHeader, "content-type": "application/json" },
          body: JSON.stringify({ lessonRevisionId: lesson.revisionId, epoch: 0, events: [stale] }),
        });
        assertEquals(pushed.status, 200);
        assertEquals((await pushed.json()).accepted, 1);

        const after = await (await fetch(`${origin}/api/v1/progress/checkpoint?revision=${lesson.revisionId}&epoch=0`, { headers: cookieHeader })).json();
        assertEquals(after.checkpoint.screen, "summary", "a checkpoint with a smaller frontier never replaces one with a larger frontier, however late it arrives or however new its clock");
        assertEquals(after.checkpoint.marker, before.checkpoint.marker ?? undefined);

        const raw = await inspect(lesson.revisionId);
        assert(raw.navigationIds.includes(stale.id), "the stale event is stored");
        assertEquals(selectCheckpoint(raw.navigation, raw.learning).screen, "summary");

        for (const device of [a, b]) {
          await openLesson(device.page, origin, title);
          await expect(status(device.page)).toHaveText("Synced", { timeout: 25000 });
          await expect.poll(async () => (await readStore(device.page, "navigation_events")).some((event: any) => event.id === stale.id), { timeout: 20000 }).toBe(true);
          await click(device.page, "Resume");
          await expect(device.page.getByRole("heading", { name: "Learned", exact: true })).toBeVisible();
        }
        expect(a.errors).toEqual([]);
        expect(b.errors).toEqual([]);
      } finally {
    ...
    // ---------------------------------------------------------------------------------------------
    await t.step("7. Kill the tab mid-upload after the server has accepted the batch: reopening shows no duplicate and an empty outbox", async () => {
      ...
      try {
        ...
        // dropped connection.
        await page.route("**/api/v1/progress/learning-events", async (route: any) => {
          if (route.request().method() !== "POST") return route.continue();
          const posted = JSON.parse(route.request().postData() ?? "{}");
          learningIds.push(...(posted.events ?? []).map((event: any) => event.id));
          const replayed = await fetch(`${origin}/api/v1/progress/learning-events`, { method: "POST", headers: { ...cookieHeader, "content-type": "application/json" }, body: JSON.stringify(posted) });
        ...
        await expect.poll(() => learningStored, { timeout: 20000 }).toBe(true);
        ...
        const navigationIds = (await readStore(page, "navigation_events")).map((event: any) => event.id);
        ...
        const learningRows = await db.execute({ sql: "SELECT id FROM progress_events WHERE account_id = ? AND lesson_revision_id = ? AND id IN (" + learningIds.map(() => "?").join(",") + ")", args: [accountId, lesson.revisionId, ...learningIds] });
        assertEquals(learningRows.rows.length, learningIds.length, "each killed-mid-upload learning event has exactly one row, not zero and not more than one");
        const navigationRows = await db.execute({ sql: "SELECT id FROM navigation_events WHERE account_id = ? AND lesson_revision_id = ? AND id IN (" + navigationIds.map(() => "?").join(",") + ")", args: [accountId, lesson.revisionId, ...navigationIds] });
        assertEquals(navigationRows.rows.length, navigationIds.length, "each killed-mid-upload navigation event has exactly one row, not zero and not more than one");
      } finally {
        await context.close();
      }
    });
    ...
    await t.step("invariant sweep: the served checkpoint always has the largest accepted frontier in its scope", async () => {
      const streams = await db.execute({ sql: "SELECT DISTINCT lesson_revision_id, epoch FROM navigation_events WHERE account_id = ?", args: [accountId] });
      for (const row of streams.rows as any[]) {
        const revisionId = String(row.lesson_revision_id);
        const epoch = Number(row.epoch);
        const navigation = (await progressRepo.all(accountId, "navigation", { lessonRevisionId: revisionId, epoch })).map((r) => r.event);
        const learning = (await progressRepo.all(accountId, "learning", { lessonRevisionId: revisionId, epoch })).map((r) => r.event);
        const accepted = new Set(learning.map((event: any) => event.id));
        const served = selectCheckpoint(navigation, learning) as any;
        if (!served) continue;
        const servedFrontier = Array.isArray(served.learningEventFrontier) ? served.learningEventFrontier.filter((id: string) => accepted.has(id)).length : 0;
        for (const candidate of navigation as any[]) {
          if (candidate.type !== "navigation_checkpointed") continue;
          const frontier = Array.isArray(candidate.checkpoint?.learningEventFrontier) ? candidate.checkpoint.learningEventFrontier.filter((id: string) => accepted.has(id)).length : 0;
          expect(frontier, `checkpoint ${candidate.id} in revision ${revisionId} epoch ${epoch} depends on more accepted evidence than the served one`).toBeLessThanOrEqual(servedFrontier);
        }
      }
    });
