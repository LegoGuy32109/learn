...
import { INVITE_TTL_MS, PasskeyService } from "../../src/server/identity/passkeys.ts";
...
Deno.test("passkey sign-in in an ephemeral database", async (t) => {
  ...
  try {
    ...
    const app = createApp({ ...tursoDependencies(db, clock), passkeys: new PasskeyService(repository, clock) });
    ...
    const mint = (token: string) => call("/api/v1/sign-in-invites", { method: "POST", headers: { authorization: `Bearer ${token}` } });
    const owner = await admin.mint({ accountId, name: "owner", scopes: ["lessons:read", "account:owner"] });
    const reader = await admin.mint({ accountId, name: "reader", scopes: ["lessons:read", "lessons:write"] });
    ...
    await t.step("only an owner-scoped token mints an invite; 403 without the scope, 401 for an invalid token", async () => {
      ...
      const row = await db.execute("SELECT account_id, minted_by_token_prefix, consumed_at, token_hash FROM sign_in_invites");
      assertEquals(row.rows.length, 1);
      assertEquals(String(row.rows[0].account_id), accountId);
      assertEquals(String(row.rows[0].minted_by_token_prefix), owner.metadata.prefix);
      assertEquals(row.rows[0].consumed_at, null);
      assert(!String(row.rows[0].token_hash).includes(inviteToken), "the invite token is stored in the clear");
    });

    await t.step("a replayed registration challenge and a wrong origin are rejected", async () => {
      const options = await post("/api/v1/passkeys/registration-options", { invite: inviteToken });
      ...
      const rejected = await post("/api/v1/passkeys/registrations", { invite: inviteToken, credential: wrongOrigin });
      assertEquals(rejected.status, 400);
      await rejected.body?.cancel();
      const replayed = await post("/api/v1/passkeys/registrations", { invite: inviteToken, credential: await authenticator.register(creation, RP) });
      assertEquals(replayed.status, 401);
      assertStringIncludes((await replayed.json()).detail, "challenge");
      assertEquals((await db.execute("SELECT COUNT(*) AS n FROM passkey_credentials")).rows[0].n, 0);
    });

    await t.step("registration stores id, public key, sign count and creation time, consumes the invite and signs in", async () => {
      const options = await post("/api/v1/passkeys/registration-options", { invite: inviteToken });
      const { options: creation } = await options.json();
      const registered = await post("/api/v1/passkeys/registrations", { invite: inviteToken, credential: await authenticator.register(creation, RP) });
      ...
      const row = (await db.execute("SELECT * FROM passkey_credentials")).rows[0] as Record<string, unknown>;
      ...
      assertEquals(Number((await db.execute("SELECT consumed_at FROM sign_in_invites")).rows[0].consumed_at), now);

      const session = await call("/api/v1/session", { headers: { cookie } });
      assertEquals(await session.json(), { signedIn: true, displayName: "Josh Hale" });
    });

    await t.step("a second use and a late invite get a clear page", async () => {
    ...
    await t.step("sign-in with the passkey advances the sign count and a replayed assertion fails", async () => {
      ...
      const signedIn = await post("/api/v1/passkeys/authentications", { credential });
      assertEquals(signedIn.status, 200);
      assert((signedIn.headers.get("set-cookie") ?? "").includes("learn_session=v1."));
      await signedIn.body?.cancel();
      const row = (await db.execute("SELECT sign_count, last_used_at FROM passkey_credentials")).rows[0];
      assertEquals(Number(row.sign_count), authenticator.signCount);
      assertEquals(Number(row.last_used_at), now);
      const replayed = await post("/api/v1/passkeys/authentications", { credential });
      assertEquals(replayed.status, 401);
      await replayed.body?.cancel();
      assertEquals((await db.execute("SELECT COUNT(*) AS n FROM webauthn_challenges")).rows[0].n, 0);
    });

    await t.step("a tampered cookie is a guest; sign-out clears the cookie", async () => {
      const tampered = await call("/api/v1/session", { headers: { cookie: cookie.slice(0, -3) + "xyz" } });
      assertEquals(await tampered.json(), { signedIn: false, displayName: null });
      const intact = await call("/api/v1/session", { headers: { cookie } });
      assertEquals((await intact.json()).signedIn, true);
      const signedOut = await call("/api/v1/session", { method: "DELETE", headers: { cookie, origin: ORIGIN } });
