...
function platform(): { api: string; headers: HeadersInit } {
  ...
  if (!apiKey || !org) {
    throw new Error(
      "TURSO_API_KEY and TURSO_ORG_SLUG must be set to create an ephemeral database",
    );
  }
  return {
...
async function waitUntilReady(db: Client): Promise<void> {
  ...
  throw new Error(
    `ephemeral database did not become reachable: ${
...
export async function createEphemeralDatabase(): Promise<EphemeralDatabase> {
  ...
  const destroy = async () => {
    ...
    if (!response.ok && response.status !== 404) {
      throw new Error(`Turso API ${response.status}: could not delete ${name}`);
    }
    await response.body?.cancel();
  };
  try {
    if (!hostname) {
      throw new Error(`Turso did not return a hostname for ${name}`);
    }
    const minted = await json(
      await fetch(
        `${api}/databases/${
          encodeURIComponent(name)
        }/auth/tokens?expiration=2h&authorization=full-access`,
        { method: "POST", headers },
      ),
    );
    if (!minted.jwt) {
      throw new Error(`Turso did not return a token for ${name}`);
    }
