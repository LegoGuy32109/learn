...
Deno.test("progress sync in an ephemeral database", async (t) => {
  ...
  try {
    ...
    const other = await admin.mint({
      accountId: otherId,
      name: "other",
      scopes: ["lessons:read", "lessons:write"],
    });
    const cookieHeader = {
      cookie: (await dependencies.sessions.issue(new Request(ORIGIN), {
        accountId,
    ...
    const pullAll = async (
      ...
      for (let pages = 0; pages < 100; pages++) {
        const response = await call(
          `/api/v1/progress/${stream}?revision=${REVISION}&epoch=0&limit=${limit}&cursor=${cursor}`,
          { headers },
        );
    ...
    await t.step(
      ...
      async () => {
        ...
        assertEquals(versions.rows.map((row) => String(row.version)), [
          "001_initial.sql",
          "002_passkeys_and_invites.sql",
          "003_progress_sync.sql",
        ]);
        const tables = await db.execute(
          "SELECT name FROM sqlite_master WHERE type = 'table' AND name IN ('progress_streams', 'progress_events', 'navigation_events') ORDER BY name",
        );
        ...
        const columns = await db.execute(
          "SELECT name FROM pragma_table_info('progress_events') ORDER BY name",
        );
        const names = columns.rows.map((row) => String(row.name));
        assert(
          names.includes("occurred_at") && names.includes("received_at"),
          "both clocks are stored",
        );
        assert(
          !names.some((name) => /duration/.test(name)),
          "no response duration is stored",
        );
      },
    );
    ...
    await t.step(
      "repeated upload is stored once, by cookie or by bearer, and the rows carry both clocks",
      async () => {
        ...
        const rows = await db.execute({
          sql:
            "SELECT id, occurred_at, received_at FROM progress_events WHERE account_id = ? ORDER BY seq",
          args: [accountId],
        });
        ...
        assert(
          receivedAt >= started && receivedAt <= Date.now(),
          "received_at is the server's clock at ingest, not the client's occurred_at",
        );
    ...
    await t.step(
      "the short-answer text round-trips unchanged and paging never skips or repeats",
      async () => {
        const pulled = await pullAll("learning-events");
        assertEquals(pulled.length, evidence.length);
        assertEquals(
          new Set(pulled.map((made) => made.id)).size,
          evidence.length,
        );
        assertEquals(
          pulled.find((made) => made.id === shortAnswer.id),
          shortAnswer,
        );
        const byReader = await pullAll(
          "learning-events",
          bearer(reader.token),
          2,
        );
        assertEquals(
          new Set(byReader.map((made) => made.id)),
          new Set(pulled.map((made) => made.id)),
        );
      },
    );

    await t.step(
      "out-of-order arrival on another account reduces to the same progress",
      async () => {
        ...
        assertEquals(
          ...
          404,
          "a private draft is unknown to another account",
        );
        ...
        const b = reduceProgress(
          lesson,
          await pullAll("learning-events", bearer(other.token)),
        );
        assertEquals([a.state, [...a.cardsSeen].sort(), a.conceptStates], [
          b.state,
          [...b.cardsSeen].sort(),
          b.conceptStates,
        ]);
    ...
    await t.step(
      "a Question outside the revision and an unknown revision are rejected without storing anything",
      async () => {
        const count = async () =>
          Number(
            (await db.execute({
              sql:
                "SELECT COUNT(*) AS n FROM progress_events WHERE account_id = ?",
              args: [accountId],
            })).rows[0].n,
          );
        const before = await count();
        const foreign = event("question_answered", {
          flowKind: "check",
          conceptId: short.conceptId,
          poolId: short.poolId,
        ...
        const rejected = await push("learning-events", [
          event("lesson_started"),
          foreign,
        ]);
        ...
        const unknown = await call("/api/v1/progress/learning-events", {
          method: "POST",
    ...
    await t.step(
      "the checkpoint follows the frontier, a stale one never replaces a fuller one, and equal frontiers break ties deterministically",
      async () => {
    ...
    await t.step(
      "replaying the raw rows through the shared rule reproduces the served checkpoint with no projection anywhere",
      async () => {
        ...
        assertEquals(
          ...
          0,
          "the server stores no checkpoint projection",
        );
        const repository = new TursoProgressRepository(db);
        const scope = { lessonRevisionId: REVISION, epoch: 0 };
        const navigation =
          (await repository.all(accountId, "navigation", scope)).map((row) =>
            row.event
          );
        const learning = (await repository.all(accountId, "learning", scope))
          .map((row) => row.event);
        const served = await checkpoint();
        assertEquals(selectCheckpoint(navigation, learning), served.checkpoint);
        assertEquals(
          selectCheckpoint([...navigation].reverse(), learning),
          served.checkpoint,
        );
    ...
    await t.step(
      "an old epoch is rejected on push and pull once the stream has advanced",
      async () => {
        const advanced = await push(
          "learning-events",
          [event("lesson_started", {}, 1)],
          cookieHeader,
          1,
        );
        ...
        const stream = await db.execute({
          ...
          args: [accountId, stored.lessonId],
        });
        assertEquals([
          Number(stream.rows[0].epoch),
          String(stream.rows[0].lesson_revision_id),
        ], [1, REVISION]);
        ...
        const theirs = await call(
          `/api/v1/progress/learning-events?revision=${REVISION}&epoch=0`,
