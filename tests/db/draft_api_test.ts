...
Deno.test("authenticated draft API persists idempotently in Turso", async () => {
  ...
  if (!token) {
    throw new Error("LEARN_OWNER_TOKEN must be set; run deno task db:owner");
  }
  const db = createDb();
  ...
  try {
    ...
    const read = await app(
      new Request(
        `http://local/api/v1/lessons/${created.lessonId}/revisions/${created.revisionId}`,
    ...
    const shelf = await app(
      new Request("http://local/api/v1/shelf", {
        headers: { authorization: `Bearer ${token}` },
      }),
    );
    ...
    const revised = await app(
      new Request(`http://local/api/v1/lessons/${created.lessonId}/revisions`, {
        method: "POST",
        headers: {
          authorization: `Bearer ${token}`,
          "content-type": "application/json",
    ...
    const owner = await db.execute({
      sql: "SELECT account_id FROM api_tokens WHERE token_prefix = ?",
      args: [token.split("_")[2]],
    });
    const accountId = String(owner.rows[0].account_id);
    const setCookie = await dependencies.sessions.issue(
      new Request("http://local/"),
      { accountId, displayName: "Josh" },
    );
    const byCookie = await app(
      new Request("http://local/api/v1/shelf", {
    ...
    const latest = await app(
      new Request(`http://local/api/v1/lessons/${created.lessonId}`, {
        headers: { cookie: setCookie.split(";")[0] },
      }),
    );
    ...
    if (lessonId) {
      await db.execute({
        sql:
          "DELETE FROM lesson_sources WHERE lesson_revision_id IN (SELECT id FROM lesson_revisions WHERE lesson_id = ?)",
        args: [lessonId],
      });
      await db.execute({
        sql: "DELETE FROM lesson_revisions WHERE lesson_id = ?",
        args: [lessonId],
      });
      await db.execute({
