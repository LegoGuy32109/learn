...
Deno.test("token lifecycle in an ephemeral database", async (t) => {
  ...
  try {
    ...
    const listLessons = (token: string) =>
      app(
        new Request("http://local/api/v1/lessons", { headers: bearer(token) }),
      );
    const createLesson = (token: string) =>
      app(
    ...
    await t.step(
      "only a prefix and a SHA-256 hash are stored; the secret is not recoverable",
      async () => {
        ...
        assert(
          !Object.values(row).some((value) =>
            String(value).includes(reader.token)
          ),
          "a stored column contains the raw token",
        );
    ...
    await t.step(
      "a read-only token can list lessons and gets 403 on create",
      async () => {
    ...
    await t.step(
      "successful and forbidden authentication both update last_used_at",
      async () => {
        now += 1000;
        await status(await listLessons(reader.token));
        assertEquals(
          Number((await rows(db, reader.metadata.prefix)).last_used_at),
          now,
        );
        now += 1000;
    ...
    await t.step(
      "listing shows exactly name, prefix, scopes, created, last used, expiry and revoked",
      async () => {
        ...
        assert(
          !serialized.includes(
            String((await rows(db, reader.metadata.prefix)).token_hash),
          ),
        );
      },
    );
    ...
    await t.step(
      "an active name cannot be minted twice, and unknown scopes are rejected",
      async () => {
        await assertRejectsAdmin(() =>
          admin.mint({ accountId, name: "reader", scopes: ["lessons:read"] })
        );
        await assertRejectsAdmin(() =>
          admin.mint({
            accountId,
            name: "publisher",
            scopes: ["lessons:publish"],
          })
        );
        await assertRejectsAdmin(() =>
          admin.mint({
    ...
    await t.step(
      "an expired token gets 401 the moment its expiry passes",
      async () => {
        const shortLived = await admin.mint({
          accountId,
          name: "short-lived",
        ...
        await assertRejectsAdmin(() =>
          admin.rotate(accountId, shortLived.metadata.prefix)
        );
      },
    );
    ...
    await t.step("malformed, unknown and foreign tokens get 401", async () => {
      assertEquals(
        await status(await app(new Request("http://local/api/v1/lessons"))),
        401,
      );
      assertEquals(await status(await listLessons("not-a-token")), 401);
      const forged = reader.token.slice(0, -1) +
        (reader.token.endsWith("A") ? "B" : "A");
      assertEquals(await status(await listLessons(forged)), 401);
    });

    await t.step(
      "rotation confirms the new token before revoking the old one",
      async () => {
        now += 1000;
        ...
        const old = await admin.find(accountId, {
          prefix: reader.metadata.prefix,
        });
        ...
        await assertRejectsAdmin(() =>
          admin.rotate(accountId, reader.metadata.prefix)
        );
    ...
    await t.step("revocation is immediate and cannot be repeated", async () => {
      ...
      assertEquals(await status(await listLessons(reader.token)), 401);
      await assertRejectsAdmin(() =>
        admin.revoke(accountId, reader.metadata.prefix)
      );
      const active = (await admin.list(accountId)).filter((token) =>
    ...
    await t.step(
      "real-process smoke: mint with the script, use it, revoke it, and the next request fails",
      async () => {
...
async function assertRejectsAdmin(work: () => Promise<unknown>): Promise<void> {
  ...
  assert(
    failure instanceof TokenAdminError,
    `expected TokenAdminError, got ${redactBearerTokens(String(failure))}`,
  );
}
...
async function runScript(
  ...
  const command = new Deno.Command(Deno.execPath(), {
    args: [
      "run",
      "--allow-env=TURSO_DB_URL,TURSO_DB_TOKEN",
      "--allow-net",
      "--allow-read",
      "scripts/tokens.ts",
      ...args,
...
async function realProcessSmoke(url: string, authToken: string): Promise<void> {
  ...
  const env = {
    TURSO_DB_URL: url,
    TURSO_DB_TOKEN: authToken,
    PORT: String(port),
  };
  const server = new Deno.Command(Deno.execPath(), {
    args: [
      "run",
      "--allow-env=TURSO_DB_URL,TURSO_DB_TOKEN,PORT,LEARN_SESSION_KEY,WEBAUTHN_RP_ID,WEBAUTHN_ORIGINS",
      "--allow-net",
      "--allow-read",
      "main.ts",
    ],
  ...
  try {
    ...
    const minted = await runScript(env, [
      "mint",
      "--name",
      "smoke",
      "--scopes",
    ...
    const read = await fetch(`${base}/api/v1/lessons`, {
      headers: bearer(token),
    });
    assertEquals(await status(read), 200);
    const write = await fetch(`${base}/api/v1/lessons`, {
      method: "POST",
      headers: bearer(token),
      body: JSON.stringify(lesson),
    });
    ...
    const revoked = await runScript(env, [
      "revoke",
      "--prefix",
      metadata.prefix,
    ]);
    ...
    const afterRevoke = await fetch(`${base}/api/v1/lessons`, {
      headers: bearer(token),
    });
