...
import { MAX_DOCUMENT_BYTES, resolveLesson } from "../../src/shared/authoring/resolver.js";
...
Deno.test("the fingerprint ignores object key order and provenance but follows authored array order", async () => {
  ...
  reordered.concepts = reordered.concepts.map((concept: any) => Object.fromEntries(Object.entries(concept).reverse()));
...
Deno.test("every authoring fixture resolves to its manifest outcome", async () => {
  for (const entry of manifest) {
    ...
    assertEquals(result.valid, entry.valid, `${entry.file}: ${JSON.stringify(result.diagnostics)}`);
    if (entry.exact) assertEquals(codes, entry.codes, entry.file);
    else for (const code of entry.codes) assert(codes.includes(code), `${entry.file} lacks ${code}: ${codes.join(", ")}`);
    for (const diagnostic of result.diagnostics) {
      assert(diagnostic.path === "" || diagnostic.path.startsWith("/"), `${entry.file}: ${diagnostic.path} is not a JSON Pointer`);
...
Deno.test("each rule has a fixture that triggers only that rule", () => {
  const isolated = new Set(manifest.filter((entry) => entry.exact && entry.codes.length === 1).map((entry) => entry.codes[0]));
  const required = [
    "concept.options.count", "mcq.key", "mcq.map.missing", "mcq.map.unknown", "mcq.feedback.missing",
    "misconception.card", "misconception.unused", "pool.reserved.missing", "numeric.reserved", "card.body.paragraphs",
    "card.words", "concept.options.ratio", "lesson.key.longest", "question.stem.unbound",
    "numeric.answer.uncovered", "numeric.tolerance.missing", "pool.drawable.minimum", "card.paragraphs.single",
  ];
  for (const code of required) assert(isolated.has(code), `no isolated fixture for ${code}`);
});

Deno.test("a lesson with only warnings resolves valid and includes them", async () => {
  const result = await resolveLesson(await fixture("authoring/valid/warning-single-paragraph-card.json"));
  assert(result.valid);
  assertEquals(result.diagnostics.map((diagnostic) => [diagnostic.severity, diagnostic.code]), [["warning", "card.paragraphs.single"]]);
  assert(result.fingerprint);
});

Deno.test("diagnostics are ordered by path, then code, whatever the input order", async () => {
  const input = structuredClone(lesson) as any;
  input.questions[9].stem = "Compared with the previous one, what does no-store mean?";
  input.questions[1].tolerance = 0;
  input.questions[1].answer = 181;
  input.concepts[0].misconceptions[0].correctingCardId = input.concepts[1].cards[0].id;
  delete input.title;
  const result = await resolveLesson(input);
  const pairs = result.diagnostics.map((diagnostic) => `${diagnostic.path} ${diagnostic.code}`);
  assertEquals(pairs, [
...
Deno.test("numeric segments sort numerically so /questions/10 follows /questions/9", async () => {
  const input = structuredClone(lesson) as any;
  input.questions[10].stem = "As mentioned before, which directive means store nothing?";
  input.questions[9].stem = "Other than the one already covered, which directive means store nothing?";
  const result = await resolveLesson(input);
  assertEquals(result.diagnostics.map((diagnostic) => diagnostic.path), ["/questions/9/stem", "/questions/10/stem"]);
});

Deno.test("card word counts strip inline HTML before counting", async () => {
  const input = structuredClone(lesson) as any;
  const card = input.concepts[0].cards[0];
  const words = card.body.join(" ").replace(/<[^>]+>/g, "").trim().split(/\s+/).length;
  card.body = [card.body.join(" ") + " <code>" + Array.from({ length: 200 - words + 1 }, (_, index) => `tag${index}`).join(" ") + "</code>"];
  const result = await resolveLesson(input);
  assertEquals(result.diagnostics.map((diagnostic) => diagnostic.code), ["card.paragraphs.single", "card.words"]);
  const tagged = structuredClone(lesson) as any;
  tagged.concepts[0].cards[0].body = tagged.concepts[0].cards[0].body.map((paragraph: string) => `<em class="x">${paragraph}</em>`);
...
Deno.test("the normalized lesson keeps only contract fields and trims text", async () => {
  ...
  assertEquals(Object.keys(normalized.questions[0].feedback), [...Object.keys(normalized.questions[0].feedback)].sort());
});
...
Deno.test("adversarial documents are rejected without a crash or a hang", async () => {
  ...
  const nested = await resolveLesson({ ...structuredClone(lesson), extra: deep });
  assertEquals(nested.diagnostics.map((diagnostic) => diagnostic.code), ["document.nesting"]);
  ...
  assertEquals(oversized.diagnostics.map((diagnostic) => diagnostic.code), ["document.size"]);

  const scalars = await Promise.all([null, 1, "lesson", [], true].map((value) => resolveLesson(value)));
  for (const result of scalars) assertEquals(result.diagnostics.map((diagnostic) => diagnostic.code), ["document.object"]);

  const polluted = JSON.parse('{"__proto__":{"valid":true},"constructor":{"prototype":{"x":1}},"schema":"lesson/v1"}');
