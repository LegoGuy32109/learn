...
function event(type: string, data: Record<string, unknown> = {}, epoch = 0): Event {
  clock += 1000;
  return { id: crypto.randomUUID(), type, lessonRevisionId: REVISION, epoch, occurredAt: at(), ...data };
}
...
function answered(question: any, answer: unknown, correct: boolean, flowKind = "check", epoch = 0): Event {
  return event("question_answered", { flowKind, conceptId: question.conceptId, poolId: question.poolId, questionId: question.id, attemptId: "attempt-1", answer, correct }, epoch);
}

function checkpointed(frontier: string[], marker: string, occurredAt?: string, id?: string): Event {
  const made = event("navigation_checkpointed", {
    checkpoint: { screen: "card", conceptIndex: 0, cardIndex: 0, flowKind: "cards", seed: 0, attemptId: "", queue: [], feedback: null, detour: null, marker, learningEventFrontier: frontier },
  });
...
async function harness() {
  const dependencies = {
    ...
    auth: stubAuthenticator({
      [OWNER]: { accountId: FIXTURE_ACCOUNT.id, scopes: ["lessons:read", "lessons:write"] },
      [READER_ONLY]: { accountId: FIXTURE_ACCOUNT.id, scopes: ["lessons:read"] },
      [WRITER_ONLY]: { accountId: FIXTURE_ACCOUNT.id, scopes: ["lessons:write"] },
      [OTHER]: { accountId: "someone-else", scopes: ["lessons:read", "lessons:write"] },
    }),
  };
  ...
  const push = (stream: string, events: unknown, headers: Record<string, string> = cookie, epoch = 0, revision: unknown = REVISION) =>
  ...
  const pull = async (stream: string, query = "", headers: Record<string, string> = cookie) => {
    const response = await call(`/api/v1/progress/${stream}?revision=${REVISION}&epoch=0${query}`, { headers });
    return { status: response.status, body: await response.json() };
  };
  const pullAll = async (stream: string, headers: Record<string, string> = cookie, limit = 100) => {
    const events: Event[] = [];
    let cursor = "";
    for (let pages = 0; pages < 50; pages++) {
      const page = await pull(stream, `&limit=${limit}&cursor=${cursor}`, headers);
  ...
  const checkpoint = async (headers: Record<string, string> = cookie) => (await call(`/api/v1/progress/checkpoint?revision=${REVISION}&epoch=0`, { headers })).json();
...
Deno.test("progress routes need an account: 401 for nobody, 403 without the scope, cookie and bearer both work", async () => {
  ...
  assertEquals((await h.push("learning-events", [], h.bearer(READER_ONLY))).status, 403);
  assertEquals((await h.push("learning-events", [], h.bearer(WRITER_ONLY))).status, 200);
  assertEquals((await h.pull("learning-events", "", {})).status, 401);
  assertEquals((await h.pull("learning-events", "", h.bearer(WRITER_ONLY))).status, 403);
  assertEquals((await h.pull("learning-events", "", h.bearer(READER_ONLY))).status, 200);
  assertEquals((await h.pull("learning-events")).status, 200);
  const foreign = await h.push("learning-events", [], { ...h.cookie, origin: "https://evil.example" });
...
Deno.test("repeated upload of the same events is accepted once and returns success each time", async () => {
  ...
  const events = [event("lesson_started"), event("card_seen", { cardId: card.id, conceptId: lesson.concepts[0].id })];
  const first = await h.push("learning-events", events);
  assertEquals(first.status, 200);
  assertEquals(await first.json(), { accepted: 2, duplicates: 0, stream: { lessonId: fixture.lessonId, lessonRevisionId: REVISION, epoch: 0 } });
  const again = await h.push("learning-events", events);
  assertEquals(again.status, 200);
  assertEquals((await again.json()).duplicates, 2);
  const partial = await h.push("learning-events", [events[1], event("card_seen", { cardId: lesson.concepts[0].cards[1].id, conceptId: lesson.concepts[0].id })]);
  assertEquals(await partial.json().then((body) => [body.accepted, body.duplicates]), [1, 1]);
...
Deno.test("events arriving out of order produce the same progress and checkpoint as in order", async () => {
  ...
  const cards = concept.cards.map((card: any) => event("card_seen", { cardId: card.id, conceptId: concept.id }));
  const wrapUp = answered(lesson.questions.find((question: any) => question.conceptId === concept.id && question.reserved), correctAnswer(lesson.questions.find((question: any) => question.conceptId === concept.id && question.reserved)), true, "wrap_up");
  const learningEvents = [event("lesson_started"), ...cards, wrapUp];
  const early = checkpointed(learningEvents.slice(0, 2).map((made) => made.id), "early");
  ...
  assertEquals((await h.push("navigation-events", [late], h.bearer(OTHER))).status, 200);
  assertEquals((await h.push("learning-events", [...learningEvents].reverse().slice(0, 3), h.bearer(OTHER))).status, 200);
  assertEquals((await h.push("learning-events", [...learningEvents].reverse().slice(3), h.bearer(OTHER))).status, 200);
  assertEquals((await h.push("navigation-events", [early], h.bearer(OTHER))).status, 200);
  const ordered = await h.pullAll("learning-events");
  const reversed = await h.pullAll("learning-events", h.bearer(OTHER));
  assertEquals(new Set(ordered.map((made) => made.id)), new Set(reversed.map((made) => made.id)));
  const a = reduceProgress(lesson, ordered);
  const b = reduceProgress(lesson, reversed);
  assertEquals([a.state, [...a.cardsSeen].sort(), [...a.learnedConcepts]], [b.state, [...b.cardsSeen].sort(), [...b.learnedConcepts]]);
  assertEquals(a.conceptStates[0], { id: concept.id, seen: true, learned: true });
...
Deno.test("events from an epoch older than the stream's current epoch are rejected with a structured error, on push and on pull", async () => {
  const h = await harness();
  assertEquals((await h.push("learning-events", [event("lesson_started")])).status, 200);
  const advanced = await h.push("learning-events", [event("lesson_started", {}, 2)], h.cookie, 2);
  assertEquals((await advanced.json()).stream.epoch, 2);
  const stale = await h.push("learning-events", [event("lesson_started")]);
  assertEquals(stale.status, 409);
  assertEquals(stale.headers.get("content-type"), "application/problem+json; charset=utf-8");
  const body = await stale.json();
  assertEquals(body.code, "epoch.stale");
  assertEquals(body.stream, { lessonId: fixture.lessonId, lessonRevisionId: REVISION, epoch: 2 });
  ...
  const staleNavigation = await h.push("navigation-events", [checkpointed([], "old")]);
  ...
  const current = await (await h.call(`/api/v1/progress/learning-events?revision=${REVISION}&epoch=2`, { headers: h.cookie })).json();
...
Deno.test("short-answer text round-trips unchanged, and the shared evaluator decides correctness", async () => {
  ...
  const numeric = lesson.questions.find((question: any) => question.type === "numeric");
  const idk = answered(numeric, null, false);
  const right = answered(short, ` ${short.answer.toUpperCase()} `, true);
  assertEquals((await h.push("learning-events", [sent, idk, right])).status, 200);
  const stored = await h.pullAll("learning-events");
  ...
  const lying = await h.push("learning-events", [answered(short, "definitely wrong", true)]);
...
Deno.test("an unknown Lesson Revision, a Question outside its revision and a malformed event are rejected and nothing is stored", async () => {
  const h = await harness();
  const unknown = await h.push("learning-events", [event("lesson_started")], h.cookie, 0, "6f1c1c2a-3b1e-4b6f-9a1c-2f6d8e4b7a10");
  assertEquals(unknown.status, 404);
  assertEquals((await unknown.json()).code, "revision.unknown");
  const unknownPull = await h.call("/api/v1/progress/learning-events?revision=nope&epoch=0", { headers: h.cookie });
  assertEquals(unknownPull.status, 404);
  await unknownPull.body?.cancel();
  const question = lesson.questions[0];
  const foreignQuestion = answered({ ...question, id: crypto.randomUUID() }, correctAnswer(question), true);
  const wrongConcept = answered({ ...question, conceptId: lesson.concepts[1].id }, correctAnswer(question), true);
  const fine = event("lesson_started");
  const rejected = await h.push("learning-events", [fine, foreignQuestion, wrongConcept]);
  assertEquals(rejected.status, 422);
  const body = await rejected.json();
  assertEquals(body.code, "events.rejected");
  assertEquals(body.rejections.map((entry: any) => [entry.index, entry.code, entry.path]), [[1, "question.unknown", "/events/1/questionId"], [2, "question.concept", "/events/2/conceptId"]]);
  assertEquals((await h.pullAll("learning-events")).length, 0, "a batch with any rejection stores nothing");
  const malformed = await h.push("learning-events", [{ id: "not-a-uuid", type: "card_seen", lessonRevisionId: REVISION, epoch: 0, occurredAt: "yesterday" }, { ...event("lesson_started"), epoch: 4 }]);
  const codes = (await malformed.json()).rejections.map((entry: any) => entry.code).sort();
  assertEquals(codes, ["event.epoch", "event.id", "event.occurredAt"]);
  const foreignCheckpoint = await h.push("navigation-events", [{ ...checkpointed([], "bad"), checkpoint: { conceptIndex: 9, queue: ["nope"], learningEventFrontier: ["x"] } }]);
  assertEquals((await foreignCheckpoint.json()).rejections.map((entry: any) => entry.code), ["checkpoint.frontier"]);
  const tooMany = await h.push("learning-events", Array.from({ length: MAX_BATCH_EVENTS + 1 }, () => event("lesson_started")));
  assertEquals((await tooMany.json()).rejections[0].code, "events.count");
  const notJson = await h.call("/api/v1/progress/learning-events", { method: "POST", headers: { ...h.cookie, "content-type": "application/json" }, body: "{" });
...
Deno.test("incremental pull pages with an opaque cursor and never skips or repeats an event", async () => {
  ...
  const events = [event("lesson_started"), ...concept.cards.map((card: any) => event("card_seen", { cardId: card.id, conceptId: concept.id }))];
  const drawable = lesson.questions.filter((question: any) => question.conceptId === concept.id && !question.reserved);
  for (const question of drawable) events.push(answered(question, correctAnswer(question), true));
  assertEquals((await h.push("learning-events", events.slice(0, 4))).status, 200);
  ...
  while (true) {
    ...
    assert(typeof page.body.cursor === "string" && !/^\d+$/.test(page.body.cursor), "the cursor is opaque");
  ...
  assertEquals(tail.body.events.map((made: Event) => made.id), seen.slice(-1).length ? tail.body.events.map((made: Event) => made.id) : []);
  const latest = (await h.pull("learning-events", `&limit=500&cursor=${cursor}`)).body;
  const added = event("card_seen", { cardId: lesson.concepts[2].cards[0].id, conceptId: lesson.concepts[2].id });
...
Deno.test("the checkpoint is rebuilt from the streams, a smaller frontier never replaces a larger one, and the tie breaker holds", async () => {
  ...
  const stale = checkpointed(ids.slice(0, 1), "stale", "2026-09-22T12:00:00.000Z");
  ...
  const bluff = checkpointed([...ids, crypto.randomUUID(), crypto.randomUUID()], "bluff", "2026-09-22T13:00:00.000Z");
  ...
  assertEquals((await h.checkpoint()).checkpoint.marker, "bluff", "equal accepted frontier: the later client clock wins");
  ...
  const low = checkpointed(ids, "low", "2026-09-22T13:00:00.000Z", "00000000-0000-4000-8000-00000000000a");
  const high = checkpointed(ids, "high", "2026-09-22T13:00:00.000Z", "ffffffff-ffff-4fff-bfff-ffffffffffff");
  ...
  assertEquals(selectCheckpoint(navigation, learning), (await h.checkpoint()).checkpoint);
  assertEquals(selectCheckpoint([...navigation].reverse(), [...learning].reverse()), (await h.checkpoint()).checkpoint);
  // More evidence moves the checkpoint forward again.
  const more = event("card_seen", { cardId: lesson.concepts[1].cards[0].id, conceptId: lesson.concepts[1].id });
  await (await h.push("learning-events", [more])).body?.cancel();
  const further = checkpointed([...ids, more.id], "further", "2026-09-22T09:00:00.000Z");
  await (await h.push("navigation-events", [further])).body?.cancel();
  const final = await h.checkpoint();
  assertEquals(final.checkpoint.marker, "further", "a larger frontier wins even with an earlier client clock");
...
Deno.test("progress on a revision the account cannot learn is unknown to it", async () => {
  const h = await harness();
  const draft = await h.call("/api/v1/lessons", { method: "POST", headers: { ...h.bearer(OTHER), "content-type": "application/json" }, body: JSON.stringify({ ...structuredClone(fixture), lessonId: undefined, revisionId: undefined, title: "Someone else's draft" }) });
  assertEquals(draft.status, 201);
  const created = await draft.json();
  const mine = await h.push("learning-events", [{ ...event("lesson_started"), lessonRevisionId: created.revisionId }], h.cookie, 0, created.revisionId);
  assertEquals(mine.status, 404);
  await mine.body?.cancel();
  const theirs = await h.push("learning-events", [{ ...event("lesson_started"), lessonRevisionId: created.revisionId }], h.bearer(OTHER), 0, created.revisionId);
