...
import manifest from "../../fixtures/authoring/manifest.json" with { type: "json" };
import openapiMetaSchema from "./support/openapi-3.1-schema.json" with { type: "json" };
import { lessonSchema, MAX_DOCUMENT_BYTES, resolveLesson } from "../../src/shared/authoring/resolver.js";
import { DIAGNOSTIC_CODES, DIAGNOSTICS, diagnosticsReference } from "../../src/shared/authoring/diagnostics.js";
import { app, composeRoutes } from "../../src/app.ts";
import { capabilitiesFor, discoveryLinks } from "../../src/server/api-docs/capabilities.ts";
...
const demo = await fixture("lessons/browser-http-cache.json") as Record<string, unknown>;
...
async function mutatedDocuments(): Promise<unknown[]> {
  ...
  const huge = mutate((input) => input.assumedKnowledge = "x".repeat(MAX_DOCUMENT_BYTES + 1));
  return [
    null,
    huge,
    mutate((input) => { input.schema = "lesson/v2"; delete input.schemaVersion; delete input.title; delete input.assumedKnowledge; input.concepts = []; input.sources = []; input.provenance = { status: "maybe" }; }),
    mutate((input) => { input.provenance = { status: "provided" }; }),
    mutate((input) => { input.sources = [{ capturedText: 5 }]; }),
    mutate((input) => {
    ...
    mutate((input) => {
      const numeric = input.questions.find((question: any) => question.type === "numeric");
    ...
    mutate((input) => {
      const short = input.questions.find((question: any) => question.type === "short");
      delete short.answer;
      short.aliases = [1];
...
Deno.test("the JSON Schema accepts every valid fixture and rejects each fixture the manifest marks schemaRejects", async () => {
  ...
  for (const entry of manifest) {
    ...
    if (entry.valid) assert(accepted, `${entry.file}: schema rejected a valid fixture: ${JSON.stringify(validate.errors)}`);
    else {
      assertEquals(accepted, !entry.schemaRejects, `${entry.file}: schema ${accepted ? "accepted" : "rejected"} but the manifest says schemaRejects ${entry.schemaRejects}`);
      assert(!resolved.valid, `${entry.file}: the resolver must reject every invalid fixture`);
    }
  }
  const invalid = manifest.filter((entry) => !entry.valid);
  assert(invalid.some((entry) => entry.schemaRejects) && invalid.some((entry) => !entry.schemaRejects), "the manifest must show both schema-caught and resolver-only fixtures");
});

Deno.test("codes the catalog marks schema: true are rejected by the JSON Schema in their isolated fixture", async () => {
  const validate = validator().compile(lessonSchema);
  const isolated = manifest.filter((entry) => entry.exact && entry.codes.length === 1);
  for (const entry of isolated) {
    ...
    assert(!validate(await fixture(entry.file)), `${entry.file}: catalog says the schema catches ${catalog.code} but it accepted the fixture`);
  }
});

Deno.test("the JSON Schema is served, is the draft 2020-12 dialect and matches the resolver's constants", async () => {
  const response = await app(new Request("http://local/api/v1/schemas/lesson/v1"));
  ...
  for (const name of ["concept", "option", "misconception", "card", "mcq", "numeric", "short", "source", "provenance"]) assert(served.$defs[name], `$defs.${name} is documented`);
});
...
function bindDynamicAnchors(value: unknown): unknown {
  ...
  return Object.fromEntries(Object.entries(rest).map(([key, item]) => [key, bindDynamicAnchors(item)]));
}

Deno.test("the OpenAPI document validates against the OpenAPI 3.1 meta-schema", () => {
  const ajv = validator(false);
  const validate = ajv.compile(bindDynamicAnchors(openapiMetaSchema) as Record<string, unknown>);
  const document = JSON.parse(JSON.stringify(openapiDocument("https://learn.joshhale.me"))) as any;
...
Deno.test("the OpenAPI examples validate against the component schemas they claim", async () => {
  ...
  const embed = (value: unknown) =>
    JSON.parse(JSON.stringify(value, (key, item) => (key === "$id" ? undefined : typeof item === "string" ? item.replace("#/components/schemas/", "#/$defs/") : item)));
  const withRefs = (schema: Record<string, unknown>) => embed({ ...schema, $defs: { ...(schema.$defs as object ?? {}), ...components } });
  const check = (name: string, example: unknown) => {
    const compiled = validator().compile(withRefs(components[name]));
    assert(compiled(example), `${name} example: ${JSON.stringify(compiled.errors)}`);
  };
  ...
  const exampleOf = (mediaObject: any) => {
    ...
    const shared = document.components.examples[ref.slice("#/components/examples/".length)];
  ...
  for (const item of Object.values(document.paths) as any[]) {
    for (const operation of Object.values(item) as any[]) {
      ...
      const bodies = [operation.requestBody, ...Object.values(operation.responses ?? {})].filter(Boolean) as any[];
      for (const body of bodies) {
        for (const mediaObject of Object.values(body.content ?? {}) as any[]) {
          ...
          check(ref.slice("#/components/schemas/".length), exampleOf(mediaObject));
          checked++;
        }
      }
    }
  ...
  const live = await (await app(new Request("http://local/api/v1/lesson-resolutions", { method: "POST", body: JSON.stringify(exampleOf(resolutions.requestBody.content["application/json"])) }))).json();
  assertEquals(live, exampleOf(resolutions.responses["200"].content["application/json"]));
  check("DiagnosticsReference", await (await app(new Request("http://local/api/v1/diagnostics"))).json());
  check("Capabilities", await (await app(new Request("http://local/api/v1/capabilities"))).json());
});

const browserOnly = new Set(["/", "/learn/*", "/css/*", "/js/*", "/src/client/*", "/src/shared/*"]);

Deno.test("the OpenAPI document covers every route the server serves and nothing it does not", () => {
  const routes = composeRoutes({ lessons: new FixtureLessonRepository(demo), auth: new RejectingAuthenticator() });
  ...
  for (const [path, item] of Object.entries(paths)) {
    for (const method of ["get", "post", "put", "patch", "delete"]) {
      ...
      const concrete = path.replaceAll(/\{[^}]+\}/g, "6f1c1c2a-3b1e-4b6f-9a1c-2f6d8e4b7a10");
      const match = routes.find((route) => route.method === method.toUpperCase() && route.pattern.test(`https://learn.joshhale.me${concrete}`));
      assert(match, `${method.toUpperCase()} ${path} is documented but no route serves it`);
      documented.add(`${match.method} ${match.pattern.pathname}`);
      if (!match.pattern.pathname.endsWith("*")) assertEquals(match.pattern.pathname.replaceAll(/:([A-Za-z]+)/g, "{$1}"), path, "path parameters share the route's names");
    }
  }
  for (const route of routes) {
    if (browserOnly.has(route.pattern.pathname)) continue;
    assert(documented.has(`${route.method} ${route.pattern.pathname}`), `${route.method} ${route.pattern.pathname} is served but not documented in OpenAPI`);
  }
  const withParameters = paths["/api/v1/lessons/{lessonId}/revisions/{revisionId}"].parameters as Array<{ name: string; in: string; required: boolean }>;
  assertEquals(withParameters.map((parameter) => [parameter.name, parameter.in, parameter.required]), [["lessonId", "path", true], ["revisionId", "path", true]]);
});
...
Deno.test("the served OpenAPI document uses the request origin as its server", async () => {
  ...
  assertEquals(document.components.examples.capabilities.value.links.schema, "http://local:8123/api/v1/schemas/lesson/v1");
});
...
async function codesInResolverSource(): Promise<Set<string>> {
  ...
  for (const match of source.matchAll(/const ([A-Z_]+) = \[([^\]]+)\];/g)) lists[match[1]] = [...match[2].matchAll(/"([^"]+)"/g)].map((item) => item[1]);
  const codes = new Set<string>();
  const calls = [...source.matchAll(/report\.(?:error|warning)\((?:"([^"]+)"|`([^`]+)`)/g)];
  assert(calls.length > 50, "the code extractor found too few report calls; update the regex");
...
Deno.test("the diagnostics reference lists every code the resolver source can emit, and no other", async () => {
  ...
  for (const code of inSource) assert(listed.has(code), `resolver emits ${code} but the diagnostics reference does not list it`);
  for (const code of listed) assert(inSource.has(code), `the diagnostics reference lists ${code} but the resolver never emits it`);
  assertEquals(DIAGNOSTIC_CODES.length, new Set(DIAGNOSTIC_CODES).size, "codes are unique");
  for (const entry of DIAGNOSTICS) {
    assert(entry.meaning.length > 10 && entry.fix.length > 10, `${entry.code} needs a meaning and a fix`);
    assert(entry.path === "" || entry.path.startsWith("/"), `${entry.code} path is a JSON Pointer shape`);
  }
});
...
Deno.test("every diagnostic emitted at runtime is listed with its severity, and every listed code is seen", async () => {
  ...
  const bySeverity = new Map(DIAGNOSTICS.map((entry) => [entry.code, entry.severity]));
  for (const document of documents) {
    const result = await resolveLesson(document);
    for (const diagnostic of result.diagnostics) {
      assert(bySeverity.has(diagnostic.code), `resolver emitted unlisted code ${diagnostic.code} at ${diagnostic.path}`);
      assertEquals(diagnostic.severity, bySeverity.get(diagnostic.code), `${diagnostic.code} severity differs from the reference`);
      seen.set(diagnostic.code, diagnostic.path);
    }
  }
  for (const entry of DIAGNOSTICS) assert(seen.has(entry.code), `no test document triggers ${entry.code}; add one to mutatedDocuments`);
  for (const entry of DIAGNOSTICS) {
    const shape = entry.path.replaceAll(/<optionId>/g, "[A-Za-z0-9_-]+").replaceAll(/<[ij]>/g, "\\d+");
    assert(new RegExp(`^${shape}$`).test(seen.get(entry.code)!), `${entry.code}: emitted path ${seen.get(entry.code)} does not match documented ${entry.path}`);
  }
});
...
Deno.test("the diagnostics reference is served as JSON and rendered as the committed markdown", async () => {
  ...
  assertEquals(response.status, 200);
  assertEquals(await response.json(), JSON.parse(JSON.stringify(diagnosticsReference)));
  const markdown = await Deno.readTextFile(new URL("docs/diagnostics.md", repo));
  for (const code of DIAGNOSTIC_CODES) assert(markdown.includes(`| \`${code}\` |`), `docs/diagnostics.md lacks ${code}`);
  const served = await app(new Request("http://local/docs/diagnostics.md"));
  assertEquals(served.status, 200);
  assertEquals(served.headers.get("content-type"), "text/markdown; charset=utf-8");
...
Deno.test("the capability document links every discovery artifact by absolute URL and each link resolves", async () => {
  for (const path of ["/api/v1/capabilities", "/.well-known/learn-joshhale.json"]) {
    const response = await app(new Request(`https://learn.example${path}`));
    assertEquals(response.status, 200);
    const body = await response.json();
    assertEquals(body, JSON.parse(JSON.stringify(capabilitiesFor("https://learn.example"))));
    assertEquals(Object.keys(body.links).sort(), Object.keys(discoveryLinks).sort());
    for (const [key, href] of Object.entries(body.links as Record<string, string>)) {
      assert(href.startsWith("https://learn.example/"), `${key} link ${href} is not absolute on the request origin`);
      const method = key === "resolver" || key === "lessons" ? "POST" : "GET";
      const linked = await app(new Request(href, { method, body: method === "POST" ? JSON.stringify(demo) : undefined }));
      assert([200, 401].includes(linked.status), `${key} -> ${href} answered ${linked.status}`);
      await linked.body?.cancel();
    }
    for (const required of ["schema", "openapi", "diagnostics", "docs", "diagnosticsDocs", "validator", "validatorTypes", "resolver", "lessons"]) assert(body.links[required], `links.${required}`);
...
Deno.test("the generated files match the committed ones (run deno task tools:generate)", async () => {
  for (const [path, content] of Object.entries(await generatedFiles())) {
    assertEquals(await Deno.readTextFile(new URL(path, repo)), content, `${path} is stale`);
  }
  const declarations = await Deno.readTextFile(new URL("public/tools/lesson-validator.d.ts", repo));
  for (const code of DIAGNOSTIC_CODES) assert(declarations.includes(`"${code}"`), `DiagnosticCode lacks ${code}`);
});
