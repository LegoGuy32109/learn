...
import { lessonSchema, resolveLesson, CARD_WORDS_MAX, CARD_WORDS_MIN, DRAWABLE_MIN, OPTION_COUNT, OPTION_RATIO_MAX } from "../../src/shared/authoring/resolver.js";
...
Deno.test("the committed plugin directory is exactly what the sources generate (run deno task plugin:generate)", async () => {
  ...
  assertEquals(Object.keys(committed).sort(), Object.keys(generated.files).sort(), "the set of files under public/plugin differs");
...
function readZip(bytes: Uint8Array): Map<string, Uint8Array> {
  ...
  assertEquals(view.getUint32(endOffset, true), 0x06054b50, "end of central directory");
...
Deno.test("the archive holds the plugin directory: manifest at the root, the skill, and nothing generated twice", () => {
  ...
  for (const [path, content] of entries) assertEquals(decoder.decode(content), tree[path], path);
  assert(entries.has(".claude-plugin/plugin.json"));
  assert(!entries.has("marketplace.json") && !entries.has(ARCHIVE_FILE), "the archive does not contain the marketplace or itself");
});
...
function serve(handler: (request: Request) => Promise<Response>) {
  ...
  return { origin: `http://127.0.0.1:${server.addr.port}`, close: () => server.shutdown() };
}

async function run(command: string, args: string[], options: { cwd?: string; env?: Record<string, string> } = {}) {
  const process = new Deno.Command(command, { args, cwd: options.cwd, env: options.env, clearEnv: options.env !== undefined, stdout: "piped", stderr: "piped" });
...
Deno.test("git clone works against the served repository and yields the plugin directory at the generated commit", async () => {
  ...
  try {
    const clone = await run("git", ["clone", "--quiet", `${server.origin}/plugin/${REPOSITORY_DIR}`, "plugin"], { cwd: directory });
    assertEquals(clone.code, 0, clone.stderr);
    const head = await run("git", ["rev-parse", "HEAD"], { cwd: `${directory}/plugin` });
    assertEquals(head.stdout.trim(), generated.commit);
    for (const [path, content] of Object.entries(tree)) assertEquals(await Deno.readTextFile(`${directory}/plugin/${path}`), content, path);
    const fsck = await run("git", ["fsck", "--strict"], { cwd: `${directory}/plugin` });
...
Deno.test("the plugin routes serve the page, the manifests, the skill, the archive and the repository with the right media types", async () => {
  ...
  assertStringIncludes(markup, `claude plugin marketplace add https://learn.example/plugin/marketplace.json`);
  assertStringIncludes(markup, `claude plugin install ${PLUGIN_NAME}@${MARKETPLACE_NAME}`);
  assertStringIncludes(markup, `git clone https://learn.example/plugin/${REPOSITORY_DIR}`);
  ...
  const expectations: Array<[string, string]> = [
    ...
    [`/plugin/skills/${SKILL_NAME}/scripts/validate.mjs`, "text/javascript; charset=utf-8"],
  ...
  for (const [path, type] of expectations) {
    ...
    assertEquals(new Uint8Array(await response.arrayBuffer()), generated.files[`${PLUGIN_ROOT}${path.slice("/plugin/".length)}`], path);
  }
  const escape = await app(new Request("https://learn.example/plugin/../tools/lesson-validator.js"));
...
Deno.test("the capability document points at the plugin with absolute links that resolve", async () => {
  const body = await (await app(new Request("https://learn.example/api/v1/capabilities"))).json();
  ...
  for (const key of ["page", "marketplace", "manifest", "archive", "skillDocument"]) {
  ...
  assertEquals(body.plugin.repository, `https://learn.example/plugin/${REPOSITORY_DIR}`);
  const refs = await app(new Request(`${body.plugin.repository}/info/refs`));
  assertEquals(refs.status, 200);
  assertStringIncludes(await refs.text(), `${generated.commit}\trefs/heads/main`);
  assertEquals(body.plugin.install.claudeCode, [`claude plugin marketplace add ${body.plugin.marketplace}`, `claude plugin install ${PLUGIN_NAME}@${MARKETPLACE_NAME}`]);
  for (const href of [body.plugin.marketplace, body.plugin.archive, body.plugin.repository, body.plugin.skillDocument, body.links.plugin]) assert(body.authentication.publicRoutes.includes(href), `${href} is public`);
});
...
Deno.test("the skill's texts are the plugin's texts adapted for lesson/v1 and carry the resolver's own numbers", async () => {
  ...
  for (const required of ["shared option set", "Distractor provenance", "correctingCardId", "## Short-answer Questions", "## Cards", "paragraph", "never the reserved instance", `${OPTION_RATIO_MAX} times`, `${CARD_WORDS_MIN} to ${CARD_WORDS_MAX} words`, "| `card.words` |", "| `pool.drawable.minimum` |"]) assertStringIncludes(authoring, required);
  assert(!/corrected_by|"X"|item's key/.test(authoring), "the quiz plugin's field names are adapted");

  const schemaReference = text(`${skill}references/lesson-schema.md`);
  for (const definition of ["### concept", "### option", "### misconception", "### card", "### mcq", "### numeric", "### short", "### source", "### provenance"]) assertStringIncludes(schemaReference, definition);
  assertStringIncludes(schemaReference, `array of [option](#option) (min ${OPTION_COUNT}, max ${OPTION_COUNT})`);

  const diagnostics = text(`${skill}references/diagnostics.md`);
  assertEquals(diagnostics, await Deno.readTextFile(new URL("docs/diagnostics.md", repo)));
  const catalog = JSON.parse(text(`${skill}references/diagnostics.json`));
  assertEquals(catalog.diagnostics.map((entry: { code: string }) => entry.code), DIAGNOSTIC_CODES);

  assertEquals(text(`${skill}scripts/lesson-validator.js`), await Deno.readTextFile(new URL("public/tools/lesson-validator.js", repo)));
  const readme = text(`${PLUGIN_ROOT}README.md`);
  for (const required of ["claude plugin marketplace add", "git clone", `~/.claude/skills/${SKILL_NAME}`, "LEARN_TOKEN"]) assertStringIncludes(readme, required);
});
...
Deno.test("the example in the schema reference passes the JSON Schema and fails the resolver only on Card length", async () => {
  ...
  assertStringIncludes(text(`${skill}references/lesson-schema.md`), JSON.stringify(example, null, 2));
  const resolved = await resolveLesson(example);
  assertEquals([...new Set(resolved.diagnostics.map((diagnostic) => diagnostic.code))], ["card.words"]);
});
...
Deno.test("validate.mjs runs the bundled resolver: exit 0 with OK on the demo lesson, exit 1 with codes and fixes on a broken one", async () => {
  const good = await run("deno", ["run", "--allow-read", `${scripts}validate.mjs`, new URL("fixtures/lessons/browser-http-cache.json", repo).pathname]);
  assertEquals(good.code, 0, good.stderr);
  assertMatch(good.stdout, /^OK: valid lesson\/v1, 0 warning\(s\), fingerprint sha256:[0-9a-f]{64}\n$/);
  const bad = await run("deno", ["run", "--allow-read", `${scripts}validate.mjs`, new URL("fixtures/authoring/invalid/demo-path-two-diagnostics.json", repo).pathname]);
  assertEquals(bad.code, 1);
  assertStringIncludes(bad.stdout, "FAIL lesson.key.longest at /concepts:");
  assertStringIncludes(bad.stdout, "FAIL card.words at /concepts/0/cards/0/body:");
  assertStringIncludes(bad.stdout, "fix: Write 120 to 200 words");
  assertStringIncludes(bad.stdout, "FAILED: 2 error(s), 0 warning(s)");
  const usage = await run("deno", ["run", "--allow-read", `${scripts}validate.mjs`]);
...
Deno.test("validate.mjs --remote agrees byte for byte with the resolution route", async () => {
  const server = serve(app);
  try {
    const result = await run("deno", ["run", "--allow-read", "--allow-net=127.0.0.1", "--allow-env=LEARN_BASE_URL", `${scripts}validate.mjs`, new URL("fixtures/lessons/browser-http-cache.json", repo).pathname, "--remote"], { env: { LEARN_BASE_URL: server.origin } });
    assertEquals(result.code, 0, result.stderr + result.stdout);
    assertStringIncludes(result.stdout, "remote 200 from " + server.origin + ": identical to the local result");
  } finally {
    await server.close();
  }
});
...
class MemoryLessons implements LessonRepository {
  ...
  async featured() { return this.fixture; }
  async createLesson(_account: string, resolved: ResolvedLesson): Promise<StoredRevision> {
    const existing = this.revisions.find((revision) => revision.fingerprint === resolved.fingerprint);
    if (existing) return existing;
    const stored: StoredRevision = { lessonId: crypto.randomUUID(), revisionId: crypto.randomUUID(), revisionNumber: 1, status: "draft", fingerprint: resolved.fingerprint, content: resolved.normalizedLesson, createdAt: Date.now() };
  ...
  async learnableRevision() { return null; }
  ...
  const application = createApp({
    ...
    auth: {
      async authenticate(request, scope) {
        if (request.headers.get("authorization") !== `Bearer ${TOKEN}`) return { ok: false, reason: "unauthenticated" };
        if (scope !== "lessons:write") return { ok: false, reason: "forbidden" };
        return { ok: true, principal: { accountId: "owner", scopes: ["lessons:write"] } };
      },
    },
  });
...
Deno.test("submit.mjs creates a draft, prints the learning URL, returns the same revision twice and never prints the token", async () => {
  ...
  const lesson = new URL("fixtures/lessons/browser-http-cache.json", repo).pathname;
  const submit = (env: Record<string, string>, ...extra: string[]) => run("deno", ["run", "--allow-read", "--allow-net=127.0.0.1", "--allow-env=LEARN_TOKEN,LEARN_BASE_URL", `${scripts}submit.mjs`, lesson, ...extra], { env });
  try {
    const first = await submit({ LEARN_TOKEN: TOKEN, LEARN_BASE_URL: server.origin });
    ...
    assertStringIncludes(first.stdout, `learn:       ${server.origin}/learn/${lessonId}`);

    const second = await submit({ LEARN_TOKEN: TOKEN, LEARN_BASE_URL: server.origin });
    assertEquals(second.code, 0);
    assertEquals(lessons.revisions.length, 1, "the same document creates no second revision");
    assertStringIncludes(second.stdout, `revisionId:  ${revisionId}`);

    const wrong = await submit({ LEARN_TOKEN: "learn_pat_wrongprefi_" + "w".repeat(43), LEARN_BASE_URL: server.origin });
    ...
    const onCommandLine = await submit({ LEARN_TOKEN: TOKEN, LEARN_BASE_URL: server.origin }, TOKEN);
    assertEquals(onCommandLine.code, 2);

    for (const output of [first, second, wrong, missing, onCommandLine]) {
      assert(!output.stdout.includes(TOKEN) && !output.stderr.includes(TOKEN), "the token never appears in output");
      assert(!/learn_pat_[A-Za-z0-9_-]{10}_/.test(output.stdout + output.stderr), "no token-shaped string appears in output");
    }
  } finally {
    await server.close();
  }
...
Deno.test("the Claude Code plugin validator accepts the plugin and the marketplace manifests", async () => {
  ...
  const plugin = await run("claude", ["plugin", "validate", new URL(PLUGIN_ROOT, repo).pathname, "--strict"]);
  assertEquals(plugin.code, 0, plugin.stdout + plugin.stderr);
  const directory = await Deno.makeTempDir();
  try {
    await Deno.mkdir(`${directory}/.claude-plugin`);
    await Deno.writeFile(`${directory}/.claude-plugin/marketplace.json`, generated.files[`${PLUGIN_ROOT}marketplace.json`]);
    const marketplace = await run("claude", ["plugin", "validate", directory, "--strict"]);
