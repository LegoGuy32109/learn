...
async function harness() {
  ...
  const dependencies = {
    ...
    lessons: new FixtureLessonRepository(fixture, FIXTURE_ACCOUNT.id, () => now),
    auth: stubAuthenticator({
      ...
      [STRANGER]: { accountId: "someone-else", scopes: ["lessons:read", "lessons:write"] },
    }),
  };
  ...
  const create = async (title: string, token = OWNER, lessonId?: string) => {
    now += 1000;
    const document = { ...structuredClone(fixture), title } as Record<string, unknown>;
    delete document.lessonId;
    delete document.revisionId;
    const path = lessonId ? `/api/v1/lessons/${lessonId}/revisions` : "/api/v1/lessons";
    const response = await call(path, { method: "POST", headers: { ...bearer(token), "content-type": "application/json" }, body: JSON.stringify(document) });
...
Deno.test("the shelf needs an account: 401 for nobody, 403 for a token without lessons:read", async () => {
  ...
  assertEquals(anonymous.headers.get("content-type"), "application/problem+json; charset=utf-8");
  await anonymous.body?.cancel();
  const forbidden = await h.call("/api/v1/shelf", { headers: h.bearer(WRITER_ONLY) });
  assertEquals(forbidden.status, 403);
  assertStringIncludes((await forbidden.json()).detail, "lessons:read");
  const bogus = await h.call("/api/v1/shelf", { headers: { cookie: "learn_session=v1.not.real" } });
...
Deno.test("the shelf lists the account's lessons newest first with each newest revision, by cookie or by bearer", async () => {
  ...
  const revised = await h.create("A lesson from the laptop, revised", OWNER, first.lessonId);
  ...
  for (const headers of [h.bearer(OWNER), await h.cookie()]) {
    ...
    assertEquals(lessons.map((lesson: any) => lesson.title), ["A lesson from the laptop, revised", "Another lesson", fixture.title]);
    ...
    assert(lessons.every((lesson: any) => !("content" in lesson)), "the shelf never carries content");
  }
});
...
Deno.test("a revision and a lesson's newest revision are readable with the cookie, and only by their owner", async () => {
  ...
  const revision = await h.call(`/api/v1/lessons/${created.lessonId}/revisions/${created.revisionId}`, { headers: cookie });
  ...
  const latest = await h.call(`/api/v1/lessons/${created.lessonId}`, { headers: cookie });
  assertEquals(latest.status, 200);
  assertEquals((await latest.json()).revisionId, created.revisionId);

  const stranger = await h.call(`/api/v1/lessons/${created.lessonId}/revisions/${created.revisionId}`, { headers: h.bearer(STRANGER) });
  ...
  assert((await listed.json()).revisions.some((entry: any) => entry.revisionId === created.revisionId));
});
...
Deno.test("the learning URL inlines the owner's newest revision for the signed-in owner and the featured lesson for anyone else", async () => {
  ...
  const owner = await h.call(`/learn/${created.lessonId}`, { headers: await h.cookie() });
...
Deno.test("the in-memory repository behaves like the database one: idempotent fingerprints and owned revisions", async () => {
  ...
  const resolved = await resolveLesson({ ...structuredClone(fixture), title: "Memory" });
  ...
  assertEquals((await repository.shelf("owner")).map((lesson) => lesson.title), ["Memory", fixture.title]);
  assertEquals(await repository.latestRevision("other", first.lessonId), null);
  assertEquals((await repository.latestRevision("owner", first.lessonId))?.revisionId, first.revisionId);
