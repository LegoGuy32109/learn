...
import { validateScopes, TokenAdminError } from "../../src/server/identity/token-admin.ts";
...
Deno.test("redaction keeps the prefix and removes the secret from any text", () => {
  const redacted = redactBearerTokens(`request failed with Authorization: Bearer ${token} and again ${token}`);
  assert(!redacted.includes(token));
  assertStringIncludes(redacted, `learn_pat_${prefix}_[redacted]`);
  assertEquals(redactBearerTokens("Bearer opaque.credential-value"), "Bearer [redacted]");
  assertEquals(redactBearerTokens("nothing secret here"), "nothing secret here");
  assert(!redactedErrorText(new Error(`driver echoed ${token}`)).includes(token));
});

Deno.test("a missing or invalid token gets 401 and a valid token without the scope gets 403", async () => {
  const forbidding = appWith({ async authenticate() { return { ok: false, reason: "forbidden" }; } });
  const forbidden = await forbidding(new Request("http://local/api/v1/lessons", { method: "POST", headers: { authorization: `Bearer ${token}` }, body: JSON.stringify(lesson) }));
  assertEquals(forbidden.status, 403);
  assertEquals(forbidden.headers.get("content-type"), "application/problem+json; charset=utf-8");
  ...
  const rejecting = appWith({ async authenticate() { return { ok: false, reason: "unauthenticated" }; } });
  const unauthenticated = await rejecting(new Request("http://local/api/v1/lessons", { headers: { authorization: `Bearer ${token}` } }));
...
Deno.test("a bearer token passed into an error path never reaches the error message or the log", async () => {
  const failing = appWith({ async authenticate(request) { throw new Error(`lookup failed for ${request.headers.get("authorization")}`); } });
  const { result, logged } = await capturingConsoleError(() => failing(new Request("http://local/api/v1/lessons", { headers: { authorization: `Bearer ${token}` } })));
...
Deno.test("scope validation rejects unknown and empty scope lists", () => {
  assertEquals(validateScopes(["lessons:read", " lessons:read", "lessons:write"]), ["lessons:read", "lessons:write"]);
  let failure: unknown;
  try { validateScopes(["lessons:publish"]); } catch (error) { failure = error; }
  assert(failure instanceof TokenAdminError);
  try { validateScopes([]); } catch (error) { failure = error; }
  assert(failure instanceof TokenAdminError);
});
...
Deno.test("token script helpers parse durations and list only metadata columns", () => {
  ...
  const listing = formatList([{ name: "codex", prefix, scopes: ["lessons:read"], createdAt: 0, lastUsedAt: null, expiresAt: 86_400_000, revokedAt: null }]);
  assertEquals(listing.split("\n")[0].split(/\s{2,}/), ["name", "prefix", "scopes", "created", "last used", "expires", "revoked"]);
