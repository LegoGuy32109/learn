...
import { PasskeyService, INVITE_TTL_MS } from "../../src/server/identity/passkeys.ts";
import { HmacSessionCookies, randomSessionKey, SESSION_COOKIE, SESSION_TTL_MS } from "../../src/server/identity/sessions.ts";
import { configuredRelyingParty, relyingPartyFor } from "../../src/server/identity/relying-party.ts";
...
async function harness(relyingParty: ReturnType<typeof configuredRelyingParty> = null) {
  ...
  const dependencies = {
    ...
    auth: stubAuthenticator({
      [OWNER]: { accountId: FIXTURE_ACCOUNT.id, scopes: ["lessons:read", "account:owner"], prefix: "ownerpref1" },
  ...
  const post = (path: string, payload: unknown, headers: Record<string, string> = {}) =>
    call(path, { method: "POST", headers: { "content-type": "application/json", origin: ORIGIN, ...headers }, body: JSON.stringify(payload) });
  const mint = async (token = OWNER) => {
    const response = await call("/api/v1/sign-in-invites", { method: "POST", headers: { authorization: `Bearer ${token}` } });
    return { status: response.status, body: await response.json() };
  };
  return { app, call, post, mint, repository, advance: (ms: number) => { now += ms; } };
}
...
async function registerThrough(h: Awaited<ReturnType<typeof harness>>, invite: string, authenticator: SoftwareAuthenticator, ceremony = RP) {
  const options = await h.post("/api/v1/passkeys/registration-options", { invite });
...
async function signInThrough(h: Awaited<ReturnType<typeof harness>>, authenticator: SoftwareAuthenticator, ceremony = RP, signCount?: number) {
...
Deno.test("minting an invite needs an owner-scoped token: 401 invalid, 403 without the scope", async () => {
  ...
  assertEquals(minted.body.expiresAt, Date.parse("2026-09-21T12:00:00Z") + INVITE_TTL_MS);
});
...
Deno.test("an invite page registers once, then reports used, expired or unknown as a plain page", async () => {
  ...
  const registered = await registerThrough(h, minted.path.slice("/sign-in/".length), await SoftwareAuthenticator.create());
  ...
  const usedApi = await h.post("/api/v1/passkeys/registration-options", { invite: minted.path.slice("/sign-in/".length) });
  ...
  const expiredApi = await h.post("/api/v1/passkeys/registration-options", { invite: late.path.slice("/sign-in/".length) });
...
Deno.test("registration then sign-in issue a signed HttpOnly SameSite=Lax cookie that the shell honours", async () => {
  ...
  const { body: minted } = await h.mint();
  const registered = await registerThrough(h, minted.path.slice("/sign-in/".length), authenticator);
  assertEquals(registered.status, 200);
  ...
  assertStringIncludes(await shell.text(), '"signedIn":true,"displayName":"Josh"');
  const guest = await h.call("/");
  assertStringIncludes(await guest.text(), '"signedIn":false');

  const session = await h.call("/api/v1/session", { headers: cookieHeader(setCookie) });
  assertEquals(await session.json(), { signedIn: true, displayName: "Josh" });

  const signedOut = await h.call("/api/v1/session", { method: "DELETE", headers: { ...cookieHeader(setCookie), origin: ORIGIN } });
...
Deno.test("a tampered or expired cookie signs the browser out", async () => {
  ...
  const forgedPayload = btoa(JSON.stringify({ accountId: "someone-else", displayName: "Mallory", issuedAt: 0, expiresAt: Date.parse("2099-01-01") }))
  ...
  for (const candidate of tampered) {
    const response = await h.call("/api/v1/session", { headers: { cookie: `${SESSION_COOKIE}=${candidate}` } });
    assertEquals(await response.json(), { signedIn: false, displayName: null }, `accepted ${candidate.slice(0, 20)}`);
  }
  const intact = await h.call("/api/v1/session", { headers: cookieHeader(setCookie) });
  assertEquals((await intact.json()).signedIn, true);
  h.advance(SESSION_TTL_MS);
  const expired = await h.call("/api/v1/session", { headers: cookieHeader(setCookie) });
  assertEquals((await expired.json()).signedIn, false);
});

Deno.test("cookies are Secure everywhere except plain localhost", async () => {
  const h = await harness();
  const remote = await h.app(new Request("https://learn.joshhale.me/api/v1/session", { method: "DELETE", headers: { origin: "https://learn.joshhale.me" } }));
  assertStringIncludes(remote.headers.get("set-cookie") ?? "", "Secure");
  await remote.body?.cancel();
  const lan = await h.app(new Request("http://192.168.1.20:8000/api/v1/session", { method: "DELETE" }));
...
Deno.test("a replayed challenge is rejected for registration and for sign-in", async () => {
  ...
  const authenticator = await SoftwareAuthenticator.create();
  const { body: minted } = await h.mint();
  const options = await h.post("/api/v1/passkeys/registration-options", { invite: minted.path.slice("/sign-in/".length) });
  const { options: creation } = await options.json();
  const wrongOrigin = await authenticator.register(creation, { origin: "https://evil.example", rpId: "localhost" });
  const first = await h.post("/api/v1/passkeys/registrations", { invite: minted.path.slice("/sign-in/".length), credential: wrongOrigin });
  assertEquals(first.status, 400);
  await first.body?.cancel();
  const replayed = await authenticator.register(creation, RP);
  const second = await h.post("/api/v1/passkeys/registrations", { invite: minted.path.slice("/sign-in/".length), credential: replayed });
  assertEquals(second.status, 401);
  assertStringIncludes((await second.json()).detail, "challenge");
  assertEquals((await h.call(minted.path)).status, 200, "the invite survives a failed ceremony");
  ...
  const again = await h.post("/api/v1/passkeys/authentications", { credential });
...
Deno.test("a wrong origin or relying-party ID fails verification, and the sign count must advance", async () => {
  ...
  const authenticator = await SoftwareAuthenticator.create();
  const { body: minted } = await h.mint();
  const badRp = await registerThrough(h, minted.path.slice("/sign-in/".length), authenticator, { origin: ORIGIN, rpId: "evil.example" });
  ...
  const wrongOrigin = await signInThrough(h, authenticator, { origin: "https://evil.example", rpId: "localhost" });
  assertEquals(wrongOrigin.status, 400);
  assert(!wrongOrigin.headers.get("set-cookie"));
  await wrongOrigin.body?.cancel();
  const wrongRp = await signInThrough(h, authenticator, { origin: ORIGIN, rpId: "evil.example" });
  ...
  assertEquals((await h.repository.credential(authenticator.id))?.signCount, authenticator.signCount);
...
Deno.test("passkeys are refused off the configured origin, and cross-site posts are refused", async () => {
  ...
  const lan = await unconfigured.app(new Request("http://192.168.1.20:8000/api/v1/passkeys/authentication-options", { method: "POST" }));
  assertEquals(lan.status, 501);
  await lan.body?.cancel();
  const crossSite = await unconfigured.post("/api/v1/passkeys/authentication-options", {}, { origin: "https://evil.example" });
  assertEquals(crossSite.status, 403);
  await crossSite.body?.cancel();

  const pinned = configuredRelyingParty({ rpId: "learn.joshhale.me", origins: "https://learn.joshhale.me, https://learn-dev.deno.dev" });
  assertEquals(pinned?.expectedOrigins, ["https://learn.joshhale.me", "https://learn-dev.deno.dev"]);
  assertEquals(configuredRelyingParty({ rpId: "learn.joshhale.me" }), null);
  assertEquals(configuredRelyingParty({ origins: "https://learn.joshhale.me" }), null);
  assertEquals(relyingPartyFor(new Request("https://learn.joshhale.me/x", { headers: { origin: "https://learn.joshhale.me" } }), pinned)?.rpId, "learn.joshhale.me");
  assertEquals(relyingPartyFor(new Request("https://other.example/x", { headers: { origin: "https://other.example" } }), pinned), null);
  assertEquals(relyingPartyFor(new Request("http://localhost:8000/x"), pinned), null);
  assertEquals(relyingPartyFor(new Request("http://localhost:8000/x"), null)?.expectedOrigins, ["http://localhost:8000"]);
  assertEquals(relyingPartyFor(new Request("http://127.0.0.1:8000/x"), null), null);

  const configured = await harness(pinned);
  const wrongHost = await configured.post("/api/v1/passkeys/authentication-options", {});
...
Deno.test("redaction hides invite links and session cookies", () => {
  ...
  const redacted = redactBearerTokens(`open http://localhost/sign-in/${token} cookie learn_session=v1.abc.def; Path=/`);
