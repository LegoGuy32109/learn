...
async function snap(page: any, name: string): Promise<string> {
  shot += 1;
  const file = `${SCREENSHOTS}${String(shot).padStart(2, "0")}-${name.replace(/[^a-z0-9]+/gi, "-").toLowerCase()}.jpg`;
...
Deno.test({
  ...
  fn: async () => {
    ...
    try {
      ...
      await check("secrets · no response body anywhere in this run leaked a credential or a token-shaped string", () => {
...
async function signIn(browser: any): Promise<SignedIn> {
  ...
  await check("setup · a sign-in invite minted the way Josh mints one opens for his account", async () => {
  ...
  try {
    ...
    await check("setup · the invite page names Josh's account with one Register a passkey button", async () => {
    ...
    await check("setup · registering a passkey signs in and lands on the shelf, as the phone would after Add to Home Screen", async () => {
    ...
    observe("home screen", "\"Add to Home Screen\" itself is not something a headless browser can drive or verify; the manifest that makes it installable (name, icons, standalone display) is already asserted by ticket 14's audit_prod suite, so this run only re-proves the sign-in half.");
  } catch (error) {
...
async function shelfAndLearningLoop(browser: any, signedIn: SignedIn) {
  ...
  await check("author · a lesson draft is created with the owner's bearer token (stands in for the plugin's own submission)", async () => {
    const response = await fetch(`${BASE}/api/v1/lessons`, { method: "POST", headers: { authorization: `Bearer ${ownerToken}`, "content-type": "application/json" }, body: lessonDocument(title) });
  ...
  try {
    ...
    await check("shelf · the authored lesson is on the shelf, Not started, for the signed-in owner", async () => {
      ...
      await expect(shelfPage.locator(`[data-lesson="${lessonId}"] .lstatus`)).toHaveText("Not started");
  ...
  await fullWalk(browser, BASE, errors, consoleMessages, { lessonPath, lessonId, timeout: 10000, context });
  await drillFromFresh(browser, BASE, errors, consoleMessages, { lessonPath, lessonId, timeout: 10000, context });
  observe("learning loop", "fullWalk carries the Cards, a wrong Check answer with its belief and correcting Card, an unseen retry, a correct answer, \"I don't know\" on another Concept, the Wrap-up (which draws a reserved Question the Checks never asked) and the Learned summary, then opens \"Every question\", answers one wrong and closes the drill unfinished, asserting the shelf and evidence stores are unchanged by it. drillFromFresh separately runs the drill to completion from a fresh state.");
}
...
async function offlineAndSecondDevice(browser: any, signedIn: SignedIn) {
  ...
  await check("author · a second lesson draft, isolated from the learning-loop run, for the offline and second-device scenarios", async () => {
    const response = await fetch(`${BASE}/api/v1/lessons`, { method: "POST", headers: { authorization: `Bearer ${ownerToken}`, "content-type": "application/json" }, body: lessonDocument(title) });
    const body = await response.json();
    expect(response.status).toBe(201);
    lessonId = body.lessonId;
    revisionId = body.revisionId;
    created.push(`lesson ${lessonId} "${title}" (offline and second-device run)`);
  ...
  try {
    ...
    await check("shelf · this uniquely-titled fresh draft is first on the shelf, newest first, Not started", async () => {
      await orderPage.getByRole("button", { name: "Refresh shelf" }).click();
      await settle(orderPage);
      const firstId = await orderPage.locator(".lesson").first().getAttribute("data-lesson");
      expect(firstId, "the newest lesson is not first on the shelf").toBe(lessonId);
      await expect(orderPage.locator(`[data-lesson="${lessonId}"] .lstatus`)).toHaveText("Not started");
  ...
  try {
    ...
    await check("offline · Card 2 of Concept 1 is on screen before going offline", () => {
      expect(resumeSignature.surface).toBe("learn");
      expect(resumeSignature.cardHeading).toBeTruthy();
      return `surface=${resumeSignature.surface} card=${JSON.stringify(resumeSignature.cardHeading)}`;
    });
    ...
    await check("offline · closing the app and reopening from the icon, offline, resumes at the exact Card", async () => {
      ...
      expect(afterReopen.surface, `landed on ${JSON.stringify(afterReopen)} instead of the Card left off on`).toBe("learn");
      expect(afterReopen.cardHeading).toBe(resumeSignature.cardHeading);
      return `resumed at card=${JSON.stringify(afterReopen.cardHeading)}; ${await snap(page, "concept 2 reached offline".replace("2", "1 resumed"))}`;
    });

    await check("offline · a whole Concept (its remaining Card, then a correct Check answer) completes with the network off", async () => {
      ...
      return `after finishing Concept 1 offline: ${JSON.stringify({ surface: after.surface, cardHeading: after.cardHeading, stem: after.stem })}; ${await snap(page, "concept 2 reached offline")}`;
    });
    ...
    await check("offline · back online, the outbox drains (the client pushed every offline event)", () => {
      expect(flushed, "the outbox never emptied within 15s of reconnecting (after a reload to skip backoff)").toBe(true);
    });
    await check("offline · the server holds the offline-completed Concept once back online", async () => {
      const checkpoint = await page.evaluate(async ({ revisionId }: { revisionId: string }) => {
        const response = await fetch(`/api/v1/progress/checkpoint?revision=${revisionId}&epoch=0`);
      ...
      expect(checkpoint.status, `GET /api/v1/progress/checkpoint -> ${checkpoint.status}: ${checkpoint.text.slice(0, 500)}`).toBe(200);
      ...
      return `GET /api/v1/progress/checkpoint -> ${JSON.stringify(checkpoint.body)}`;
    });
  } finally {
  ...
  try {
    ...
    await check("second device · a second signed-in context shows the same progress and resumes at the same place", async () => {
      await expect(secondPage.locator(".overview .state")).toHaveText("In progress");
      ...
      return `second device surface: ${JSON.stringify({ surface: there.surface, cardHeading: there.cardHeading, stem: there.stem, status: there.status })}; ${await snap(secondPage, "second device same progress")}`;
    });
  } finally {
