...
import { BATCH_SIZE, createSyncClient, KICK_DELAY_MS, RETRY_BASE_MS, type SyncState } from "../../src/client/sync/client.js";
import type { Transport } from "../../src/client/sync/transport.js";
import type { OutboxEntry, ProgressStream } from "../../src/client/storage/repository.js";
...
function memoryRepository() {
  const stores: Record<string, Map<string, any>> = { learning_events: new Map(), navigation_events: new Map() };
  ...
  return {
    ...
    async appendOutgoing(store: string, event: any) {
      ...
      clock += 1;
      outbox.set(event.id, { id: event.id, store, event, queuedAt: String(clock).padStart(6, "0") });
    },
    async appendRemote(store: string, events: any[]) {
      for (const event of events) if (!stores[store].has(event.id)) stores[store].set(event.id, event);
    },
    async events(store: string) {
      return Array.from(stores[store].values());
    },
    async outbox() {
      return Array.from(outbox.values()).sort((a, b) => a.queuedAt.localeCompare(b.queuedAt));
    },
...
function fakeTimers() {
  ...
  return {
    ...
    async run(ms: number) {
      const until = now + ms;
      while (true) {
        const due = Array.from(scheduled.entries()).filter(([, entry]) => entry.at <= until).sort((a, b) => a[1].at - b[1].at)[0];
        if (!due) break;
        now = due[1].at;
...
function fakeServer() {
  const stored: Record<string, Map<string, any>> = { learning_events: new Map(), navigation_events: new Map() };
  const arrival: Record<string, any[]> = { learning_events: [], navigation_events: [] };
  const calls: string[] = [];
  let epoch = 0;
  /** @type {null | { status: number, code?: string, loseResponse?: boolean }} */
  let failing: { status: number; code?: string; loseResponse?: boolean } | null = null;
  let pageSize = 1000;
  const transport: Transport = {
    async push(store, scope, events) {
      calls.push(`push:${store}:${events.length}`);
      if (failing && !failing.loseResponse) return { ok: false, status: failing.status, code: failing.code ?? "http", message: "nope", stream: failing.code === "epoch.stale" ? { lessonId: LESSON, lessonRevisionId: "newer-revision", epoch } : null };
      if (scope.epoch < epoch) return { ok: false, status: 409, code: "epoch.stale", message: "stale", stream: { lessonId: LESSON, lessonRevisionId: "newer-revision", epoch } };
      let accepted = 0;
      ...
      if (failing?.loseResponse) return { ok: false, status: 0, code: "network", message: "lost", stream: null };
      return { ok: true, accepted, duplicates: events.length - accepted, stream: { lessonId: LESSON, lessonRevisionId: REVISION, epoch } };
    },
    async pull(store, scope, cursor) {
      calls.push(`pull:${store}:${cursor || "start"}`);
      if (failing) return { ok: false, status: failing.status, code: failing.code ?? "http", message: "nope", stream: null };
      if (scope.epoch < epoch) return { ok: false, status: 409, code: "epoch.stale", message: "stale", stream: { lessonId: LESSON, lessonRevisionId: "newer-revision", epoch } };
      const after = cursor ? Number(atob(cursor)) : 0;
      const slice = arrival[store].slice(after, after + pageSize);
      const next = after + slice.length;
      return { ok: true, events: slice, cursor: btoa(String(next)), hasMore: next < arrival[store].length, stream: { lessonId: LESSON, lessonRevisionId: REVISION, epoch } };
    },
  };
  return {
    transport,
...
function event(type: string, extra: Record<string, unknown> = {}) {
  return { id: crypto.randomUUID(), type, lessonRevisionId: REVISION, epoch: 0, occurredAt: new Date().toISOString(), ...extra };
}
...
function harness() {
  ...
  const client = createSyncClient({ repository, transport: server.transport, onStatus: (state) => states.push(state), timers, random: () => 0.5 });
...
Deno.test("the retry delay doubles from one second and caps at one minute", async () => {
  ...
  assertEquals(delays, [1, 2, 4, 8, 16, 32, 60, 60].map((seconds) => seconds * RETRY_BASE_MS));
});
...
Deno.test("an event enters the outbox before the UI is told, and no request leaves before a render kicks the client", async () => {
  ...
  assertEquals((await h.repository.outbox()).map((entry) => entry.id), [made.id]);
...
Deno.test("uploads are bounded batches per stream and epoch; repeated kicks coalesce into one cycle", async () => {
  ...
  h.client.setEnabled(true);
  h.client.attach(SCOPE);
  for (let i = 0; i < BATCH_SIZE * 2 + 3; i++) await h.repository.appendOutgoing("learning_events", event("lesson_started"));
  for (let i = 0; i < 2; i++) await h.repository.appendOutgoing("navigation_events", event("navigation_checkpointed", { checkpoint: null }));
  h.client.kick();
  h.client.kick();
  h.client.kick();
  await h.timers.run(KICK_DELAY_MS * 3);
  const pushes = h.server.calls.filter((call) => call.startsWith("push:"));
  assertEquals(pushes, [`push:learning_events:${BATCH_SIZE}`, `push:learning_events:${BATCH_SIZE}`, "push:learning_events:3", "push:navigation_events:2"]);
...
Deno.test("a response lost after the server accepted a batch is retried with backoff and duplicates nothing", async () => {
  ...
  for (const made of events) await h.repository.appendOutgoing("learning_events", made);
  h.server.fail({ status: 0, loseResponse: true });
  await h.client.flush();
  assertEquals(h.server.stored.learning_events.size, 2, "the server accepted the batch");
  assertEquals((await h.repository.outbox()).length, 2, "the client never learned that, so the events stay queued");
  assertEquals(h.statuses().at(-1), "local", "an unreachable network reads as Saved on this device");
  assertEquals(h.timers.pending(), [RETRY_BASE_MS]);
  h.server.fail(null);
  await h.timers.run(RETRY_BASE_MS);
  assertEquals(h.server.calls.filter((call) => call.startsWith("push:")).length, 2);
  assertEquals(h.server.stored.learning_events.size, 2, "the retry stored nothing twice");
  assertEquals((await h.repository.outbox()).length, 0);
  assertEquals(h.statuses().at(-1), "synced");
...
Deno.test("with every request failing, learning continues, every event stays in the outbox, and backoff grows and caps", async () => {
  ...
  h.server.fail({ status: 503 });
  for (let i = 0; i < 5; i++) {
    await h.repository.appendOutgoing("learning_events", event("card_seen", { cardId: `c${i}` }));
    h.client.kick();
    await h.timers.run(KICK_DELAY_MS);
  }
  assertEquals((await h.repository.outbox()).length, 5);
  assertEquals((await h.repository.events("learning_events")).length, 5, "the local store is authoritative and complete");
  ...
  assertEquals(delays, [16, 32, 60, 60, 60, 60].map((seconds) => seconds * RETRY_BASE_MS));
  assertEquals((await h.repository.outbox()).length, 5, "still queued after every failure");
  h.server.fail({ status: 0 });
  await h.timers.run(h.timers.pending()[0]);
  assertEquals(h.statuses().at(-1), "local", "offline after a server error reads as Saved on this device");
  h.server.fail(null);
  await h.client.wake();
  assertEquals((await h.repository.outbox()).length, 0);
  assertEquals(h.statuses().at(-1), "synced");
...
Deno.test("the four states follow the network: local, syncing, synced, failed", async () => {
  ...
  await h.client.flush();
  assertEquals(h.server.calls.filter((call) => call.startsWith("push:")).length, 3, "a guest sends nothing");
});
...
Deno.test("pull merges by id across pages, never by id order, and hands the reducers the union once", async () => {
  ...
  h.client.setEnabled(true);
  const merges: number[] = [];
  h.client.attach(SCOPE, async () => void merges.push((await h.repository.events("learning_events")).length));
  // Another device pushed five events whose ids sort opposite to their arrival.
  const remote = ["f", "e", "d", "c", "b"].map((letter, index) => ({ ...event("card_seen", { cardId: `card-${index}` }), id: `${letter}0000000-0000-4000-8000-000000000000` }));
  for (const made of remote) await h.server.transport.push("learning_events", SCOPE, [made]);
  const mine = { ...event("lesson_started"), id: "a0000000-0000-4000-8000-000000000000" };
  ...
  assertEquals(new Set(local.map((made) => made.id)), new Set([mine.id, ...remote.map((made) => made.id)]));
  assertEquals(merges, [6], "one merge after the whole pull, with the full union");
  const pulls = h.server.calls.filter((call) => call.startsWith("pull:learning_events"));
...
Deno.test("a newer epoch on the server is reported, never applied on its own, and applied only by the explicit discard", async () => {
  ...
  await h.repository.appendOutgoing("learning_events", event("card_seen", { cardId: "c" }));
  await h.client.flush();
  const state = h.states.at(-1)!;
  assertEquals(state.status, "failed");
  assertEquals(state.remote, { lessonId: LESSON, lessonRevisionId: "newer-revision", epoch: 1 });
  assertEquals(h.repository.streams.size, 0, "the local stream did not move");
  assertEquals((await h.repository.events("learning_events")).length, 2, "local evidence is kept");
  assertEquals((await h.repository.outbox()).length, 0, "the refused event is no longer retried");
  assertEquals(h.timers.pending(), [], "no retry: only the learner can resolve this");
...
Deno.test("permanent refusals drop the batch from the outbox and read as failed; a 401 turns the client off", async () => {
  ...
  await h.client.flush();
  assertEquals(h.statuses().at(-1), "hidden");
  assertEquals((await h.repository.outbox()).length, 1, "a guest's events wait for the next sign-in");
});
