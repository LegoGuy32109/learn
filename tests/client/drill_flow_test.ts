...
import { advanceDrill, drillPosition, startDrill } from "../../src/client/learning/drill-flow.js";
import { current, enterCorrective, leaveCorrective, submitAnswer } from "../../src/client/learning/flow.js";
...
function wrongAnswer(question: any): string {
  if (question.type === "mcq") {
    const concept = lesson.concepts.find((candidate) => candidate.id === question.conceptId) as any;
...
Deno.test("drill asks each Question once, wrong or not, and ends on the summary", () => {
  ...
  for (let steps = 0; steps < 50 && flow.screen !== "summary"; steps++) {
    ...
    const submitted = submitAnswer(lesson, flow as any, steps % 2 ? wrongAnswer(question) : null, steps % 2 === 0);
    assertEquals(submitted.correct, false);
    assert(submitted.flow.feedback);
    assertEquals(submitted.flow.queue, flow.queue, "feedback never auto-advances");
...
Deno.test("a wrong drill answer carries the belief and the correcting Card, and the detour returns to the same Question", () => {
  let flow = startDrill(lesson, run);
  const index = flow.queue.findIndex((id) => (lesson.questions as any[]).find((question) => question.id === id).type === "mcq");
  ...
  const misconception = concept.misconceptions.find((candidate: any) => candidate.id === question.map[distractor]);
