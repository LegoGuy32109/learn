...
function revision(lessonId: string, revisionId: string, title: string, cachedAt = "2026-09-20T10:00:00Z") {
  return { lesson: { ...structuredClone(fixture), lessonId, revisionId, title }, cachedAt };
}

function remote(lessonId: string, latestRevisionId: string, title: string, updatedAt: number) {
  return { lessonId, title, conceptCount: 3, questionCount: 12, latestRevisionId, updatedAt };
}

function started(revisionId: string, epoch = 0) {
  return { id: crypto.randomUUID(), type: "lesson_started", lessonRevisionId: revisionId, epoch, occurredAt: "2026-09-20T10:01:00Z" };
}

Deno.test("server lessons and cached revisions merge into one entry per Lesson, newest first", () => {
  const shelf = buildShelf({
    cached: [revision(LESSON, R1, "Cached and owned"), revision("guest-only", "rev-g", "Only on this device", "2026-09-01T00:00:00Z")],
    remote: [remote("server-only", "rev-s", "Just created", Date.parse("2026-09-21T09:00:00Z")), remote(LESSON, R1, "Cached and owned", Date.parse("2026-09-10T00:00:00Z"))],
    streams: [],
    learningEvents: [],
  });
  assertEquals(shelf.map((entry) => entry.lessonId), ["server-only", LESSON, "guest-only"]);
  assertEquals(shelf.map((entry) => entry.progress.state), ["not_started", "not_started", "not_started"]);
...
Deno.test("a guest sees only what this device cached, and evidence pins the revision it was recorded against", () => {
  const shelf = buildShelf({
    cached: [revision(LESSON, R1, "Old", "2026-09-01T00:00:00Z"), revision(LESSON, R2, "New", "2026-09-15T00:00:00Z")],
    remote: null,
    streams: [],
...
Deno.test("Outdated appears only when progress exists on an older revision than the server's newest", () => {
  ...
  assertEquals(withProgress.revisionId, R1, "progress stays pinned to the old revision");
  assertEquals(withProgress.title, "Old");

  const [unstarted] = buildShelf({ ...input, learningEvents: [] });
  assertEquals(unstarted.outdated, false);
  assertEquals(unstarted.revisionId, R2, "an unstarted lesson follows the server to the newest revision");
...
Deno.test("discarding advances the epoch and repins; opening pins at the current epoch", () => {
  ...
  const [entry] = buildShelf({ cached: [revision(LESSON, R1, "Old")], remote: null, streams: [], learningEvents: [] });
  assertEquals(pinOnOpen(undefined, entry), { id: LESSON, revisionId: R1, epoch: 0 });
  assertEquals(pinOnOpen(stream, { ...entry, revisionId: R1 }), stream);
  assertEquals(pinOnOpen({ id: LESSON, revisionId: R1, epoch: 2 }, { ...entry, revisionId: R2 }), { id: LESSON, revisionId: R2, epoch: 2 });
});

Deno.test("an empty device and no server answer is an empty shelf, not an error", () => {
  assertEquals(buildShelf({ cached: [], remote: null, streams: [], learningEvents: [] }), []);
});
