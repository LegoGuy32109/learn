...
import { base64Url, fromBase64Url } from "../../src/server/identity/encoding.ts";
...
export function cbor(value: number | string | Uint8Array | Map<number | string, unknown> | Record<string, unknown>): Uint8Array {
  ...
  if (value instanceof Uint8Array) return concat(header(2, value.byteLength), value);
  const entries = value instanceof Map ? Array.from(value.entries()) : Object.entries(value);
  const encoded = entries.map(([key, item]) => concat(cbor(key as number | string), cbor(item as Parameters<typeof cbor>[0])));
...
function clientData(type: "webauthn.create" | "webauthn.get", ceremony: Ceremony): Uint8Array {
  return encoder.encode(JSON.stringify({ type, challenge: ceremony.challenge, origin: ceremony.origin, crossOrigin: false }));
}

function flags(ceremony: Ceremony, attested: boolean): number {
  return 0x01 | (ceremony.userVerified === false ? 0 : 0x04) | (attested ? 0x40 : 0);
}
...
export class SoftwareAuthenticator {
  ...
  static async create(): Promise<SoftwareAuthenticator> {
    ...
    const coseKey = cbor(new Map<number, unknown>([[1, 2], [3, -7], [-1, 1], [-2, fromBase64Url(jwk.x!)!], [-3, fromBase64Url(jwk.y!)!]]));
    return new SoftwareAuthenticator(keyPair, crypto.getRandomValues(new Uint8Array(16)), coseKey, null, 0);
  }
  ...
  async register(options: { challenge: string; user: { id: string } }, ceremony: Omit<Ceremony, "challenge">): Promise<Record<string, unknown>> {
    ...
    const authData = concat(
      ...
      new Uint8Array(16),
      Uint8Array.of(this.credentialId.length >> 8, this.credentialId.length & 0xff),
      this.credentialId,
      this.coseKey,
    );
  ...
  async assert(options: { challenge: string }, ceremony: Omit<Ceremony, "challenge">, signCount?: number): Promise<Record<string, unknown>> {
    const full = { ...ceremony, challenge: options.challenge };
    if (signCount === undefined) this.signCount += 1;
    const counter = signCount ?? this.signCount;
    const authenticatorData = concat(await sha256(encoder.encode(ceremony.rpId)), Uint8Array.of(flags(full, false)), uint32(counter));
    const client = clientData("webauthn.get", full);
    const signed = concat(authenticatorData, await sha256(client));
    const raw = new Uint8Array(await crypto.subtle.sign({ name: "ECDSA", hash: "SHA-256" }, this.keyPair.privateKey, signed.buffer as ArrayBuffer));
    return {
      id: this.id,
      rawId: this.id,
      type: "public-key",
