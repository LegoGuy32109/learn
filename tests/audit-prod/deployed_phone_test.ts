...
Deno.test({
  name: "audit: the deployed phone experience",
  sanitizeOps: false,
  ...
  fn: async () => {
    ...
    try {
      ...
      const workerFetch = consoleMessages.filter((message) =>
        /bad HTTP response code \(404\) was received when fetching the script/
          .test(message)
      );
      const deliberate410 = consoleMessages.filter((message) =>
        /status of 410/.test(message)
      );
      const other = consoleMessages.filter((message) =>
        !workerFetch.includes(message) && !deliberate410.includes(message)
      );
      observe(
        "console",
        `${workerFetch.length} messages were the failed /sw.js fetch (see the worker checks); ${deliberate410.length} came from deliberately reopening the used invite (410).`,
      );
      await check(
      ...
      await check(
        ...
        () => {
          expect(
            workerFetch.length,
            `${workerFetch.length} page loads logged: ${
              redact(workerFetch[0] ?? "")
            }`,
          ).toBe(0);
        },
      );
      ...
      const text = [
        ...
        `- Served by revision(s): ${
          revisionsSeen.size ? [...revisionsSeen].join(", ") : "(none seen)"
        }`,
...
async function transportAndSecurity() {
  ...
  await check(
    ...
    () => {
      ...
      expect(shell.response.headers.get("content-type") ?? "").toMatch(
        /^text\/html/,
      );
  ...
  await check(
    ...
    () => {
      ...
      expect(Number(hsts!.match(/max-age=(\d+)/)?.[1] ?? 0))
  ...
  await check(
    ...
    () => {
      expect(
        capabilities.response.headers.get("strict-transport-security"),
  ...
  for (
    ...
    observe(
      `header · ${name}`,
      shell.response.headers.has(name)
        ? `present: ${shell.response.headers.get(name)}`
        : "absent on the shell (not required by the ticket)",
    );
  }
  ...
  await check(
    ...
    () => {
      ...
      expect(validator.response.headers.get("content-type") ?? "").toMatch(
        /^text\/javascript/,
      );
  ...
  observe(
    "content types · validator declarations",
    redact(
      `${types.response.status} ${
        types.response.headers.get("content-type")
      } (text/plain is what the asset route gives .ts files)`,
    ),
  );
  ...
  await check("content types · manifest is application/manifest+json", () => {
    ...
    expect(manifest.response.headers.get("content-type") ?? "").toMatch(
  ...
  await check(
    ...
    () => {
      expect(worker.response.status, redact(worker.text.slice(0, 300))).toBe(
        200,
      );
      expect(worker.response.headers.get("content-type") ?? "").toMatch(
        /^text\/javascript/,
      );
  ...
  for (
    ...
    await check(
      `cors · ${path} with a foreign Origin gets no wildcard or reflected Access-Control-Allow-Origin`,
      () => {
  ...
  const preflight = await probe("/api/v1/shelf", {
    method: "OPTIONS",
    headers: {
  ...
  await check("cors · a preflight from a foreign origin is not granted", () => {
    expect(preflight.response.headers.get("access-control-allow-origin"))
      .toBeNull();
    return preflight.evidence;
  });
  const crossSite = await probe("/api/v1/passkeys/authentication-options", {
    method: "POST",
    headers: {
      origin: "https://evil.example",
  ...
  const sameSite = await probe("/api/v1/passkeys/authentication-options", {
    method: "POST",
  ...
  await check(
    ...
    () => {
      ...
      expect(unauthenticated.response.headers.get("content-type") ?? "")
  ...
  const badToken = await probe("/api/v1/sign-in-invites", {
    method: "POST",
    headers: {
      authorization: "Bearer learn_pat_audit_notarealsecretvalue0000000000",
    },
  });
  await check(
  ...
  try {
    ...
    await check(
      ...
      () => {
        ...
        const sizes = parsed.manifest.icons.map((icon: { sizes: string }) =>
          icon.sizes
        );
        ...
        return `Page.getAppManifest: ${
          JSON.stringify({
            ...
            display: parsed.manifest.display,
            startUrl: parsed.manifest.startUrl,
            icons: sizes,
          })
        }`;
      },
    ...
    observe(
      "manifest · Page.getInstallabilityErrors",
      `${
        JSON.stringify(installability)
      } (headless Chromium reports an empty list for every page, so this proves nothing on its own)`,
    );
    for (
      const icon of [
        "/icons/icon-192.png",
    ...
    const registration = await page.evaluate(async () => {
      ...
      while (Date.now() < deadline) {
        ...
        if (found?.active && navigator.serviceWorker.controller) {
          return {
            registered: true,
            controlled: true,
            scope: found.scope,
            scriptURL: found.active.scriptURL,
          };
        }
      ...
      return {
        ...
        scriptURL: found?.active?.scriptURL ?? found?.installing?.scriptURL ??
          null,
      };
    });
    ...
    await check(
      ...
      () => {
        expect(
          ...
          `no registration; GET /sw.js answered ${workerScript.status}: ${
            redact(workerText.slice(0, 200))
          }`,
        ).toBe(true);
    ...
    await check(
      ...
      () => {
        expect(cacheNames.length, `caches: ${JSON.stringify(cacheNames)}`).toBe(
          1,
        );
        expect(cacheNames[0]).toMatch(/^learn-shell-[0-9a-f]{12}$/);
        expect(cachedPaths).toContain("/shell");
        expect(cachedPaths.some((path) => path.startsWith("/api/"))).toBe(
          false,
        );
        return `caches: ${
          JSON.stringify(cacheNames)
        }; ${cachedPaths.length} cached paths`;
      },
    );
    ...
    const apiFetch = await page.evaluate(async () => {
      const first = await fetch("/api/v1/session", {
        headers: { accept: "application/json" },
      });
      await first.text();
      const second = await fetch("/api/v1/session", {
        headers: { accept: "application/json" },
      });
      await second.text();
      return {
        ...
        cacheStatus: second.headers.get("cache-status"),
    ...
    await check(
      ...
      () => {
        ...
        return `second GET /api/v1/session: ${
          JSON.stringify(apiFetch)
        }; /api/ entries in caches: 0 of ${afterApi.length}`;
      },
    );
    ...
    try {
      ...
      offlineOutcome = `reload offline failed: ${
        error instanceof Error ? error.message.split("\n")[0] : String(error)
      }`;
    }
    const offlineText = offlineOutcome === "shell rendered offline"
...
async function signIn(browser: Browser): Promise<SignedIn> {
  ...
  await check(
    ...
    async () => {
      ...
      expect(invite.expiresAt - Date.now()).toBeLessThanOrEqual(
        10 * 60 * 1000 + 5000,
      );
      created.push(
        `sign-in invite ${
  ...
  try {
    ...
    await check(
      ...
      async () => {
        ...
        await expect(page.getByRole("button", { name: "Register a passkey" }))
          .toBeVisible();
        return `GET ${
          minted.path.slice(0, 16)
        }… -> ${landing.status()}; invite names ${
          JSON.stringify(outcome.displayName)
        }; ${await snap(page, "invite page")}`;
      },
    );
    ...
    if (outcome.credentialId) {
      created.push(
        `passkey credential ${
          outcome.credentialId.slice(0, 12)
        }… on Josh's production account (from this run's virtual authenticator; it cannot sign in from any real device)`,
      );
    }
    ...
    await check(
      ...
      () => {
        ...
        const attributes = setCookieHeader
          ? cookieAttributes(setCookieHeader)
          : "(Set-Cookie header not captured)";
        return `Set-Cookie: ${attributes}\nbrowser sees: httpOnly=${cookie.httpOnly} secure=${cookie.secure} sameSite=${cookie.sameSite} path=${cookie.path} domain=${cookie.domain} lifetime=${
          (lifetime / DAY).toFixed(1)
        } days`;
      },
    );
    await check(
      ...
      () => {
        expect(
          setCookieHeader,
          "Set-Cookie not observed on the registration response",
        ).toBeTruthy();
    ...
    await check(
      ...
      async () => {
        ...
        return `GET ${
          minted.path.slice(0, 16)
        }… -> ${reused.status()}; ${await snap(page, "invite already used")}`;
      },
    );
    const reusedOptions = await probe("/api/v1/passkeys/registration-options", {
      method: "POST",
      headers: { origin: BASE, "content-type": "application/json" },
      body: JSON.stringify({ invite: minted.path.slice("/sign-in/".length) }),
    });
    await check(
  ...
  if (outcome.storageState) {
    const reopened = await browser.newContext({
      ...PHONE,
      storageState: outcome.storageState,
    });
    ...
    try {
      ...
      await check(
        ...
        async () => {
          ...
          }; ${await snap(page, "shelf after restart")}`;
        },
      );
    } finally {
...
async function shelf(browser: Browser, signedIn: SignedIn) {
  if (!signedIn.storageState) {
    await check("shelf · the shelf section needs a signed-in session", () => {
      throw new Error(
        "Sign-in did not complete, so the shelf checks could not run.",
      );
    });
    return;
  }
  ...
  await check(
    ...
    () => {
      ...
      return describe("POST", "/api/v1/lessons", draft.response, [
        `lessonId ${lessonId}`,
  ...
  const context = await browser.newContext({
    ...PHONE,
    storageState: signedIn.storageState,
  });
  const page = await context.newPage();
  page.setDefaultTimeout(10000);
  attachListeners(page, errors, consoleMessages);
  ...
  try {
    ...
    await check(
      ...
      async () => {
        ...
        await expect(page.getByRole("button", { name: "Start lesson" }))
          .toBeVisible();
        const cached = (await readStore(page, "lessons")).map((record) =>
          record.id
        );
        expect(cached).toContain(revisionId);
        expect(await readStore(page, "progress_streams")).toEqual([{
          id: lessonId,
          revisionId,
          epoch: 0,
        }]);
        return `IndexedDB lessons has ${revisionId}; progress_streams pins epoch 0; ${await snap(
          page,
          "audit draft overview",
        )}`;
      },
    );
    ...
    try {
      ...
      offlineReload = error instanceof Error
        ? error.message.split("\n")[0]
        : String(error);
    }
    await check(
    ...
    await check(
      ...
      async () => {
        ...
        expect(
          events.every((event) =>
            event.lessonRevisionId === revisionId && event.epoch === 0
          ),
        ).toBe(true);
        return `${events.length} learning events recorded offline against revision 1; ${await snap(
          page,
          "concept 2 reached offline",
        )}`;
      },
    );
    await context.setOffline(false);
    ...
    await check(
      ...
      () => {
        ...
        created.push(
          `lesson ${lessonId} "${revisedTitle}" revision 2 ${revisedId}`,
        );
        return describe(
          "POST",
          `/api/v1/lessons/${lessonId}/revisions`,
          revised.response,
          [`revisionId ${revisedId}`],
        );
      },
    );
    await page.goto(`${BASE}/`);
    ...
    await check(
      ...
      async () => {
        ...
        await page.getByRole("button", { name: "Resume this revision" })
          .click();
    ...
    await check(
      ...
      async () => {
        ...
        await expect(page.locator(".overview .state")).toHaveText(
          "Not started",
        );
        ...
        expect((await readStore(page, "lessons")).map((record) => record.id))
          .toContain(revisedId);
        return `stream now revision 2, epoch 1; ${before} old events kept; ${await snap(
          page,
          "new revision overview",
        )}`;
      },
    );
    await check(
  ...
  try {
    ...
    await check(
      ...
      async () => {
        await expect(guestPage.locator("h1")).toHaveText(
        ...
        return `inlined lesson for the guest: ${
          JSON.stringify(inlined)
        }; ${await snap(guestPage, "guest sign-in prompt")}`;
      },
    );
  } finally {
    await guest.close();
...
async function learningLoop(browser: Browser) {
  ...
  observe(
    "learning loop",
    `${walk.filter((result) => result.ok).length} walk checks passed and ${
      walk.filter((result) => !result.ok).length
    } failed against production; the walk is the ticket 12 script with the production lesson path.`,
  );
}
...
async function secretScan() {
  ...
  await check(
    ...
    () => {
      ...
      return `${bodies.length} bodies scanned (${
        new Set(bodies.map((body) => body.source)).size
      } distinct requests)`;
    },
  );
  void observations;
