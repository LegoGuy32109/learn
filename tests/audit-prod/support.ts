...
export const BASE = (Deno.env.get("LEARN_BASE_URL") ?? "https://learn-joshhale.legoguy32109.deno.net").replace(/\/$/, "");
...
export async function productionSecrets(): Promise<ProductionSecrets> {
  ...
  const ownerToken = parsed.LEARN_OWNER_TOKEN || Deno.env.get("LEARN_OWNER_TOKEN") || "";
  if (!ownerToken) throw new Error("No owner token: put LEARN_OWNER_TOKEN in .env.prod (or LEARN_ENV_FILE) or the environment.");
  ...
  for (const [key, value] of Object.entries(parsed)) {
    ...
    if (key === "TURSO_DB_URL" || (/TOKEN|KEY|SECRET/.test(key) && value.length >= 16)) values.add(value);
  }
...
export function auditTitle(suffix: string): string {
  return `audit-${new Date().toISOString().slice(0, 19).replaceAll(":", "-")}${suffix}`;
}
...
export function describe(method: string, path: string, response: Response, extra: string[] = []): string {
  ...
  const headers = EVIDENCE_HEADERS
    ...
    .map((name) => `${name}: ${name === "set-cookie" ? cookieAttributes(response.headers.get(name) ?? "") : response.headers.get(name)}`);
  return redact([`${method} ${path} -> ${response.status}`, ...headers, ...extra].join("\n"));
}
...
export async function probe(path: string, init: RequestInit = {}, base = BASE): Promise<Probe> {
  const method = init.method ?? "GET";
  const response = await fetch(`${base}${path}`, { redirect: "manual", ...init });
  const text = await response.text();
  const probeResult = { response, text, evidence: describe(method, path, response) };
...
export function recordPageBodies(page: any, label: string) {
  page.on("response", async (response: any) => {
    try {
      ...
      const text = await response.text();
      recordBody(`${label}: ${response.request().method()} ${url.pathname}`, response.status(), text);
    } catch {
...
const TOKEN_SHAPES: { name: string; pattern: RegExp }[] = [
  { name: "personal token with a secret", pattern: /learn_pat_[A-Za-z0-9]+_[A-Za-z0-9_-]{16,}/ },
...
export function scanBodies(known: string[]): string[] {
  ...
  for (const body of bodies) {
    for (const value of known) {
      if (body.text.includes(value)) problems.push(`${body.source} (${body.status}) contains a value from .env.prod`);
    }
    for (const shape of TOKEN_SHAPES) {
      const match = body.text.match(shape.pattern);
      if (match) problems.push(`${body.source} (${body.status}) contains a ${shape.name}: ${redact(match[0]).slice(0, 60)}`);
    }
  }
...
export async function mintInviteWithTask(): Promise<{ url: string; path: string; expiresAt: number }> {
  ...
  if (!output.success) throw new Error(`deno task invite:mint failed (${output.code}): ${redact(stderr.trim() || stdout.trim())}`);
  const line = stdout.trim().split("\n").findLast((candidate) => candidate.startsWith("{"));
  if (!line) throw new Error(`deno task invite:mint printed no JSON: ${redact(stdout.slice(0, 200))}`);
  const minted = JSON.parse(line);
  if (typeof minted.url !== "string" || !minted.url.startsWith(`${BASE}/sign-in/`)) throw new Error("invite:mint returned an unexpected link");
  return minted;
}
...
export function looksLikeStackTrace(text: string): boolean {
  return /\n\s+at\s+\S+ \(|Internal Server Error|TypeError:|ReferenceError:|Deno\.errors/.test(text);
}
